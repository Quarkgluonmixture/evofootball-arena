...
| Discrete touches / the poke window (Phase 36) | push gate in `Match.stepBall` (playing, outfield, action Dribble, `touchTimer≤0`, speed>2.5, nearest opp > `TOUCH_CONTROL_DIST` 4.2 — under it the OLD glue is close control and the tackle duel lives there); push in `mechanics.performDribbleTouch` (base `TOUCH_PUSH_BASE` 1.2 + cone-ahead·`TOUCH_PUSH_SPACE` 0.3, technique × length + wobble 0.07·(1.35−tech·0.7)); poke window = toucher `kickCooldown` `TOUCH_RECOLLECT_COOLDOWN` 0.34; `touchTimer` at capture 0.32+(1−tech)·0.08 ≥ decision settle (restart takers kick FIRST; the pass game keeps its timing); recollect: pFail ×0.45 in `attemptFirstTouch`, decision 0.18, no dribbles++ — probed (touch-rates.ts): 18 pushes/match, flight 0.74s, gap 1.53m, poke 15.8% (first cut 3.5/1.7/0.38 = 39% poked, unplayable) |
