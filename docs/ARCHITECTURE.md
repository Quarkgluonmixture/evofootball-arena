...
| `src/evolution/` | TacticalGenome + squad DNA + evolvable policy genes (`policyGenome.ts`, Phase 42-44) operators, fitness, selection (elite/mutate/reborn), names/kits, franchise lineage | know about matches at runtime (it consumes season aggregates only) |
...
build-up (passBase / passLane / passOpen / outlet / support) — each bounded
...
(`scripts/probes/policy-emergence.ts`); saves v9→v12 backfill the growing key
set. (Player-attribute expansion + a resource budget are the next emergence
levers — deferred to a Fable model, see `docs/EMERGENCE-PIVOT.md`.)
...
| Discrete touches / the poke window (Phase 36 + regimes 36.1) | push gate in `Match.stepBall` (playing, outfield, action Dribble, `touchTimer≤0`, speed>2.5, nearest opp > `TOUCH_CONTROL_DIST` 4.2 — under it the OLD glue is close control and the tackle duel lives there); push in `mechanics.performDribbleTouch`: **base 0.9 + open-cone·0.32 (cap 9m)** so the field prices the touch (一步一带 vs 爆趟), LINE GUARD halves a knock that would roll into touch (12.9% rolled dead without it), technique × length + wobble; poke window = `kickCooldown` **0.26 + 0.04·push** (a knock is a chase); `touchTimer` at capture 0.32 / recollect 0.2 (+low-tech 0.08) ≥ decision settle; recollect pFail ×0.45, decision 0.18, no dribbles++ — probed regimes: stride 41.5% @0.43s/1.14m, mid 55% @1.05s, knock 3.6% @3.56m; poke 16.4% (0.38/0.06 slopes = 26% poked + goals −0.27, pulled back) |
