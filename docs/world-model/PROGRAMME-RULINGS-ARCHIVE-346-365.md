# PROGRAMME — Commander rulings ARCHIVE, #346–#365 (byte-verbatim)

> Rotated out of the live file by ruling **#368** item 6 (2026-09-03; the #303 rotation law).
> Rulings **#346–#365 verbatim** — nothing reworded (GC-T1B → the DX / RA arc → the twelfth
> world's entry). The live file [`PROGRAMME-RULINGS.md`](PROGRAMME-RULINGS.md) holds #366
> onward; earlier eras:
...
> **COMMANDER RULING #353 (2026-08-26 — ⭐⭐ DX-T0 BANKED: the wind-up
> now strikes the elected point when armed, and it is provably asleep
> otherwise; the two pin narrowings RATIFIED; the rulings file rotated
> per #303; DX-T1 THE EXPRESSION EXAM DISPATCHED — block 12,527,000
> opens):**
>
> 1. **LANDED** (seam `f3fa820` → receipts `c388716` → rider
>    `85dca23`; verify PASS-WITH-FINDINGS, 1 MED + 2 LOW, all
>    discharged — DX-T0-WINDUP-AIM-SEAM.md §COMMANDER CORRECTIONS
>    (#353): the G-INERT zero-writes claim re-scoped to what the
>    byte-identity receipts actually prove; the deposit-side
>    behavioural pin ORDERED into DX-T1's §P; the cross-ref pointed
>    and the uncleared-deposit residue stated as a limit). RECEIPTS:
>    fingerprint unmoved (run twice); G-OFF byte-identity across the
>    dispatch commit in both world shapes × 2 scratch seeds (the
>    baseline walked on a pristine worktree of `5174a6c`, the
>    instrument cross-validated against GC-T0 §R1's published
>    digests first — the #344 lesson, applied); G-INERT (armed
>    without the DLC doors ≡ shut, all four cells); G-BITE the
>    strongest of the set — the fixture proves the struck DIRECTION
>    equals the elected point's and the SPEED law reads the elected
>    distance, to 1e-9; 6 mutants killed; 14/14 new pins +
>    neighbouring suites green; scratch seeds only, ZERO
>    frontier/stats. THE SEAM FACTS: the discard site was exactly
>    GC-T1B's O1-WINDUP-PRECEDENCE (armPendingPass re-derived
>    mate.pos and the resolve called performPass at three
>    arguments); the plumb-through is the PW-T0b deposit idiom so
>    the pinned signature and call site stand untouched; exactly ONE
>    pass kind flows through the wind-up channel (the open-play
>    short pass) — machine-pinned.
> 2. ⭐⭐ **THE TWO PIN NARROWINGS RATIFIED** (the DF-T0 §P7 → #323
>    procedure, third exercise): o1PassWindup's two exact-argument
>    pins (`toHaveBeenCalledWith(passer, mate, false)`) could not
>    survive the ordered plumb-through and now pin the two dormant
>    inputs AT THEIR CERTIFIED VALUES (`…, 1, null`) — O1-T1's
>    substantive claims are unweakened and strictly stronger about
>    what the release carries. Deviations 2–7 RATIFIED as declared
>    (the deliberate spelling that kept PTP-T0's ordering pin
>    honest; the dxAimCarried counter REMOVED rather than narrowing
>    a third pin — DX-T1 reads the carry from pendingPassWindup or
>    its own instrumentation; the moving-target composition
>    inherited unchanged per M-DX.1's no-new-physics law).
> 3. **PROCESS — THE ROTATION PERFORMED** (#303's law; the live file
>    passed ~1,500 lines at #352): rulings **#331–#345** moved
>    byte-verbatim (cmp-verified both halves) to
>    [`PROGRAMME-RULINGS-ARCHIVE-331-345.md`](PROGRAMME-RULINGS-ARCHIVE-331-345.md);
>    this file holds **#346 onward** (392 lines).
> 4. ⭐⭐⭐ **DX-T1 DISPATCHED — THE EXPRESSION EXAM** (contract §3;
>    the exam the whole carom arc converges on). Scope bound at
>    dispatch: scored on virgin seeds, frozen rules at dispatch;
>    arms — BOTH the GC-T2 composition (world-11 stack +
>    `dlcDeliveryChoice` + `dlcStrikePlane` + `passLeadSupport` per
>    DLC-T1's idiom + `bkGroundCorridor`, `dvExposureWeight` at the
>    0.5 pin, geneOk a VALUE check), SHUT vs ARMED on `dxWindupAim`
>    ALONE. **H-DX.1**: (a) `altDisplacedShare` on wind-up flights
>    LEAVES ZERO RESOLVEDLY (the door expresses — the 30,318-zero
>    fact dies); (b) ground passes per match hold a non-inferiority
>    band frozen from THIS exam's own shut arm (the GC-T2
>    suppression face re-asked with expression possible); (c) the
>    strike faces DO NOT WORSEN, REPORTED with leave-one-out
>    sensitivity per face (the #346/#348 orders); (d) the
>    lofted-family controls inside THIS shut arm's intervals.
>    ⭐ ALSO IN §P (the #353 rider's order): the deposit-side
>    behavioural pin — a captured armed decision's
>    `pendingPassWindup.aimLead` equals the elected candidate's own
>    displacement. REPORTED: the §P6 re-aim signature (T1B's rule
>    verbatim) · the joint cells · led/grid usage by arm · perf ·
>    the season ladder (probe-side, the DLC gene evolvable). Sizing
>    from GC-T2's published variances at its robust magnitudes,
>    arithmetic shown, gN with the env-escape CLOSED (#348 §CORR
>    2's order). Block **12,527,000–999** opens; stats from
>    **117,600** if published; smokes in scratch. IF H-DX.1 LANDS →
>    the entry-rung question (the DX bundle world) goes to disposal
>    and slice 2 (weight) queues; IF (b) STILL FAILS with
