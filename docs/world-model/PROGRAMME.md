...
**Queue state (2026-07-27, ruling #28 issued): C5 T1 RUNS NEXT** (approved as
...
**(1) C5 T1 PRE-REGISTERED, not run** →
...
**Open, and all the commander's:** re-size C4 T0's coverage floor off this run's
per-archetype rates; re-aim C4 v1 now that C1 is 5.70% and C2 is 27.95%
(arrival vs contest vs the delivery's flight height); rule on Q1; and name the
conversion measurement I2 gates on. C5 T1 is pre-registered and unrun — it may
proceed on its own authority (#27.5) whenever a session takes it. F9 render-only
any time; R20 in gaps.
