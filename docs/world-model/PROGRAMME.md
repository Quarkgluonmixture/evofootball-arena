...
| E3 | Co-evolution audit: sealed evo runs, full bundle ON — §2 band, route mix, the C1-B behavioural suite, no-strict-dominance (power usage must stay situational), co-evo restoration, style diversity | [`EDS-E3-COEVOLUTION-AUDIT.md`](EDS-E3-COEVOLUTION-AUDIT.md) | **PRE-REGISTERED 2026-07-25.** ⚠️ **Scope fact: E3's first act is a BUILD, not a run** — everything to date ran dormant (probe worlds or `forcedPassTarget`), so **nothing has ever chosen a pass inside a live match**; sealed evo runs are ordinary matches with no probe beside them. The new `edsPerceivedChoice` consumer is the last dormant-to-live step in the slice, which is why E3 carries the §2 band C1-B broke. **X4 keeps the chain honest**: the live consumer's per-moment choices must be IDENTICAL to E2b-1R's banked choices on the same staging — if the consumer doesn't reproduce the probe that validated it, the probe validated something else. Gates: §2 band (goals ±15%, routes ±25%) = the EDS thesis' own test, since C1-B broke it with a one-sided cost + speed-blind evaluator; the three behavioural contracts C1-B inverted; **no-strict-dominance 20–80%** = E0's canary made live with E1b's warning attached (cost 6.53pp vs benefit 21.2pp — explicitly unsettled); co-evo restoration + style diversity ≥60% of the flags-off spread. Ruling #10.4 pre-frames the verdict: the evaluator agrees with the live brain only **38–47% even under omniscience**, so play WILL change — the question is whether it stays in band |
...
**Queue state (2026-07-25, after E2b-1R): E3 is drafted; the queue stops at
E4 — the user's play-test.** E2b-1R passed every gate, so E2b's science is
verified rather than banked-with-an-asterisk and the perception layer is
inside budget at 1.069×.
