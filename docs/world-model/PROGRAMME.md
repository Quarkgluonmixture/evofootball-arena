...
**Queue state (2026-07-25, after E2a): STOPPED at a commander gate.** E2a ran
and came back split — the census calibrates, the pricing layer does exactly
what "unseen ≠ unavailable" asks, but **P3 fails by 0.18pp** and the reason is
structural rather than numeric: a base rate measured over the passes the AI
CHOSE to play is a selected sample, and E2b needs a prior for options nobody
chose and nobody can see. E2b is conditioned on an E2a PASS, so it does not
proceed. Two questions are the commander's, both with measured numbers behind
them in [`EDS-E2A-CENSUS-PRIORS.md`](EDS-E2A-CENSUS-PRIORS.md) §7: what
population an honest unseen-option prior is measured over (three candidate
directions listed), and whether P4's routing should be re-keyed from the flat
`interceptedRate` to the 8.32pp `receptionSuccessRate` gradient.
