...
**Queue state (2026-07-25, after E2b-0): E2b-1 is next — the both-sides A/B.**
...
| F7 | **The goal moment** (user 2026-07-25: "进球得有点特效比如烟花喷火之类的"). Started as a check and found a real **F0 regression**: goal particles use ADDITIVE blending, which glowed against the old night diorama and all but vanishes against bright daylight grass — so shipping the toy/day arm had quietly killed every celebration. Blending is now preset-driven (`fxBlending`): solid confetti by day, glow by night. **Fireworks** added — three staggered shells over the main stand, verified rendering. ⛔ **Flame jets BUILT but NOT SHIPPED** (`PYRO_ENABLED = false`): they render nothing and I could not root-cause it within a reasonable budget. Eliminated: vertex colours, particle size (6 m ≈ 36 px still invisible), occlusion (moved from behind the goal line onto the pitch flanking the posts), integration style (rewritten to mirror `Firework` exactly), shader failure (console clean), scene graph (probe shows visible/parented/opacity≈1/full drawRange/sane on-screen NDC) — while the near-identical `Firework` Points renders fine. Kept disabled with the full elimination list in `FxSystem.ts`; next attempt should try Sprites, not more Points debugging. ⭐ Also fixed a perf debt **F6 created**: `CrowdSystem.update` rewrote and re-uploaded all 369×2 instance matrices EVERY frame, double the pre-bowl cost, on the platform where the phone is the binding constraint — calm stands now refresh at 20 Hz (a 3.5 cm idle bob nobody can resolve), eruptions still get every frame, pinned by a test. Gates: tsc + build clean · **731/731** · fingerprint `57b0bdab…c673` unchanged | ⚖️ **PARTIAL — fireworks yes, 喷火 no** |
