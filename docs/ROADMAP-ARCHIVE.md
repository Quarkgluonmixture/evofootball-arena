# Roadmap ARCHIVE — the pre-pivot play-report era

Split out of `ROADMAP.md` (2026-07-17) so the live entry document stays
readable. Everything here is SHIPPED history from the play-report-driven
era (phases 28.5–40, chapters 30–34.3 in detail): still valid as
mechanics reference — the numbers, probes and dials are real — but the
ITERATION MODEL is superseded by `EVO-BLUEPRINT.md` and the live
`ROADMAP.md` head.
