 ...
 * (docs/world-model/IF-T1-FLIGHT-RUN-EXAM.md).
 *
 * Authorized by COMMANDER RULING #420 item 2. Lineage: DS-C0 (the WALKER) → DS-T0/T0b/T0c (the
 * own-run seam) → DS-T1/T1b/T1c → DS-ENTRY (world 16) → DS-T0d + DS-T1d (THE FORM THIS FILE
 * COPIES BY RECIPE — #413 item 5, banked at #414; read 1) → DS-ENTRY-2 (world 17) → IF-C0
 * (THE CENSUS whose Q4 FACES this exam copies BY FIELD NAME) → IF-T0 + IF-T0-FIX (THE SEAM
 * UNDER EXAM: ONE dormant flag `ifFlightRun`, ONE per-body belief `ifLastSeenOwnerGid`, ONE more
 * perceived state and THE EIGHTH `why` — banked-dormant at #420 item 1).
 *
 ...
 * STORES the rows beside IF-C0's as `repro.crossArch`, with `gDeterminism` and `gLockstep`
 * carrying the reproduction burden here; (d) X-FP-PROD is ARCH-KEYED.
 *
 ...
 * ⭐⭐⭐ THE ONE NEW REGISTERED LEDGER READ (registry 86 → 87): `match.ifLastSeenOwnerGid` — the
 * seam's OWN belief, read (never written) for the eighth class's start-state partition.
 *
 ...
 * `decideOffBall` carries the IF-T0 insertion, so all three WHOLE-TEXT hashes are STATED AT THIS
 * HEAD and COMPARED TO NOTHING BANKED.
 */
...
/* ========================================================================== */
const ENV_WHITELIST = ['IFT1_MODE', 'IFT1_N', 'IFT1_OUT'] as const;
...
const rogueOwn = Object.keys(process.env)
  .filter((k) => k.startsWith('IFT1_') && !(ENV_WHITELIST as readonly string[]).includes(k));
const rogueEngine = ENGINE_DOORS.filter((k) => process.env[k] !== undefined);
if (rogueOwn.length > 0 || rogueEngine.length > 0) {
  banner(`IF-T1 FATAL — unrecognised env: ${[...rogueOwn, ...rogueEngine].join(', ')}`);
  process.exit(3);
}
const MODE = process.env.IFT1_MODE as 'smoke' | 'full' | undefined;
if (MODE !== undefined && MODE !== 'smoke' && MODE !== 'full') {
  banner('IF-T1 FATAL — IFT1_MODE must be smoke|full'); process.exit(3);
}
const N_ENV = process.env.IFT1_N !== undefined ? Number(process.env.IFT1_N) : undefined;
if (N_ENV !== undefined && (!Number.isInteger(N_ENV) || N_ENV < 1)) {
  banner('IF-T1 FATAL — IFT1_N must be a positive integer'); process.exit(3);
}
const OUT_ENV = process.env.IFT1_OUT;
const OVERRIDE_REASONS = [
  ...(MODE !== undefined ? [`IFT1_MODE=${MODE}`] : []),
  ...(N_ENV !== undefined ? [`IFT1_N=${N_ENV}`] : []),
  ...(OUT_ENV !== undefined ? [`IFT1_OUT=${OUT_ENV}`] : []),
];
const IS_OVERRIDE = OVERRIDE_REASONS.length > 0;
const CANONICAL_OUT = 'docs/world-model/data/if-t1-flight-run-exam.json';
const CANONICAL_DIR_ABS = pathResolve('docs/world-model/data');
const OUT_BASE = OUT_ENV ?? (IS_OVERRIDE ? '/tmp/if-t1-override.json' : CANONICAL_OUT);
const isCanonical = (p: string): boolean => pathResolve(p).startsWith(CANONICAL_DIR_ABS);
if (IS_OVERRIDE && isCanonical(OUT_BASE)) {
  banner('IF-T1 FATAL — an override run may never write the canonical artifact path');
  process.exit(3);
}
/** ⭐⭐ THE INSTRUMENT OF RECORD — this file's own path, for the stage block's hash. */
const INSTRUMENT_PATH = 'scripts/probes/if-t1-flight-run-exam.ts';
...
/** ⭐⭐⭐ THE SEAM UNDER EXAM — IF-T0's own lines (READ, NEVER TOUCHED), at the FIX's placement. */
...
const IF_BELIEF_RW_LINE = '          ? (ifLastSeenOwner.set(p.gid, ownerGid), ownerGid)';
const IF_STATE_LINE = '        const ifOntoFlight = ifFlightRun && seenBall !== null '
  + '&& ownerGid === null';
...
const IF_BELIEF_FIELD_LINE = '  readonly ifLastSeenOwnerGid: Map<number, number | null>;';
...
anchor('⭐⭐⭐ IF-T0 — THE FLIGHT DOOR\'S ALIAS, the FIRST executable statement INSIDE the own-run '
  + 'fork (#419 item 2\'s repair; `PlayerBrain.ts:2221`)', BRAIN_PATH, IF_DOOR_ALIAS_LINE, 1);
anchor('⭐⭐⭐ IF-T0 — THE BELIEF\'S ALIAS, the SECOND statement inside the fork (`:2222`)',
  BRAIN_PATH, IF_BELIEF_ALIAS_LINE, 1);
anchor('⭐⭐⭐ M-IF.2 — THE BELIEF\'S ONE WRITE SITE and ONE READ SITE, his OWN entry by his own gid',
  BRAIN_PATH, IF_BELIEF_RW_LINE, 1);
anchor('⭐⭐⭐ M-IF.1 — THE EIGHTH STATE\'S OWN TEST: the door · a ball SEEN with NO owner · the '
  + 'remembered owner resolving on the ROSTER to a mate who is not him. IDENTITY TESTS ONLY',
  BRAIN_PATH, IF_STATE_LINE, 1);
...
anchor('⭐⭐ the belief\'s readonly field', MATCH_PATH, IF_BELIEF_FIELD_LINE, 1);
...
/* ========================================================================== */
const BLOCK_BASE = 12_559_000;
const BLOCK_TOP = 12_559_999;
...
const SCRATCH_BASE = 900_008_400;
...
if (L3_DOSE_BYTES_SHA !== L3_DOSE_PIN || PC_DOSE_BYTES_SHA !== PC_DOSE_PIN) {
  banner('IF-T1 FATAL — a dose file\'s BYTES do not match the pinned value');
...
if (!D13_REACHABLE) {
  banner(`IF-T1 FATAL — the D13 arms are not reachable: ${DOSE_LOAD_ERROR ?? 'empty dose'}`);
  process.exit(3);
}
 ...
 *  ANOTHER MATE. THE ONE NEW REGISTERED LEDGER READ (registry 86 → 87). */
...
interface MatchView {
  ...
  ifFlightRun?: boolean; ifLastSeenOwnerGid?: Map<number, number | null>;
      ...
      if (runNow && !runActiveAll[i]) {
        ...
        if (armPulls && (cls === 'ownRunInBehind' || cls === IF_CLASS)) {
          ...
          if (cls === IF_CLASS) {
            ...
            const remembered = beliefMap === null ? null : (beliefMap.get(p.gid) ?? null);
...
/** ⭐⭐⭐ THE FIVE-MEMBER READ SET (#419 item 2, #420 item 1(b)). DS-T0's doc names THREE — a
 *  sentence dated to ITS head; IF-T0's §LAW §2 names THE TWO THIS STAGE ADDS. The gate PARSES
 *  BOTH homes and compares their UNION to the block's own extracted member set. Neither
 *  sentence is edited; a difference is RED. */
const IF_SEAM_DOC_PATH = 'docs/world-model/IF-T0-FLIGHT-RUN-SEAM.md';
const IF_SEAM_DOC = readFileSync(IF_SEAM_DOC_PATH, 'utf8');
const DOC_READ_SET_TWO_ADDED = (() => {
  const flat = IF_SEAM_DOC.replace(/\s+/g, ' ');
  const m2 = /THE TWO THIS STAGE ADDS, NAMED.{0,20}`match\.(\w+)` \(the door\) and `match\.(\w+)`/
    .exec(flat);
  return m2 === null ? [] : [m2[1], m2[2]].map((x) => `match.${x}`).sort();
})();
const DOC_READ_SET = [...new Set([...DOC_READ_SET_THREE, ...DOC_READ_SET_TWO_ADDED])].sort();
/** ⭐⭐⭐ THE FIX'S OWN PLACEMENT (#420 item 1(b)): the two alias reads are the FIRST TWO
 *  EXECUTABLE STATEMENTS inside `if (match.dsOwnRun) {`, at `PlayerBrain.ts` 2221–2222. */
...
const IF_ALIASES_ARE_THE_FIRST_TWO_STATEMENTS = (() => {
  const lines = SRC_OF[BRAIN_PATH].split('\n');
  const head = IF_ALIAS_LINES.fork;
  if (head < 1) return false;
  const body: string[] = [];
  for (let k = head; k < lines.length && body.length < 2; k++) {
 ...
 *  the SAME expression, both his OWN entry by his OWN gid. */
...
const BLOCK_MEMBERS_AGREE = OWN_BLOCK !== null && DOC_READ_SET.length === 5
  && DOC_READ_SET_THREE.length === 3 && DOC_READ_SET_TWO_ADDED.length === 2
  && JSON.stringify(OWN_BLOCK.members) === JSON.stringify(DOC_READ_SET)
  && IF_ALIASES_ARE_THE_FIRST_TWO_STATEMENTS
...
fx('block.theMemberSetIsFIVEMembers', OWN_BLOCK === null ? 0 : OWN_BLOCK.members.length, 5);
fx('block.theTHREEInheritedMembersAreDSTs',
  DOC_READ_SET_THREE, ['match.dsOwnRun', 'match.perceivedSnapshot', 'match.simTime']);
fx('block.theTWOThisSeamADDSAreNamedInITSOwnDoc',
  DOC_READ_SET_TWO_ADDED, ['match.ifFlightRun', 'match.ifLastSeenOwnerGid']);
fx('block.theTWOAliasesAreTheFIRSTTWOStatementsINSIDETheFork',
  IF_ALIASES_ARE_THE_FIRST_TWO_STATEMENTS, true);
fx('block.theirLINESAreTheFIXSOwn', [IF_ALIAS_LINES.door, IF_ALIAS_LINES.belief],
  [IF_ALIAS_LINES.fork + 8, IF_ALIAS_LINES.fork + 9]);
...
/* ========================================================================== */
banner('IF-T1 — the lockstep receipt (observed vs unobserved, PER ARM)');
  ...
  return {
    ...
    asDue: ARM_CARRIES_IF[armK]
      ...
      : rowArm.ifWhyDecisions === 0 && rowArm.ifEpSets === 0
        && rowArm.beliefEntriesAtFullTime === 0,
  };
}));
...
const cells: Cell[] = [];
banner(`IF-T1 — the battery: ${N} seeds × ${ARMS.length} arms, seeds `
 ...
 *  the re-walked rows are STORED BESIDE IF-C0's as `repro.crossArch` with EVERY differing field
...
defFace('ifStart.stampedStartsPerMatch', 'stamped eighth-class run starts per match',
  'the denominator of the two partitions above, per match', 'matches',
...
const face = (k: string, armK: Arm): FaceRow => {
  ...
  if (f === undefined) { banner(`IF-T1 FATAL — unknown face ${k}/${armK}`); process.exit(3); }
  return f as FaceRow;
};
...
const delta = (faceKey: string, cid: Cid): DeltaRow => {
  ...
  if (dd === undefined) { banner(`IF-T1 FATAL — unknown Δ ${faceKey}@${cid}`); process.exit(3); }
...
const READ_LITERAL_HOMES = (() => {
  const homes = [
    ...
    { home: CONTRACT_PATH, what: '§3 IF-T1',
...
const eighthClassFor = (armK: Arm) => ({
  ...
  belief: {
    ...
    theOneNewRegisteredRead: 'match.ifLastSeenOwnerGid (registry 86 → 87) — READ, never '
 ...
 *  900,008,400–411 (FIVE walks per seed), transcribed here from §DEV-PREFLIGHT and re-derived
 *  off the artifact by `gFaces`. The declared target (#420 item 2(v)) is a 0.05 HALF-WIDTH on
 *  R1's paired Δ (the candidate vs world 17, E13) AND on the NEGATIVE-Δt HALF's paired Δ on
 *  the SAME pair. */
const SMOKE_HW_R1 = 0.09404095922915776;
const SMOKE_HW_NEGATIVE_DT = 0.02472186703045473;
const SIZING_INPUTS = [
...
const CONSUMED_BLOCKS = [12_544_000, 12_545_000, 12_546_000, 12_547_000, 12_548_000,
  ...
  12_556_000, 12_557_000, 12_558_000];
...
const gates: Record<string, { ok: boolean; note: string }> = {
  ...
  gLedgerRead: {
    ...
    note: '⭐⭐ canon, VERBATIM: "an event attribution reads the engine\'s own record when one '
      ...
      + '`pcLatency.ledger.decisionsHeld` itself (debt (a)\'s calibration target); ⭐⭐⭐ THE ONE '
      + 'NEW REGISTERED READ OF THIS STAGE (registry 86 → 87): `match.ifLastSeenOwnerGid`, THE '
      + 'SEAM\'S OWN BELIEF — READ, NEVER WRITTEN, for the eighth class\'s START-STATE MEMORY '
      + 'PARTITION and for the belief\'s own size at full time (which `gArmIF` asserts EMPTY on '
      + 'every arm without the door). WHERE NO '
...
const artifact: Record<string, unknown> = {
  stage: {
    id: 'IF-T1',
    title: '\u300c\u7403\u5728\u98de\u65f6\u7684\u524d\u63d2 \u00b7 \u8003\u300d THE FLIGHT RUN\'S EXAM \u2014 with world 17\'s door set in '
      ...
      + 'START-STATE, RESTRAINT and YIELD partitions, PLUS IF-C0\'s Q4 FACES copied BY FIELD '
      + 'NAME with their \u2248 CROSS-ARCHITECTURE twins.',
    doc: 'docs/world-model/IF-T1-FLIGHT-RUN-EXAM.md',
    instrument: INSTRUMENT_PATH,
    instrumentSha256: sha(readFileSync(INSTRUMENT_PATH, 'utf8')),
    inheritedInstrument: 'scripts/probes/ds-t1d-coop-hats-exam.ts',
    inheritedStageDoc: 'docs/world-model/DS-T1D-COOP-HATS-EXAM.md',
    ...
    seamUnderExam: 'docs/world-model/IF-T0-FLIGHT-RUN-SEAM.md §LAW',
    ...
    authorizedBy: 'COMMANDER RULING #420 item 2',
    ...
    honestLimitsNote: '⛔ canon, VERBATIM: "a stage doc\'s HONEST LIMITS list is the ONE home; '
      ...
      + 'The ONE home is docs/world-model/IF-T1-FLIGHT-RUN-EXAM.md §HONEST LIMITS.',
  ...
  sizing: {
    ...
    varianceSource: 'THIS exam\'s own 12-cluster SCRATCH SMOKE (seeds 900,008,400–411, FIVE '
  ...
  stats: { consumed: 0, nextBase: 117_600, registryOfRecord: 87 },
  perf: {
...
banner('');
banner(`IF-T1 — ${ALL_GREEN_FINAL ? 'ALL GATES GREEN' : '⛔ RED — artifact routed to .RED'}`);
