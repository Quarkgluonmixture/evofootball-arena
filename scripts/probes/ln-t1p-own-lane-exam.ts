 ...
 * (docs/world-model/LN-T1P-OWN-LANE-EXAM.md).
 *
 * Authorized by COMMANDER RULING #394 item 4. Lineage: PT-C0 (the population and the
 * `ball.lastTouch` FIRST-BODY channel, byte for byte) -> BN-C0 (the corridor membership test)
 * -> LN-C0 (the walker, the wind-up ARM-tick channel, the cause classes, the estimator, the
 * hash order) -> LN-T1 (the EXAM form: paired Δ, the guard tolerance `NI_FRACTION`, the band,
 * the offside FLAG form, FLAG-HYGIENE, G-ARM, LOO, the sizing rule) -> LN-C1 (the choice tick,
 * the own-openness CALLED reconstruction, the menu) -> LN-C2 (the ledger PATH classes, the
 * shell at the choice) -> LN-C3 (the FAMILY rule, the strike sites, the trace join, the walker,
 * the extracted call graph) -> LN-T0 (the SEAM under exam) -> this exam.
 *
...
/* ========================================================================== */
const ENV_WHITELIST = ['LNT1P_MODE', 'LNT1P_N', 'LNT1P_OUT'] as const;
...
const rogueOwn = Object.keys(process.env)
  .filter((k) => k.startsWith('LNT1P_') && !(ENV_WHITELIST as readonly string[]).includes(k));
const rogueEngine = ENGINE_DOORS.filter((k) => process.env[k] !== undefined);
if (rogueOwn.length > 0 || rogueEngine.length > 0) {
  banner(`LN-T1P FATAL — unrecognised env: ${[...rogueOwn, ...rogueEngine].join(', ')}`);
  process.exit(3);
}
const MODE = process.env.LNT1P_MODE as 'smoke' | 'full' | undefined;
if (MODE !== undefined && MODE !== 'smoke' && MODE !== 'full') {
  banner('LN-T1P FATAL — LNT1P_MODE must be smoke|full'); process.exit(3);
}
const N_ENV = process.env.LNT1P_N !== undefined ? Number(process.env.LNT1P_N) : undefined;
if (N_ENV !== undefined && (!Number.isInteger(N_ENV) || N_ENV < 1)) {
  banner('LN-T1P FATAL — LNT1P_N must be a positive integer'); process.exit(3);
}
const OUT_ENV = process.env.LNT1P_OUT;
const OVERRIDE_REASONS = [
  ...(MODE !== undefined ? [`LNT1P_MODE=${MODE}`] : []),
  ...(N_ENV !== undefined ? [`LNT1P_N=${N_ENV}`] : []),
  ...(OUT_ENV !== undefined ? [`LNT1P_OUT=${OUT_ENV}`] : []),
];
const IS_OVERRIDE = OVERRIDE_REASONS.length > 0;
const CANONICAL_OUT = 'docs/world-model/data/ln-t1p-own-lane-exam.json';
const CANONICAL_DIR_ABS = pathResolve('docs/world-model/data');
const OUT_BASE = OUT_ENV ?? (IS_OVERRIDE ? '/tmp/ln-t1p-own-lane-exam-override.json' : CANONICAL_OUT);
const isCanonical = (p: string): boolean => pathResolve(p).startsWith(CANONICAL_DIR_ABS);
if (IS_OVERRIDE && isCanonical(OUT_BASE)) {
  banner('LN-T1P FATAL — an override run may never write the canonical artifact path');
  process.exit(3);
}
...
const SELF_PATH = 'scripts/probes/ln-t1p-own-lane-exam.ts';
...
const LNC3_KICKOFF_SPAN = (() => {
  ...
  if (node === undefined) {
    banner('LN-T1P FATAL — LN-C3\'s banked artifact carries no `kickoffPlaybackScorer` node');
    process.exit(3);
  }
  return {
...
/* ========================================================================== */
/* §4 SEEDS — block 12,549,000–999 (#394 item 4(vii) / item 7's frontier)      */
/* ========================================================================== */
const BLOCK_BASE = 12_549_000;
const BLOCK_TOP = 12_549_999;
...
const N_FROZEN = 91; /* §DEV-PREFLIGHT: SIZED, see §15 — min(nRequired, blockAffords) */
const N = N_ENV ?? (MODE === 'smoke' ? 3 : N_FROZEN);
const SCRATCH_BASE = 900_003_900;
...
if (L3_DOSE_BYTES_SHA !== L3_DOSE_PIN || PC_DOSE_BYTES_SHA !== PC_DOSE_PIN) {
  banner('LN-T1P FATAL — a dose file\'s BYTES do not match the pinned value (#388 item 2(i))');
  process.exit(3);
}
...
if (!DOSED_ARM_REACHABLE) {
  banner(`LN-T1P FATAL — the DOSED arm is not reachable from Node: ${DOSE_LOAD_ERROR ?? 'empty dose'}`);
  process.exit(3);
}
...
const emptyRow = (): Row => ({
  ...
  lnGeneOnInfoGenome: false, signature: '',
});
...
banner('LN-T1P — the lockstep receipt (observed vs unobserved, PER ARM)');
...
banner('LN-T1P — the TRACE-INERTNESS receipt (traced vs untraced, PER ARM)');
...
if (!LOCKSTEP_TRACE_OK) {
  banner('LN-T1P FATAL — the choice ledger is NOT byte-inert: the exam is BLOCKED. No battery '
    + 'seed is walked.');
  process.exit(4);
}
...
const runCore = (pass: number): { cells: Cell[]; receipt: Record<Arm, Row> } => {
  const out: Cell[] = [];
  banner(`LN-T1P — pass ${pass}: ${N} seeds × ${ARMS.length} arms, seeds `
...
banner(`  [ln-t1p] pass 1 digest ${digestA} — X-DET second pass...`);
const coreB = runCore(2);
const digestB = coreDigest(coreB);
const X_DET = digestA === digestB;
banner(`  [ln-t1p] pass 2 digest ${digestB} — X-DET ${X_DET ? 'PASS' : 'FAIL'}`);
...
banner(`  [ln-t1p] X-FP-PROD ${X_FP_PROD ? 'PASS' : 'FAIL'} (${fpObserved.slice(0, 16)}…)`);
...
banner(`LN-T1P — G-REPRO-LNC3: re-walking LN-C3 seeds ${REPRO_LNC3_SEEDS[0]}–`
...
const HYGIENE_EXCLUDED_FIELDS = ['lnFlag', 'lnWeightEff', 'lnWeightBase', 'lnWeightInfo',
  'lnGeneOnInfoGenome', 'wallMs', 'cpBlindRead'] as const;
...
const face = (k: string, armK: Arm): FaceRow => {
  ...
  if (f === undefined) { banner(`LN-T1P FATAL — unknown face ${k}/${armK}`); process.exit(3); }
  return f as FaceRow;
};
...
const delta = (faceKey: string, armK: Arm): DeltaRow => {
  ...
  if (dd === undefined) { banner(`LN-T1P FATAL — unknown Δ ${faceKey}@${armK}`); process.exit(3); }
 ...
 *  smoke artifact's own `deltas[].halfWidth` field (12 seeds, 900,003,900–911) and written here
 *  BEFORE the FREEZE commit and BEFORE any battery seed. ⛔ Never re-typed from the console's
 *  rounded print. The variance source is the CEILING arm W100 (LN-T1's form). */
const SMOKE_HW_R1 = 0.019262551319936264;
...
/** ⭐ THE PUBLISHED LEDGER, QUOTED (#394 item 7) — the frontier and the consumed intervals. */
...
const PUBLISHED_FRONTIER_AT_394: number = 12_549_000;
const SEED_DISJOINT = (BLOCK_BASE as number) === PUBLISHED_FRONTIER_AT_394
...
const gates: Record<string, { ok: boolean; note: string }> = {
  ...
  gArm: {
    ...
    note: '⭐⭐⭐ THE DOSE IS WHERE THE BRAIN READS IT. On EVERY dosed seed the gene reads back as '
      ...
      + 'effective genome". ⭐ LIVENESS, because the seam has no counter of its own: the '
      + 'WHOLE-MATCH SIGNATURE differs between ABSENT and W100 on '
  ...
  gSeedDisjoint: {
    ...
    note: 'SEED-DISJOINT against the PUBLISHED ledger: the block base equals the frontier of '
      + `record at #394 item 7 (${PUBLISHED_FRONTIER_AT_394}); the block `
      ...
      + 'G-REPRO-LNC3 seeds lie inside LN-C3\'s OWN already-consumed block and are DECLARED '
...
const BODY_SCHEMA = [
  ...
  'reproLnc3', 'xDet', 'xFpProd', 'codeFactGraph', 'lnC3KickoffSpan',
...
const artifact: Record<string, unknown> = {
  stage: {
    id: 'LN-T1P',
    title: '「传球者看见自己人 · 考试」 THE OWN-LANE EXAM — does the own-lane price, at doses '
    ...
    lineage: 'PT-C0 (the population and the `ball.lastTouch` FIRST-BODY channel) → BN-C0 (the '
      ...
      + 'graph) → LN-T0 (the SEAM under exam) → this exam',
    userVerdictVerbatim: '12我看了下,还是有人挤人,传不出去球,传到人身上弹回,或经常传到对面身上',
    kind: 'EXAM — pre-registered rules, frozen read sentences. It ARMS NOTHING for the user: the '
  ...
  gArm: { rows: gArmRows, ok: G_ARM_OK },
  ...
  seeds: {
    ...
    publishedFrontierAt394: PUBLISHED_FRONTIER_AT_394,
  ...
  stats: { consumed: 0, nextBase: 117_600, registryOfRecord: 78 },
  perf: {
  ...
  honestLimitsNote: '⛔ canon, VERBATIM: "a stage doc\'s HONEST LIMITS list is the ONE home; the '
    ...
    + 'of record is THIS exam\'s own doc, docs/world-model/LN-T1P-OWN-LANE-EXAM.md §HONEST '
    + 'LIMITS.',
  gates,
...
banner('');
banner(`LN-T1P — ${ALL_GREEN_FINAL ? 'ALL GATES GREEN' : '⛔ RED — artifact routed to the .RED path'}`);
