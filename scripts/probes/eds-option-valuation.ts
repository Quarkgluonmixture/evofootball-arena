// EDS E0 PASS-OPTION VALUATION vs THE MEASURED WORLD.
...
const runExperiment = () => {
  ...
  const exact = {
    ...
    finite: nonFinite === 0,
    struck: notStruck === 0,
    noNullValuations: totalNullValuations === 0,
    everyStatePredicted: withPredictions.length === acceptedStates,
  ...
  const prediction = {
    ...
    p2TouchCostPredictedNearlyFlat:
      (predMeans[2].touchFailPrior - predMeans[0].touchFailPrior) < 0.03,
  ...
  return {
    experiment: 'EDS-E0',
    authority: 'EDS-E0-OPTION-VALUATION',
    parameters: {
...
console.error(
  `EDS-E0 ${output.verdict} · accepted ${output.support.acceptedStates}/${REQUIRED_STATES}`
