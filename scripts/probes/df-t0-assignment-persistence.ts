...
/**
 * ============================================================================
 * DF-T0 — ASSIGNMENT PERSISTENCE: THE RECEIPT WALKS (armed vs shut)
 * ============================================================================
 * ORDERED BY COMMANDER RULING #322 item 2, bound by
 * `docs/world-model/DF-DEFENSIVE-BRAIN-CONTRACT.md` §2 (M-DF.1/M-DF.2/M-DF.4).
 *
 ...
 * units. NO football claim is made here; H-DF.1 is a later stage's business.
 *
 * WHAT IT MEASURES — the four churn faces of DF-C0 §R2, whose instrument
 * definitions are REUSED VERBATIM (the `Row` accumulator, `familyOf`,
 * `DUP_RUN_M = 4`, the latency bins, the swarm radii and every face's
 * (numerator, denominator) pair are copied from
 * `scripts/probes/df-c0-defensive-brain-census.ts`, which this file never
 * touches):
 *   · markSwitchesPerDefenderMinute   (乱跑 itself)
 *   · dupMarkShare                    (the dupRun-lineage double-marking face)
 *   · markHeldShare                   (assignment coverage)
 *   · reTargetLatencyMeanS            (the re-target wait, with STORED BINS)
 * …plus the SWARM BAND and the CAP's own output (`chaserCountBins`), which
 * must be intact ARMED (M-DF.2: two compensators never move in one slice).
 *
 ...
 *     §CORR item 1) — the swarm radii, §2.
 *   · "a field carries the unit its name claims" (home ruling #294 item 3).
 *   · clock honesty (paraphrase) — every rate on the match clock AND per
 *     defender-minute; APPLIED values.
 ...
 *   accepted: DFT0_MODE (smoke|full, REQUIRED) · DFT0_OUT.
 *   ANY other `DFT0_*` var is a FATAL refusal, and so is ANY engine env door.
 *   An override run may not write the canonical path.
 *
 * RUN: DFT0_MODE=full npx tsx scripts/probes/df-t0-assignment-persistence.ts
...
/* ========================================================================== */
const ENV_WHITELIST = ['DFT0_MODE', 'DFT0_OUT'] as const;
const ENGINE_DOORS = ['EDS_BUNDLE_ARMED', 'A4_WORLD', 'EVO_', 'SIM_'];
for (const k of Object.keys(process.env)) {
  if (k.startsWith('DFT0_') && !(ENV_WHITELIST as readonly string[]).includes(k)) {
...
const MODE = process.env.DFT0_MODE;
if (MODE !== 'smoke' && MODE !== 'full') {
  console.error('ENV REFUSAL: DFT0_MODE must be smoke|full');
  process.exit(2);
}
const OUT_OVERRIDE = process.env.DFT0_OUT ?? null;
const CANONICAL_OUT = 'docs/world-model/data/df-t0-assignment-persistence.json';
...
/* ========================================================================== */
/** DF-T0's OWN booked block (ruling #322 item 2): 12,509,000–999. */
const BLOCK_BASE = 12_509_000;
 ...
 * re-typed literal. Both are the SAME lines DF-C0 §R2 extracted.
 */
...
/** the DF-C0 §R2 world, verbatim, plus THIS slice's door as the only arm difference */
...
/* ========================================================================== */
/* §5 FACES — DF-C0 §R2's (numerator, denominator) pairs, verbatim            */
/* ========================================================================== */
...
for (const arm of ['shut', 'armed'] as const) {
  for (const [k, d] of Object.entries(CHURN_FACES)) faces.push(faceOf(arm, k, d));
}
...
/* ---- the CAP's own output, pooled per arm (M-DF.2's receipt) ---- */
const chaserBins = {
  shut: Array.from({ length: 5 }, (_, b) =>
    sum(rows.filter((r) => r.arm === 'shut').map((r) => r.chaserCountBins[b]))),
  armed: Array.from({ length: 5 }, (_, b) =>
    sum(rows.filter((r) => r.arm === 'armed').map((r) => r.chaserCountBins[b]))),
};
const swarmBins = {
  shutStance: Array.from({ length: SWARM_BINS }, (_, b) =>
    sum(rows.filter((r) => r.arm === 'shut').map((r) => r.swarmStanceBins[b]))),
  armedStance: Array.from({ length: SWARM_BINS }, (_, b) =>
    sum(rows.filter((r) => r.arm === 'armed').map((r) => r.swarmStanceBins[b]))),
  shutZone: Array.from({ length: SWARM_BINS }, (_, b) =>
    sum(rows.filter((r) => r.arm === 'shut').map((r) => r.swarmZoneBins[b]))),
  armedZone: Array.from({ length: SWARM_BINS }, (_, b) =>
    sum(rows.filter((r) => r.arm === 'armed').map((r) => r.swarmZoneBins[b]))),
};
...
const perf = {
  ...
  budgetNote: 'DF-C0 §R5: 2 % of the anchor tick. THIS IS A WALL MEASUREMENT taken inside '
    + 'the instrumented walk loop (the instrument itself is inside the timer), so it is an '
    + 'UPPER BOUND on the seam\'s cost, never the engine\'s own profiler number.',
...
gates.gArmsPairedPerSeed = seedsWalked.every((s) =>
  rows.filter((r) => r.seed === s).length === 2);
...
gates.gCapIntactArmed = chaserBins.armed[4] === 0 && chaserBins.shut[4] === 0;
gates.gCapBinsNonEmpty = sum(chaserBins.armed) > 0 && sum(chaserBins.shut) > 0;
/** the ledger never aliases: two markers on one man is impossible by construction */
...
gates.gArmsDistinguishable =
  faceValue('armed', 'markSwitchesPerDefenderMinute') !== faceValue('shut', 'markSwitchesPerDefenderMinute');
...
/* ========================================================================== */
/* §7 THE ARTIFACT — allowlist schema, body hashed LAST (DF-C0 §CORR item 2)  */
/* ========================================================================== */
const instrumentSha = createHash('sha256')
  .update(readFileSync(new URL(import.meta.url))).digest('hex');
const body = {
  stage: 'DF-T0-ASSIGNMENT-PERSISTENCE',
  kind: 'receipt-walks (NOT an exam; canon: receipts ≠ effect sizes)',
  ruling: '#322 item 2',
  contract: 'DF-DEFENSIVE-BRAIN-CONTRACT.md §2 M-DF.1/M-DF.2/M-DF.4',
  mode: MODE,
  isOverrideRun: IS_OVERRIDE,
  instrument: {
    file: 'scripts/probes/df-t0-assignment-persistence.ts',
    sha256: instrumentSha,
    instrumentDefinitionsReusedFrom: 'scripts/probes/df-c0-defensive-brain-census.ts §R2 '
      + '(Row accumulator, familyOf, DUP_RUN_M, latency bins, swarm radii, face pairs)',
  },
  world: {
    version: DF_WORLD,
    doors: 'a4MatchFlags(9) + bkFacingLaw + bkContactLaw (DF-C0 §R2\'s world, verbatim)',
    armDifference: 'dfAssignPersist only',
  },
  seeds: { block: `${BLOCK_BASE}–${BLOCK_BASE + 999}`, booked: SEEDS, walked: seedsWalked, walksTotal: rows.length },
...
console.log(`\nchurn (shut → armed):`);
for (const f of ['markSwitchesPerDefenderMinute', 'dupMarkShare', 'markHeldShare', 'reTargetLatencyMeanS']) {
