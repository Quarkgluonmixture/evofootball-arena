  ...
  for (let g = 0; g < GENS; g++) {
    ...
    if (g === 0 || g === Math.floor(GENS / 2) || g === GENS - 1) {
      ...
      console.log(
        ...
        `shootB ${gene('shootBias').toFixed(2)} press ${gene('pressIntensity').toFixed(2)} width ${gene('attackingWidth').toFixed(2)} jockey ${gene('jockeyBias').toFixed(2)} cover ${gene('coverBias').toFixed(2)} aggr ${gene('keeperAggression').toFixed(2)} trap ${gene('trapBias').toFixed(2)}`,
      );
    }
  }
