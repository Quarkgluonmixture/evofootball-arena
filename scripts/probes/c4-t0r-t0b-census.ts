...
// Authority: docs/world-model/C4-T0R-T0B.md (commander rulings #28.4, #29.4)
// Derives from T0's probe (`c4-t0-arrival-census.ts`, unedited and kept).
//
// T0R — the cross rate is population-bound and the match budget is not (#24),
//       so each combination gets its OWN budget sized to a common cross target
//       instead of a common match count. Budgets derive from the SEEN 909k
//       block; certification happens on two FRESH blocks (C3R).
// T0b — C2 decomposes on `tryAerial`'s own gate order, ex ante: the ladder
//       H0 height-preempted / H1 keeper / H2 taken down at height / H3 no
//       contender at height / H4 residual. Its split DECIDES C4 v1's re-aim,
//       so §6's decision rule was frozen before any number existed.
//
...
const runBlock = (seedStart: number, budgetOf: (tag: string) => number) => {
  const combos: ReturnType<typeof describeCombo>[] = [];
  const pooled: CrossRow[] = [];
  for (const atk of attackers) {
    for (const shell of shells) {
      const tag = `${atk.tag} vs ${shell.tag}`;
      const { rows, matches } = harvestCombo(atk, shell, seedStart, budgetOf(tag));
...
const runExperiment = () => {
  const build = runBlock(BUILD_SEED_START, budgetOf);
  const heldout = runBlock(HELDOUT_SEED_START, budgetOf);
  ...
  // S2: the C2 ladder must be a property of the world, not of a block.
  const ladderStability = RUNGS.map((r) => {
    const delta = Math.abs(build.c2Ladder[r].share - heldout.c2Ladder[r].share);
    return { rung: r, delta: round(delta), holds: delta <= S2_TOLERANCE };
  });
  ...
  const coverage = {
    ...
    buildC2: build.c2Pooled >= C2_POOLED_FLOOR,
    heldoutC2: heldout.c2Pooled >= C2_POOLED_FLOOR,
  };

  // §6's decision rule, frozen before any number existed: H0's share of C2.
  const h0 = build.c2Ladder.H0heightPreempted;
  const reAim = h0.ci.lower > 0.5 ? 'HEIGHT-DOMINATED'
    : h0.ci.upper < 0.5 ? 'CONTEST-DOMINATED' : 'MIXED';

  const gates = {
    x5Partition: build.partitionOk && heldout.partitionOk,
    x4Rollup: build.rollupOk && heldout.rollupOk,
    x6LadderPartition: build.ladderPartitionOk && heldout.ladderPartitionOk,
    c1ComboCoverage: coverage.buildCombos && coverage.heldoutCombos,
    c2PooledCoverage: coverage.buildPooled && coverage.heldoutPooled,
    c3C2Coverage: coverage.buildC2 && coverage.heldoutC2,
    s1Stability: stability.every((row) => row.holds),
    s2LadderStability: ladderStability.every((row) => row.holds),
  };

  return {
    experiment: 'C4-T0R+T0b',
    authority: 'C4-T0R-T0B',
    parameters: {
      crossTarget: CROSS_TARGET, matchBudget: MATCH_BUDGET, window: WINDOW,
      buildSeedStart: BUILD_SEED_START, heldoutSeedStart: HELDOUT_SEED_START,
      pinSeedStart: PIN_SEED_START, pinMatches: PIN_MATCHES,
      headerRadius: HEADER_RADIUS,
      ...
      s2Tolerance: S2_TOLERANCE,
      comboCrossFloor: COMBO_CROSS_FLOOR, pooledCrossFloor: POOLED_CROSS_FLOOR,
      c2PooledFloor: C2_POOLED_FLOOR,
    },
    build, heldout, stability, ladderStability, coverage,
    // Reported, never a gate — §6 makes it a DECISION input, which is a
    // different job (contract §4.4).
    reAim: { rule: 'H0 share of C2, 95% cluster CI vs 0.50', h0, verdict: reAim },
    gates,
  };
};
...
/**
 * X4 — the rollup pin, on `cross-anatomy`'s own staging, which is the only
 * block where the external comparison exists. Run ONCE, outside the
 * determinism pair (T1's `tempo` precedent, disclosed in contract §2.2): it is
 * a fixed-staging equality check whose content IS the comparison, and the
 * census the determinism pin covers is judged on the fresh blocks.
 */
const pin = runBlock(PIN_SEED_START, () => PIN_MATCHES);
const gates = {
  ...first.gates,
  x4Rollup: first.gates.x4Rollup && pin.rollupOk,
  dDeterministic: deterministic,
};
const output = {
  ...first,
  x4Pin: {
    seedStart: PIN_SEED_START, matches: PIN_MATCHES, rollupOk: pin.rollupOk,
    // Compared by hand against the unmodified probe's printed output.
    perCombo: pin.combos.map((c) => ({
      tag: c.tag, crossesPerMatch: c.crossesPerMatch, oldWay: c.oldWay,
    })),
  },
  gates,
  sha256,
...
console.error(
  `C4-T0R+T0b ${output.verdict} · build ${output.build.pooledCrosses} crosses`
  + ` / heldout ${output.heldout.pooledCrosses}`
  + ` · per-combo build ${output.build.combos.map((c) => c.crosses).join('/')}`
  + ` heldout ${output.heldout.combos.map((c) => c.crosses).join('/')}`
  ...
  + ` · S1 ${output.stability.map((s) => `${s.klass} ${(s.delta * 100).toFixed(2)}pp`).join(' ')}`
  + ` · C2 LADDER (n=${output.build.c2Pooled}) `
  + Object.entries(output.build.c2Ladder).map(([r, v]) => (
    `${r} ${pct(v.share)} CI[${pct(v.ci.lower)},${pct(v.ci.upper)}]`
  )).join(' ')
  + ` · S2 ${output.ladderStability.map((s) => `${s.rung} ${(s.delta * 100).toFixed(2)}pp`).join(' ')}`
  + ` · RE-AIM ${output.reAim.verdict}`
  + ` · X4 pin rollup ${output.x4Pin.rollupOk} ${output.x4Pin.perCombo.map((c) => (
    `${c.tag} ${(c.oldWay.atkHeader * 100).toFixed(1)}/${(c.oldWay.defHeader * 100).toFixed(1)}`
    + `/${(c.oldWay.noAerial * 100).toFixed(1)} (${c.crossesPerMatch}/m)`
  )).join(' | ')}`
  + ` · I2 baseline shot ${pct(output.build.conversionBaseline.shotWithinWindow)}`
  + ` goal ${pct(output.build.conversionBaseline.goalWithinWindow)}`
  + ` (heldout ${pct(output.heldout.conversionBaseline.goalWithinWindow)})`
