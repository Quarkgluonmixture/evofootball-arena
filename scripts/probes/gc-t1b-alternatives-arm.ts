/**
 * ⭐⭐ GC-T1B — THE ALTERNATIVES ARM (docs/world-model/GC-T1B-ALTERNATIVES-ARM.md).
 *
 * Authorized by COMMANDER RULING #345 item 5 (the dispatch), bound by
 * docs/world-model/GC-GROUND-CORRIDOR-CONTRACT.md §3 and re-arming GC-T1's own instruments
 * (docs/world-model/GC-T1-GROUND-CORRIDOR-EXAM.md). The reading it probes is #345 item 4's
 * LABELLED HYPOTHESIS: the world-11 chooser holds NO alternative ground lines to re-aim
 * through, so a binary price can only make a blocked line worse — 「别传了」 instead of
 * 「换条线传」. This exam gives the chooser the BANKED alternatives (DLC-T1 `4d0120e` #238 and
 * DLC-T1s `fe955b9` #243) in BOTH arms and asks whether the SUPPRESSION converts to RE-AIM.
 *
 * THE SCORED PAIR — the world-11 stack + the DLC pair, SHUT vs ARMED on `bkGroundCorridor`:
 *   · `shut`  = a4MatchFlags(11) + `dlcDeliveryChoice` + `dlcStrikePlane` + armA4World(m,null,11)
 *               + `passLeadSupport` written match-local at the banked value 1
 *   · `armed` = THE SAME, plus the ONE construction flag `bkGroundCorridor: true`
 * `dvExposureWeight` sits at WORLD 11's OWN 0.5 pin in BOTH arms (the SHIPPED `armCorridorWorld`
 * → `setCorridorWeight`), and `passLeadSupport` sits at 1 in BOTH arms. ⭐ THE DLC AXIS DOES NOT
 * DIFFER BETWEEN THE ARMS — only the ground price does.
 *
 * ⚠⚠ THE FROZEN PRECEDENCE LAW, MEASURED AND NOT ASSUMED (src/sim/Match.ts's own words on
 * `dlcStrikePlane`): "`ptpPassLead` and `dlcDeliveryChoice` keep PRECEDENCE — no grid forms while
 * either seat exists, so armed-both is the banked door armed alone, byte for byte". The dispatch
 * orders BOTH doors armed; under that law the ORDERED world IS the two-point contest and the K=9
 * grid is structurally inert in it. The composition proof MEASURES that identity rather than
 * quoting it, and — so the commander's alternatives question is not answered by half — a SECOND,
 * REPORTED-ONLY pair walks the grid door alone:
 *   · `shutPlane` / `armedPlane` = a4MatchFlags(11) + `dlcStrikePlane` ALONE + gene 1, shut vs
 *     armed on `bkGroundCorridor`. ⛔ REPORTED, NEVER GATED, NEVER PART OF H-GC.2's VERDICT.
 *
 * H-GC.2, the four conjuncts, FROZEN at §P of the doc BEFORE any battery seed was read (the
 * forms are ruling #345 item 5's; the numbers are THIS battery's own shut arm's):
 *   (a) the ground-strike faces FALL RESOLVEDLY — `groundStrikesPerMatch` and
 *       `caromedGroundOnOpenLaneShare`, paired per-seed bootstrap Δ, 95 % interval below zero.
 *   (b) ⭐ NON-SUPPRESSION — `armed.groundPassesPerMatch` sits AT OR ABOVE THIS EXAM's OWN SHUT
 *       arm's 95 % interval LOWER EDGE (BK-T4 §P3's band construction, no taste constant).
 *   (c) `teammateStrikesPerMatch` falls, the same paired form as (a).
 *   (d) the LOFTED-FAMILY CONTROLS stay INSIDE THIS SHUT ARM's own 95 % intervals.
 * REPORTED, NEVER GATED: ⭐ THE RE-AIM SIGNATURE (the blocked/clear joint cells by arm beside
 * `deliveriesPerMatch`, with GC-T1's published cells quoted as DIFFERENT-BATTERY CONTEXT) ·
 * the led/grid USAGE SHARE by arm · completion + possession spells · the interception
 * decomposition · perf (GC-T1 §P7's method) · the plane pair.
 *
 * ⛔ NO SEASON LADDER (#345 item 5 orders none; GC-T1 §R6's ladder stands).
 *
 ...
 *   accepted: GCT1B_MODE (smoke|full, REQUIRED) · GCT1B_N · GCT1B_OUT.
 *   ANY other `GCT1B_*` var is a FATAL refusal, and so is ANY of the ENGINE's own env doors.
 *   Every override is an OVERRIDE run: it may not write a canonical repo path.
 *
 * RUN: GCT1B_MODE=full npx tsx scripts/probes/gc-t1b-alternatives-arm.ts
...
/* ========================================================================== */
const ENV_WHITELIST = ['GCT1B_MODE', 'GCT1B_N', 'GCT1B_OUT'] as const;
...
const rogueOwn = Object.keys(process.env)
  .filter((k) => k.startsWith('GCT1B_') && !(ENV_WHITELIST as readonly string[]).includes(k));
const rogueEngine = ENGINE_DOORS.filter((k) => process.env[k] !== undefined);
if (rogueOwn.length > 0 || rogueEngine.length > 0) {
  banner('GC-T1B FATAL — refused env surface. '
    + `rogue GCT1B_*: [${rogueOwn.join(', ')}] · ENGINE doors set: [${rogueEngine.join(', ')}]. `
    + `The whitelist is exactly: ${ENV_WHITELIST.join(' | ')} (whitelist-or-refuse).`);
  process.exit(2);
}
const MODES = ['smoke', 'full'] as const;
type Mode = (typeof MODES)[number];
const MODE = process.env.GCT1B_MODE as Mode | undefined;
if (MODE === undefined || !(MODES as readonly string[]).includes(MODE)) {
  banner(`GC-T1B FATAL — GCT1B_MODE is REQUIRED and must be one of ${MODES.join(' | ')}.`);
  process.exit(2);
}
const N_ENV = process.env.GCT1B_N !== undefined ? Number(process.env.GCT1B_N) : undefined;
if (N_ENV !== undefined && (!Number.isInteger(N_ENV) || N_ENV < 1 || N_ENV > 160)) {
  banner('GC-T1B FATAL — GCT1B_N must be an integer in [1, 160].');
  process.exit(2);
}
const OUT_ENV = process.env.GCT1B_OUT;
const PREFLIGHT_REASONS = [
  ...(MODE === 'smoke' ? ['mode=smoke'] : []),
  ...(N_ENV !== undefined ? ['GCT1B_N set'] : []),
];
const IS_PREFLIGHT = PREFLIGHT_REASONS.length > 0;
const CANONICAL_OUT = 'docs/world-model/data/gc-t1b-alternatives-arm.json';
const CANONICAL_DIR_ABS = pathResolve('docs/world-model/data');
const OUT_BASE = OUT_ENV ?? (IS_PREFLIGHT ? '/tmp/gc-t1b-override.json' : CANONICAL_OUT);
...
if (IS_PREFLIGHT && isCanonical(OUT_BASE)) {
  banner(`GC-T1B FATAL — an OVERRIDE run (${PREFLIGHT_REASONS.join(', ')}) may not write a `
...
const SEAM_OK = GC_FORK_HITS.length === 1 && GC_PRICE_HITS.length === 1
  ...
  && SP_PRECEDENCE_HITS.length === 1 && DLC_ABSENT_FROM_A4;
if (!ANCHORS_OK || !SEAM_OK) {
  banner('GC-T1B FATAL — the anchored extraction / seam pin did not land. '
    ...
    + `dlcAbsentFromA4=${DLC_ABSENT_FROM_A4}`);
  process.exit(3);
}
 ...
 */
const GCT1_PATH = 'docs/world-model/data/gc-t1-ground-corridor-exam.json';
const GCT1_BYTES = readFileSync(GCT1_PATH, 'utf8');
const GCT1_SHA = sha(GCT1_BYTES);
interface GcT1Artifact {
...
const GCT1 = JSON.parse(GCT1_BYTES) as GcT1Artifact;
const gct1Face = (k: string): { value: number; ci95: [number, number] } => {
  const f = GCT1.faces.find((x) => x.face === k);
  if (f === undefined) { banner(`GC-T1B FATAL — GC-T1 face ${k} not found`); process.exit(3); }
  return f!;
};
/** ⭐ THE §P-FROZEN GC-T1 LITERALS (its own §R fields, at source precision) */
const GCT1_FROZEN = {
  'shut.groundPassesPerMatch': { value: 72.86875, ci95: [70.775, 74.9] },
  'armed.groundPassesPerMatch': { value: 69.58125, ci95: [67.59375, 71.5125] },
  'shut.deliveriesPerMatch': { value: 78.075, ci95: [76.14375, 79.975] },
  'armed.deliveriesPerMatch': { value: 75.08125, ci95: [73.275, 76.85625] },
  'shut.groundStrikesPerMatch': { value: 16.48125, ci95: [15.2375, 17.8125] },
  'armed.groundStrikesPerMatch': { value: 15.825, ci95: [14.19375, 18.0625] },
} as Record<string, { value: number; ci95: number[] }>;
const GCT1_JOINT_FROZEN: Record<string, number[][]> = {
  shut: [[1447, 6365], [1772, 2075]],
  armed: [[1134, 6349], [1632, 2018]],
};
const GCT1_QUOTED_OK = Object.entries(GCT1_FROZEN).every(([k, v]) => {
  const f = gct1Face(k);
  return f.value === v.value && f.ci95[0] === v.ci95[0] && f.ci95[1] === v.ci95[1];
}) && ['shut', 'armed'].every((a) => JSON.stringify(GCT1.staleMap.byArm[a].jointLaneOpenByShellBlocked)
  === JSON.stringify(GCT1_JOINT_FROZEN[a]));

/* ========================================================================== */
/* §4 THE ARMS — the world-11 stack, SHUT vs ARMED, armed by CALLING a4World   */
/* ========================================================================== */
/**
 * ⭐⭐ FOUR ARMS, TWO PAIRS. The SCORED pair is `shut` / `armed` — the dispatch's own arms. The
 * `shutPlane` / `armedPlane` pair is REPORTED ONLY and enters NO conjunct of H-GC.2.
 */
const ARMS = ['shut', 'armed', 'shutPlane', 'armedPlane'] as const;
type Arm = (typeof ARMS)[number];
/** the SCORED pair, named once so no face table can quietly re-point the verdict */
const SCORED_PAIR = ['shut', 'armed'] as const;
const PLANE_PAIR = ['shutPlane', 'armedPlane'] as const;
const PAIRS: readonly (readonly [Arm, Arm])[] = [
  [SCORED_PAIR[0], SCORED_PAIR[1]], [PLANE_PAIR[0], PLANE_PAIR[1]],
];
const isPricedArm = (a: Arm): boolean => a === 'armed' || a === 'armedPlane';
...
const DOORS: Record<Arm, { dlcDeliveryChoice: boolean; dlcStrikePlane: boolean }> = {
  ...
  shutPlane: { dlcDeliveryChoice: false, dlcStrikePlane: true },
  armedPlane: { dlcDeliveryChoice: false, dlcStrikePlane: true },
};
...
const buildMatch = (arm: Arm, seed: number): Match => {
  ...
  const m = new Match({
    ...
    ...DOORS[arm],
    ...(isPricedArm(arm) ? { bkGroundCorridor: true } : {}),
...
const walkMatch = (
  ...
  row.flagOn = m.bkGroundCorridor === true;
  row.worldOk = row.armedVersion === CORRIDOR_WORLD_VERSION
    && (compositionWalk || row.flagOn === expectFlag);
  /**
  ...
  const readChoice = (p: Player, targetGid: number, tick: number, fromWindup: boolean,
    aim: V2, refPos: V2): ChoiceRead | null => {
  ...
  const bookFlight = (f: Flight): void => {
    ...
    if (c.fromWindup) {
      row.altDecisions++;
      const displaced = c.aimDisplacementMetres > 0;
      if (displaced) {
        row.altDisplaced++;
        row.altDispSumMetres += c.aimDisplacementMetres;
        row.altDispBins[binOf(c.aimDisplacementMetres, DISP_BIN_M, DISP_BINS)]++;
      }
...
const COMP_SEEDS = [SCRATCH_BASE + 300, SCRATCH_BASE + 301, SCRATCH_BASE + 302];
type CompWorld = 'base' | 'bothAbsent' | 'bothZero' | 'bothOne' | 'contestOnly'
  | 'planeAbsent' | 'planeOne';
const COMP_WORLDS: Record<CompWorld, {
  doors: { dlcDeliveryChoice?: boolean; dlcStrikePlane?: boolean }; gene: number | null;
}> = {
  base: { doors: {}, gene: null },
  bothAbsent: { doors: { dlcDeliveryChoice: true, dlcStrikePlane: true }, gene: null },
  bothZero: { doors: { dlcDeliveryChoice: true, dlcStrikePlane: true }, gene: 0 },
  bothOne: { doors: { dlcDeliveryChoice: true, dlcStrikePlane: true }, gene: DLC_GENE_VALUE },
  contestOnly: { doors: { dlcDeliveryChoice: true }, gene: DLC_GENE_VALUE },
  planeAbsent: { doors: { dlcStrikePlane: true }, gene: null },
  planeOne: { doors: { dlcStrikePlane: true }, gene: DLC_GENE_VALUE },
};
...
for (const seed of COMP_SEEDS) {
  for (const w of Object.keys(COMP_WORLDS) as CompWorld[]) {
    ...
    const m = new Match({
      ...
      ...a4MatchFlags(CORRIDOR_WORLD_VERSION), ...spec.doors,
    } as ConstructorParameters<typeof Match>[0]);
    armA4World(m, null, CORRIDOR_WORLD_VERSION);
    if (spec.gene !== null) {
...
const COMP_RELATIONS = [
  { name: 'G-BORN.bothDoors', law: 'world 11 + BOTH DLC doors with the gene ABSENT is BYTE-'
    + 'IDENTICAL to world 11 alone (each door\'s own born-absent identity).',
  ok: compIdentity('bothAbsent', 'base') },
  { name: 'G-ZERO.contest', law: 'world 11 + BOTH doors at gene 0 is BYTE-IDENTICAL to world 11 '
    + 'alone — DLC-T0 §LAW\'s own G-law (the candidate FORMS at 0 and loses every tie; #238\'s '
    + 'ARMED-ZERO ≡ ABSENT receipt).',
  ok: compIdentity('bothZero', 'base') },
  { name: 'G-BITE.bothOne', law: 'the SCORED arms\' world (gene 1) DIFFERS from world 11 alone — '
    + 'the doors reach the chooser. ⚠ AN ARMING RECEIPT, never an effect size.',
  ok: compDiffers('bothOne', 'base') },
  { name: 'G-PRECEDENCE', law: '⭐⭐ world 11 + BOTH doors at gene 1 is BYTE-IDENTICAL to world '
    + '11 + `dlcDeliveryChoice` ALONE at gene 1 — src/sim/Match.ts\'s frozen precedence law '
    + '("no grid forms while either seat exists, so armed-both is the banked door armed alone, '
    + 'byte for byte"), MEASURED here rather than quoted.',
  ok: compIdentity('bothOne', 'contestOnly') },
  { name: 'G-BORN.planeAlone', law: 'world 11 + `dlcStrikePlane` ALONE with the gene ABSENT is '
    + 'BYTE-IDENTICAL to world 11 alone (#243\'s PLANE-INERT ≡ ABSENT identity).',
  ok: compIdentity('planeAbsent', 'base') },
  { name: 'G-BITE.planeOne', law: 'the REPORTED pair\'s world (`dlcStrikePlane` alone, gene 1) '
    + 'DIFFERS from world 11 alone — the grid reaches the chooser.',
  ok: compDiffers('planeOne', 'base') },
  ...
  ok: (['bothOne', 'contestOnly', 'planeOne'] as CompWorld[]).every((w) => compRows
    .filter((r) => r.world === w).some((r) => r.ledNonZero > 0))
    && (['base', 'bothAbsent', 'bothZero', 'planeAbsent'] as CompWorld[]).every((w) => compRows
      .filter((r) => r.world === w).every((r) => r.ledNonZero === 0)) },
  ...
  { name: 'O1-WINDUP-PRECEDENCE', law: '⭐⭐ THE COMPOSITION FACT THIS PROOF EXISTS TO FIND: on '
    + 'the world-11 stack `o1PassWindup` is ARMED, and `PlayerBrain.decideOnBall`\'s execution '
    + 'branch takes the wind-up FIRST — "the O1 wind-up keeps precedence (that banked seam owns '
    + 'the release timing; with both armed the strike resolves through the wind-up\'s own aim, '
    + 'an honest limit stated in the stage doc)". `armPendingPass` stores `aim: {x: mate.pos.x, '
    + 'y: mate.pos.y}` and resolves through `performPass(passer, mate, offsideExempt)` with NO '
    + 'lead. So on THIS composition a wind-up pass CANNOT carry a re-aim, and the relation '
    + 'asserted here is the MEASUREMENT of that: in EVERY world the wind-up seat\'s own aim is '
    + 'displaced from the target\'s pre-step position on EXACTLY ZERO decisions.',
  ok: compRows.every((r) => r.displacedDecisions === 0) },
];
...
/* ========================================================================== */
const BLOCK_BASE = 12_525_000;
/**
 * ⭐ THE SUB-BAND SPLIT, DECLARED AT §P (block 12,525,000–999, consumed WHOLE of record):
 *   · the SCORED pair   = 12,525,000–159 (160 seeds × 2 arms = 320 walks)
 *   · the REPORTED pair = 12,525,200–359 (160 seeds × 2 arms = 320 walks)
 *   · the construction receipts = 12,525,999, one per arm (4 constructions)
 * The COMPOSITION PROOF and the SIZING SMOKE walk the OUT-OF-BAND SCRATCH range only.
 */
const N_SEEDS = N_ENV ?? (MODE === 'smoke' ? 3 : 160);
const SMOKE_SEEDS = Array.from({ length: 40 }, (_, i) => SCRATCH_BASE + 400 + i);
...
const PLANE_SEEDS = MODE === 'smoke'
  ? SMOKE_SEEDS.slice(0, Math.min(N_SEEDS, SMOKE_SEEDS.length))
  : Array.from({ length: N_SEEDS }, (_, i) => BLOCK_BASE + 200 + i);
const RECEIPT_SEED = MODE === 'smoke' ? SMOKE_SEEDS[0] : BLOCK_BASE + 999;
const SEEDS_OF: Record<Arm, number[]> = {
  shut: SCORED_SEEDS, armed: SCORED_SEEDS, shutPlane: PLANE_SEEDS, armedPlane: PLANE_SEEDS,
};

interface Cell4 { index: number; scoredSeed: number; planeSeed: number; rows: Record<Arm, Row> }
const cells: Cell4[] = [];
for (let i = 0; i < SCORED_SEEDS.length; i++) {
  const sSeed = SCORED_SEEDS[i];
  const pSeed = PLANE_SEEDS[i];
  ...
  const rows = {
    ...
    shutPlane: walkMatch(buildMatch('shutPlane', pSeed), false, false),
    armedPlane: walkMatch(buildMatch('armedPlane', pSeed), true, false),
  } as Record<Arm, Row>;
  cells.push({ index: i, scoredSeed: sSeed, planeSeed: pSeed, rows });
  if (cells.length % 10 === 0 || cells.length === SCORED_SEEDS.length) {
...
const flagDiffPlane = flagDiffOf(PLANE_PAIR[0], PLANE_PAIR[1]);
...
const dlcAxisIdentical = ARMS.every((a) => receiptFacts[a].dlcStrikePlane === true)
  && SCORED_PAIR.every((a) => receiptFacts[a].dlcDeliveryChoice === true)
  && PLANE_PAIR.every((a) => receiptFacts[a].dlcDeliveryChoice === false);
...
perArm('altDisplacedShare', (r) => r.altDisplaced, (r) => r.altDecisions,
  ...
  '⭐⭐ REPORTED — THE USAGE SHARE: of WIND-UP-SEAT measured ground passes, the share whose '
  + 'ENGINE-RECORDED aim differs from the named target\'s own position at the choice tick ⇒ a '
  + 'NON-TO-FEET candidate (a led delivery, or a sampled grid member) won the argmax. '
  + '⛔ NOT A DECLINE RATE — DLC-T1s\'s retraction rides (§8A): an undisplaced kick may be a '
  + 'declined LIVE alternative or a DEGENERATE one, and this instrument cannot separate them.',
 ...
 * PAIRED by construction. ⚠ The two PAIRS walk DIFFERENT seed sub-bands, so a resampled cell
 * carries the scored pair's seed and the plane pair's seed together — the pairing is within a
 * pair, never across the two pairs, and NO Δ is ever computed across pairs.
 */
const BOOTSTRAP = 2000;
const scoredSeedsWalked = [...new Set(cells.map((c) => c.scoredSeed))].sort((a, b) => a - b);
const planeSeedsWalked = [...new Set(cells.map((c) => c.planeSeed))].sort((a, b) => a - b);
...
const face = (k: string): FaceRow => {
  ...
  if (f === undefined) { banner(`GC-T1B FATAL — unknown face ${k}`); process.exit(3); }
  return f!;
};
...
const DELTA_KEYS = ['groundStrikesPerMatch', 'caromedGroundOnOpenLaneShare',
...
const deltaOf = (k: string, pair: readonly [Arm, Arm]): DeltaRow => {
  ...
  if (d === undefined) { banner(`GC-T1B FATAL — unknown delta ${k}`); process.exit(3); }
  return d!;
};
 ...
 */
const A_KEYS = ['groundStrikesPerMatch', 'caromedGroundOnOpenLaneShare'];
const D_KEYS = ['loftedDeliveriesPerMatch', 'crossesPerMatch'];
const conjunctA = A_KEYS.map((k) => ({ key: k, pass: delta(k).excludesZeroBelow }));
...
const conjunctC = {
  key: 'teammateStrikesPerMatch', pass: delta('teammateStrikesPerMatch').excludesZeroBelow,
};
...
const H_GC_2 = {
  a: conjunctA.every((x) => x.pass),
  b: conjunctB.pass,
  c: conjunctC.pass,
  d: conjunctD.every((x) => x.pass),
};
const H_GC_2_ALL = H_GC_2.a && H_GC_2.b && H_GC_2.c && H_GC_2.d;
...
const gates: Record<string, boolean> = {
  ...
  gWorld: cells.every((c) => ARMS.every((a) => c.rows[a].worldOk))
    ...
    && ARMS.every((a) => receiptFacts[a].bkGroundCorridor === isPricedArm(a))
   ...
   */
  gArmsIsolated: flagDiffScored.length === 1 && flagDiffScored[0] === 'bkGroundCorridor'
    && flagDiffPlane.length === 1 && flagDiffPlane[0] === 'bkGroundCorridor',
  ...
  gSharedSeeds: cells.length === SCORED_SEEDS.length
    && cells.every((c, i) => c.scoredSeed === SCORED_SEEDS[i] && c.planeSeed === PLANE_SEEDS[i])
   ...
   */
  gPriceFires: ARMS.filter(isPricedArm).every(
  ...
  gQuotationsFaithful: BKC2_QUOTED_OK && BKC2_SHA.length === 64 && BKC2_FACES.length > 0
    && GCT1_QUOTED_OK && GCT1_SHA.length === 64,
  /**
  ...
  gSeedsBookedEqualWalked: scoredSeedsWalked.length === SCORED_SEEDS.length
    && planeSeedsWalked.length === PLANE_SEEDS.length
    && scoredSeedsWalked.every((sd) => SCORED_SEEDS.includes(sd))
    && planeSeedsWalked.every((sd) => PLANE_SEEDS.includes(sd))
    && cells.length * ARMS.length + ARMS.length === walksBooked
    && (MODE === 'smoke' || [...scoredSeedsWalked, ...planeSeedsWalked, RECEIPT_SEED]
...
const BODY_SCHEMA = ['stage', 'definitions', 'world', 'compositionProof', 'hypothesis', 'seeds',
  'stats', 'strikeCensus', 'staleMap', 'reAimSignature', 'alternativesUsage', 'perf',
  'quotedContext', 'walkFixtures', 'perSeedCells', 'faces', 'deltas', 'gates'] as const;
...
const reAimSignature = {
  ...
  gcT1Context: {
    warning: '⚠⚠ DIFFERENT BATTERY, DIFFERENT WORLD, DIFFERENT SEEDS: GC-T1 walked the world-11 '
      + 'stack WITHOUT the DLC pair on block 12,524,000–159. These cells are quoted so the '
      + 'signature can be READ BESIDE its predecessor. NO gate compares them and NO Δ is '
      + 'computed across the two batteries.',
    source: { file: GCT1_PATH, sha256: GCT1_SHA,
    ...
    gcT1HashedBodySha256: GCT1.hashedBodySha256,
    jointFrozen: GCT1_JOINT_FROZEN,
    facesFrozen: GCT1_FROZEN,
    gcT1Measured: {
      shut: GCT1.staleMap.byArm.shut.measuredGroundPasses,
      armed: GCT1.staleMap.byArm.armed.measuredGroundPasses,
      shutDeliveries: GCT1.staleMap.byArm.shut.deliveries,
      armedDeliveries: GCT1.staleMap.byArm.armed.deliveries,
    },
  },
};
...
const quotedContext = {
  ...
  gcT1: {
    source: { file: GCT1_PATH, sha256: GCT1_SHA },
    frozenLiterals: GCT1_FROZEN,
    frozenJoint: GCT1_JOINT_FROZEN,
    frozenLiteralsMatchArtifact: GCT1_QUOTED_OK,
  },
};
...
const hypothesis = {
  id: 'H-GC.2',
  frozenAt: 'docs/world-model/GC-T1B-ALTERNATIVES-ARM.md §P, at the FREEZE COMMIT, before a '
    + 'battery seed was read. The conjunct forms are ruling #345 item 5\'s; the gate '
    + 'CONSTRUCTIONS are stated there and are not re-cut after sight.',
  scoredPair: `${SCORED_PAIR[0]}→${SCORED_PAIR[1]}`,
  reportedPair: `${PLANE_PAIR[0]}→${PLANE_PAIR[1]} — REPORTED ONLY, in NO conjunct`,
  a: {
    rule: 'at BOTH named faces the PAIRED per-seed bootstrap Δ (armed − shut) has its 95 % '
      + 'interval ENTIRELY BELOW ZERO (the house resolution form)',
    faces: A_KEYS, conjuncts: conjunctA, pass: H_GC_2.a,
  },
  b: {
    rule: 'the ARMED arm\'s `groundPassesPerMatch` point estimate sits AT OR ABOVE THIS EXAM\'S '
      + 'OWN SHUT arm\'s 95 % interval LOWER EDGE (BK-T4 §P3 / GC-T1 §P4\'s band construction — '
      + 'the band is DERIVED from this battery\'s own control arm, no taste constant)',
    detail: conjunctB, pass: H_GC_2.b,
  },
  c: {
    rule: 'the same paired form as (a), on `teammateStrikesPerMatch`',
    detail: { ...conjunctC, delta: delta('teammateStrikesPerMatch') }, pass: H_GC_2.c,
  },
  d: {
    rule: 'each lofted-family control\'s ARMED point estimate lies INSIDE THIS EXAM\'S OWN SHUT '
      + 'arm\'s 95 % interval (BK-T4 §P2\'s control form)',
    faces: D_KEYS, conjuncts: conjunctD, pass: H_GC_2.d,
  },
  allConjunctsPass: H_GC_2_ALL,
  honestLimits: [
    '⚠ NO CLAIM THAT THE CAROM DISAPPEARS (contract §4): the price is a price, not a wall.',
    '⚠⚠ THE ORDERED ARMS ARM BOTH DLC DOORS, AND THE FROZEN PRECEDENCE LAW MAKES THE K = 9 GRID '
    + 'STRUCTURALLY INERT IN THEM (measured at `compositionProof.relations[G-PRECEDENCE]`). The '
    + 'alternatives the SCORED verdict is about are therefore the TWO-POINT CONTEST\'s led '
    + 'candidates. The grid door\'s own effect is the REPORTED pair\'s, and it is not scored.',
    '⚠ THE DLC GENE IS PINNED AT 1 IN EVERY ARM (DLC-T1\'s own `PTP_GENE_MAX`). This exam walks '
    + 'NO gene ladder on either axis; `dvExposureWeight` is pinned at world 11\'s own 0.5.',
    '⚠ THE USAGE SHARE IS NOT A DECLINE RATE (DLC-T1s\'s retraction rides — see '
    ...
    '⚠ NO SEASON LADDER IS WALKED (#345 item 5 orders none). GC-T1 §R6\'s ladder stands and is '
    + 'not re-run; nothing here is a claim about evolution.',
    '⚠ THIS EXAM\'S SHUT ARM IS NOT GC-T1\'s SHUT ARM (it carries the DLC pair) and not BK-C2\'s '
    + '`w11` world. Every quotation from either is labelled DIFFERENT-BATTERY CONTEXT.',
  ],
};
...
const artifact: Record<string, unknown> = {
  stage: {
    id: 'GC-T1B',
    title: 'THE ALTERNATIVES ARM — the DLC pair armed in BOTH arms, shut vs armed on the '
      + 'ground price',
    doc: 'docs/world-model/GC-T1B-ALTERNATIVES-ARM.md',
    contract: 'docs/world-model/GC-GROUND-CORRIDOR-CONTRACT.md',
    predecessor: 'docs/world-model/GC-T1-GROUND-CORRIDOR-EXAM.md',
    seam: 'docs/world-model/GC-T0-DORMANT-SEAM.md',
    ...
    authorizedBy: 'COMMANDER RULING #345 item 5 (contract §3)',
    ...
    userWordsOfRecord: [
      '但是弹身体感觉很影响比赛 (#341 item 1 — THE RED this arc serves)',
    ...
    instrumentSha256: sha(readFileSync('scripts/probes/gc-t1b-alternatives-arm.ts', 'utf8')),
  ...
  definitions: {
    ...
    displacedDecision: 'a WIND-UP-SEAT measured ground pass whose ENGINE-RECORDED aim differs '
      + 'from the named target\'s position at the choice tick — i.e. a NON-TO-FEET candidate won '
      + 'the argmax. ⛔ Not a decline rate (see `alternativesUsage.declaredLimit`).',
  ...
  world: {
    ...
    reportedPair: PLANE_PAIR,
    stacks: {
      shut: 'a4MatchFlags(11) + `dlcDeliveryChoice` + `dlcStrikePlane` + armA4World(m, null, 11) '
        + '+ `passLeadSupport` 1 on the match-local views. `bkGroundCorridor` FALSE.',
      armed: 'THE SAME, plus the ONE construction flag `bkGroundCorridor: true`.',
      shutPlane: 'a4MatchFlags(11) + `dlcStrikePlane` ALONE + armA4World(m, null, 11) + '
        + '`passLeadSupport` 1. `bkGroundCorridor` FALSE. ⛔ REPORTED ONLY.',
      armedPlane: 'THE SAME, plus `bkGroundCorridor: true`. ⛔ REPORTED ONLY.',
    },
    theOnlyDifferenceScored: flagDiffScored,
    theOnlyDifferencePlane: flagDiffPlane,
    ...
    geneNote: '`dvExposureWeight` sits at world 11\'s own 0.5 pin and `passLeadSupport` at the '
      + 'banked DLC value 1, in EVERY arm, on MATCH-LOCAL views only. ⭐ THE DLC AXIS DOES NOT '
      + 'DIFFER BETWEEN THE SCORED ARMS — only the ground price does.',
  ...
  seeds: {
    ...
    subBands: {
      ...
      reportedPairBattery: `${BLOCK_BASE + 200}–${BLOCK_BASE + 200 + N_SEEDS - 1} `
        + `(${N_SEEDS} seeds x 2 arms)`,
      constructionReceipt: RECEIPT_SEED,
    },
    bookedScored: SCORED_SEEDS,
    bookedPlane: PLANE_SEEDS,
    walkedScored: scoredSeedsWalked,
    walkedPlane: planeSeedsWalked,
...
binChecks.push(['composition.reDerives', (() => {
  ...
  const want: Record<string, boolean> = {
    'G-BORN.bothDoors': ident('bothAbsent', 'base'),
    'G-ZERO.contest': ident('bothZero', 'base'),
    'G-BITE.bothOne': diff('bothOne', 'base'),
    'G-PRECEDENCE': ident('bothOne', 'contestOnly'),
    'G-BORN.planeAlone': ident('planeAbsent', 'base'),
    'G-BITE.planeOne': diff('planeOne', 'base'),
    LIFECYCLE: rows.every((r) => r.armedVersion === CORRIDOR_WORLD_VERSION
      && r.ledgerZeroAtBirth && r.franchiseClean && r.finished),
    'CANDIDATES-FORM': (['bothOne', 'contestOnly', 'planeOne'] as CompWorld[])
      .every((w) => rows.filter((r) => r.world === w).some((r) => r.ledNonZero > 0))
      && (['base', 'bothAbsent', 'bothZero', 'planeAbsent'] as CompWorld[])
        .every((w) => rows.filter((r) => r.world === w).every((r) => r.ledNonZero === 0)),
    'G-LOCKSTEP': rows.every((r) => r.untracedSignature === r.signature),
    'O1-WINDUP-PRECEDENCE': rows.every((r) => r.displacedDecisions === 0),
  };
...
binChecks.push(['quotations.reRead', (() => {
  ...
  if (pub.gcT1.source.sha256 !== sha(readFileSync(GCT1_PATH, 'utf8'))) return false;
  return Object.entries(pub.bkC2.frozenLiterals).every(([k, v]) => {
    const src = bkc2(k);
    return src.value === v.value && src.ci95[0] === v.ci95[0] && src.ci95[1] === v.ci95[1];
  }) && Object.entries(pub.gcT1.frozenLiterals).every(([k, v]) => {
    const src = gct1Face(k);
    return src.value === v.value && src.ci95[0] === v.ci95[0] && src.ci95[1] === v.ci95[1];
  });
})()]);
...
binChecks.push(['verdict.reDerives', (() => {
  ...
  const fOf = (k: string): { value: number; ci95: [number, number] } | undefined =>
    (disk.faces as { face: string; value: number; ci95: [number, number] }[])
      .find((x) => x.face === k);
  const aOk = A_KEYS.every((k) => (dOf(k)?.excludesZeroBelow ?? false)) === h.a.pass;
  ...
  const cOk = (dOf('teammateStrikesPerMatch')?.excludesZeroBelow ?? false) === h.c.pass;
...
/* ========================================================================== */
banner('');
banner('=== GC-T1B — THE ALTERNATIVES ARM ===');
...
for (const a of ARMS) {
  ...
  for (const k of ['groundStrikesPerMatch', 'caromedGroundOnOpenLaneShare',
    ...
    'passStrikesPerMatch', 'altDisplacedShare',
...
banner('--- H-GC.2 (scored on shut→armed ONLY) ---');
banner(`  (a) ${H_GC_2.a ? 'PASS' : 'FAIL'} · (b) ${H_GC_2.b ? 'PASS' : 'FAIL'} `
  + `(band ${round(bBand, 6)}, armed ${round(bArmed, 6)}) · (c) ${H_GC_2.c ? 'PASS' : 'FAIL'} `
  + `· (d) ${H_GC_2.d ? 'PASS' : 'FAIL'}`);
