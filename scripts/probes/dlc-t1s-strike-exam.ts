// DLC T1s — THE STRIKE-PLANE EXAM (控制的是那一脚): does making THE KICK ITSELF a priced choice
// keep the two-point contest's gain and shed its residual cost? The banked GROUND STRIKE PLANE
// (DLC-T0s, 54a45df/8333911) dosed on the DLC-T1 instrument set INHERITED WHOLE.
//
// Doc:      docs/world-model/DLC-T1S-STRIKE-EXAM.md (§FORM/§SEEDS/§GATES frozen before sight)
// Contract: docs/world-model/DELIVERY-CHOICE-CONTRACT.md §2 M-DLC.1″ (THE STRIKE SPACE — slice
//           ONE-S: the GROUND plane, direction × power, elevation 0, spin 0, the incumbent kick
//           as the zero-point), §1 H-DLC read at this slice's treatment cell, §4 the non-claims,
//           and the FOUR #236 amendments binding throughout.
// Rulings:  #240 (continuous aim ruled in — 传到什么位置; the gene's MAGNITUDE RETIRES; the
//           OVERSHOOT prediction pre-registered) · #241 (控制的是那一脚 — M-DLC.1″ supersedes the
//           unbuilt 1D aim segment; loft/bender dissolve into one continuous kick space) · #242
//           (DLC-T0s BANKED: the plane exists dormant, IEEE-exact at its zero-point, 19/19 gates;
//           ⚠ #242.2 THE SUBSTITUTION FACT — in percept worlds ~2/3 of the plane's choices are
//           DISCARDED by `choosePerceivedPassTarget`, delivered rate 0.298 vs 0.776 bare, so THIS
//           EXAM MUST PUBLISH DELIVERED RATE PER ARM) · #239 (DLC-T1 adjudicated: the two-point
//           contest is strictly better than the dial on every axis but leaves interceptions +4.1
//           resolved and goals 0.18 below the band — the residual this slice is about, and whose
//           CHOICE arm is re-walked here as the CONTRAST ANCHOR) · #225.3(c) (per-dose STOP
//           granularity) · #228.6 (the exam world MUST be percept-armed) · #181.2 (every HARD
//           gate in-probe) · #197-M1/#198 (hashed body vs UNHASHED envelope) · #163 (seed/stats
//           disjointness) · #20 (cluster = match seed) · #128 (wall is CONTEXT ONLY) · #207
//           (checkpoint) · #203 (PER-ARM ROWS and paired deltas ONLY — this probe fires NO
//           branch) · #226.1 (the transcript form) · #229.2 (no table typed that the artifact
//           does not carry).
//
// ⭐ INSTRUMENT-ONLY ROUND. src/** is byte-untouched (X-SRC-UNTOUCHED is a HARD gate); every
// seam this stage touches is already banked — the STRIKE PLANE at 54a45df/8333911 (#242), the
// two-point CONTEST at 9360882/b8f5ef0 (#237), the pass-lead seat at e7eb041 (#232), the eyes
// seat at 600ff04 (#228). Arms are built by THREE MatchConfig flags (`dlcStrikePlane`,
// `dlcDeliveryChoice`, `obmMovement`) plus the two gene channels (the scalar `passLeadSupport`,
// the 16-weight OBM matrix) written on ALL THREE genome views of BOTH teams (#196.3-D6) — no
// engine byte moves, `ctbSupportPlane` and `ptpPassLead` are FALSE in every arm, and NO ARM EVER
// OPENS TWO DELIVERY DOORS (under DLC-T0s §LAW the NEWEST seam yields to every banked one, so an
// armed-both arm would silently BE the banked contest wearing the plane's name).
//
// ⭐⭐ THE GENE'S MAGNITUDE HAS RETIRED UNDER THIS DOOR (#240/#241, MEASURED at DLC-T0s by
// G-VALUE: gene 0 ≡ 0.37 ≡ 1 byte for byte). `passLeadSupport` GATES PRESENCE and scales NOTHING
// — the grid's scale is the banked projection AT WEIGHT 1, i.e. the receiver-reachable set the
// engine already believes in. ⚠ SO 0 IS NOT "OFF" HERE, ABSENCE IS: this stage's arming-identity
// arm is PLANE-INERT (the flag armed, the gene ABSENT ⇒ the arming rule is evaluated on every
// on-ball decision and returns null ⇒ no grid forms ⇒ byte-identical to ABSENT), and NOT a
// gene-zero arm. Under `dlcDeliveryChoice` (the CONTRAST ANCHOR) the same gene means TASTE.
//
// ⭐⭐ THE TREATMENT AS REALLY DELIVERED IS A FIRST-CLASS NUMBER (#242.2, the OBM P1-trap lesson
// in this slice's form). The plane chooses at DECISION time; the percept world re-chooses the
// pass TARGET afterwards, and the banked led-strike guard then DISCARDS the plane's winner. So
// this exam publishes, per arm: (a) the zero-pull STRIKE-TIME delivered rate on the exam walks
// themselves (`ledPassShare`, battery grain, in the bootstrap), and (b) the DECODED four-bucket
// rate on a declared observational seed (sampled-struck · genuine zero-point · target-substituted
// · no chooser row), with a lockstep twin proving the chooser's sidecar trace perturbed nothing.
//
// ⭐ EVERY RULER QUANTITY IS INHERITED FROM DLC-T1 (and through it PTP-T1 / OBM-T1 / CTB-T1),
// each with its own G-REPRO receipt:
...
//   ⭐⭐ AND THE WHOLE INSTRUMENT IS PROVED TO BE DLC-T1's BY G-ANCHOR (= G-REPRO-DLCT1): the
//   CHOICE-ANCHOR arm RE-WALKS the committed DLC-T1 BATTERY block's first rows in DLC-T1's own
//   world under `dlcDeliveryChoice` alone, and must reproduce every published per-match field
//   EXACTLY — signature (rng stream state inside) and the DELIVERED-STRIKE columns included. One
//   receipt, three loads: the instrument is DLC-T1's; the CONTRAST anchor really is ruling #239's
//   own arm (which is what the #240 OVERSHOOT prediction is read against); and this stage's
//   `performPass` wrapper perturbs NOTHING. G-REPRO-OBMT1 / -CTBT1 / -O2T1 / -173 / -GGC ride
//   here unchanged beneath it.
//
//   DLCT1S_MODE=smoke|full    (default smoke: 12 seeds @ 12,428,000)
//   DLCT1S_RESUME=1           full mode only — restore finished (pass, seed) units (#207)
//   DLCT1S_CHECKPOINT=<path>  /tmp scratch; never committed, never read by a gate
//   DLCT1S_N=<n> / DLCT1S_SKIP_FP=1 — OVERRIDES: routed onto the EXIT-SEMANTICS GUARD BLOCK,
...
const MODE = (process.env.DLCT1S_MODE ?? 'smoke') === 'full' ? 'full' : 'smoke';
...
const DOSE_READ_SEED = 12_428_015;
...
const STRIKE_READ_SEED = 12_428_020;
const GUARD_BLOCK: readonly [number, number] = [12_428_050, 12_428_099];
const BATTERY_BASE = 12_428_100;
...
const REPRO_DLCT1_BASE = 12_426_100;
const REPRO_DLCT1_N = 8;
...
const CONSUMED: readonly { name: string; range: readonly [number, number] }[] = [
  ...
  { name: 'DLC-T1s reserved test-seed band (this stage)', range: [12_428_900, 12_428_906] },
];
...
/** §4.2 the stats stream — a SEPARATE namespace. DLC-T1's base was 105,400 ⇒ the next legal
 *  base under the #163 200-floor is 105,600; this stage takes 105,800 (the dispatch's number,
 *  a gap of 400). The list is DLC-T1's COMPLETE published ledger + 105,400 (DLC-T1's own). */
const BOOTSTRAP_SEED = 105_800;
...
const PUBLISHED_STATS_BASES = [
  ...
  104_000, 104_200, 104_400, 104_600, 104_800, 105_000, 105_200, 105_400,
];
...
/** ⭐⭐ THE DIRECT PARENT AND THE CONTRAST ANCHOR'S SOURCE: DLC-T1's committed BATTERY artifact
 *  (#239) — the run whose CHOICE arm this exam re-walks and sets the STRIKE PLANE against. */
const DLCT1_PATH = 'docs/world-model/data/dlc-t1-choice-exam.json';
...
const N_ENV = process.env.DLCT1S_N ? Math.max(1, Number.parseInt(process.env.DLCT1S_N, 10)) : null;
const SKIP_FP = process.env.DLCT1S_SKIP_FP === '1';
const OVERRIDDEN = N_ENV !== null || SKIP_FP;
const OUT_PATH = OVERRIDDEN
  ? '/tmp/dlc-t1s-guard-run.json'
  : (MODE === 'smoke'
...
/* ========================================================================== */
type ArmName = 'absent' | 'planeInert' | 'plane' | 'planeXCas' | 'choiceAnchor';
const ARMS: readonly ArmName[] = ['absent', 'planeInert', 'plane', 'planeXCas', 'choiceAnchor'];
const CONTROL_ARM: ArmName = 'absent';
/** ⭐ THE PLANE CELL — the cell the stage exists for, and the cell the JOINT primary is read at. */
const PLANE_ARM: ArmName = 'plane';
/** ⭐⭐ THE CONTRAST ANCHOR — DLC-T1's CHOICE arm re-walked here, and the second reference of
 *  the estimator: the #240 OVERSHOOT prediction is a PLANE-versus-ANCHOR statement. */
const ANCHOR_ARM: ArmName = 'choiceAnchor';
...
const DOSE: Record<ArmName, number[] | null> = {
  ...
  planeInert: null,
  plane: null,
  planeXCas: CHECK_AND_SHOW_MATRIX,
  choiceAnchor: null,
};

/* --- ⭐ THE SHARED SCALAR GENE `passLeadSupport`, and the THREE DOORS it is read through --- */
/**
 * ⭐⭐ ONE GENE, THREE DOORS, THREE MEANINGS — DECLARED PER ARM, NEVER INFERRED.
 *
 *   through `ptpPassLead` (the BANKED, RETIRED dial): a DOSE that FORCES the aim. NO ARM HERE
 *     OPENS IT — the dial is retired (#235); this stage's contrast anchor is the CONTEST.
 *   through `dlcDeliveryChoice` (the BANKED two-point CONTEST — the CHOICE-ANCHOR arm only):
 *     TASTE. It scales the PROJECTION MAGNITUDE of the ONE led candidate, which is free to lose.
 *   through `dlcStrikePlane` (⭐ THE STRIKE PLANE, this stage's treatment): ⭐⭐ THE GENE'S
 *     MAGNITUDE HAS RETIRED ENTIRELY (#240/#241, measured at DLC-T0s by G-VALUE: gene
 *     0 ≡ 0.37 ≡ 1, byte for byte). Under this door the gene GATES PRESENCE and scales NOTHING —
 *     the grid's scale is the banked projection AT WEIGHT 1, i.e. the receiver-reachable set the
 *     engine already believes in. ⚠ THEREFORE 0 IS NOT "OFF" HERE: what disables the plane is
 *     ABSENCE and only absence (DLC-T0s G-BORN), which is why the arming-identity arm of THIS
 *     stage is PLANE-INERT (flag armed, gene ABSENT) and NOT a gene-zero arm.
 *
 * The domain is the seam's own (`clamp01`, PTP-T0 §LAW 2) and is PROBED off the shipped
 * `passLeadSupportWeight` map by G-TRACE-PTP at absence, both ends and beyond both ends.
 */
const PTP_GENE_MIN = 0;
const PTP_GENE_MAX = 1;
const PTP_GENE_HALF = (PTP_GENE_MIN + PTP_GENE_MAX) / 2;
/** `null` = the gene ABSENT on this arm. ⚠ PLANE-INERT carries `null` WITH ITS DOOR OPEN — that
 *  is the whole point of the arm (the ARMING IDENTITY at exam grain). */
const GENE: Record<ArmName, number | null> = {
  absent: null,
  planeInert: null,
  plane: PTP_GENE_MAX,
  planeXCas: PTP_GENE_MAX,
  choiceAnchor: PTP_GENE_MAX,
};
/** ⭐⭐ WHICH DOOR EACH ARM OPENS — and the promise that NO ARM OPENS TWO, in code rather than
 *  in prose. Asserted off the REAL constructed matches by FLAG-HYGIENE's doors row.
 *  `sp` = `dlcStrikePlane` (this stage's seam) · `dlc` = the banked two-point contest ·
 *  `ptp` = the retired dial (unused here, kept so the exclusivity check stays total). */
...
/** the four-seam sentence for each arm, frozen with the arm (stage doc §FORM's table). */
const ARM_SENTENCE: Record<ArmName, string> = {
  absent: 'the CONTROL — the percept-armed world with EVERY seat unreached. DLC-T1\'s own '
    + 'ABSENT arm, world for world.',
  planeInert: '⭐ THE ARMING-IDENTITY ARM, AT EXAM GRAIN: the strike-plane door OPEN with the '
    + 'gene ABSENT. Under the DLC-T0s law the arming rule is EVALUATED on every on-ball decision '
    + 'and returns `null`, so NO grid forms, no candidate is priced and the pass loop runs the '
    + 'shipped statements alone ⇒ it must be BYTE-IDENTICAL to ABSENT per seed. ⚠ IT IS NOT A '
    + 'GENE-ZERO ARM AND COULD NOT BE: the gene\'s magnitude retired at #240/#241, so a gene-zero '
    + 'plane is a LIVE plane (DLC-T0s G-VALUE). What this arm buys is the sentence THE PLANE '
    + 'COSTS THE WORLD NOTHING UNTIL IT IS GIVEN A GENE — measured here, not cited from T0s.',
  plane: '⭐⭐ THE CELL THE STAGE EXISTS FOR (M-DLC.1″ slice one-s, 控制的是那一脚): per support '
    + 'mate the chooser prices a 3×3 GROUND STRIKE PLANE — direction (±θ, θ = atan2(reach, d0) '
    + 'DERIVED) × power (d0 ± reach) — with the ONE hoisted pricer at each member\'s OWN '
    + 'receiving point, and the ARGMAX picks THE KICK. The zero-point (i=0, j=0) IS today\'s kick '
    + 'EXACTLY (±0 IEEE) and wins every tie. No threshold, no predicate, no taste multiplier. '
    + 'The pre-registered JOINT primary and the #240 OVERSHOOT contrast are read here.',
  planeXCas: '⭐ THE RELATIONAL PAIR WITH THE STRIKE FREE: the receiver comes short and asks '
    + '(the OBM CHECK-AND-SHOW matrix, OBM-T1\'s own, re-walked as a receipt) AND the passer '
    + 'chooses his kick. DLC-T1\'s COMBINED cell with the two-point contest replaced by the '
    + 'PLANE — the OBM matrix beside the treatment.',
  choiceAnchor: '⭐⭐ THE CONTRAST ANCHOR: DLC-T1\'s CHOICE arm — the BANKED TWO-POINT CONTEST '
    + '(`dlcDeliveryChoice` armed alone at gene 1, `dlcStrikePlane` OFF) — RE-WALKED on this '
    + 'stage\'s own seeds. It is ruling #239\'s own arm: the contest that beat the dial on every '
    + 'axis and still left interceptions +4.1 resolved with goals 0.18 below the band. ⭐ THE '
    + '#240 OVERSHOOT PREDICTION IS READ AGAINST IT, and G-ANCHOR proves it is that arm '
    + 'field-for-field against DLC-T1\'s committed battery rows rather than a look-alike.',
};
...
type ReproArm = 'reproO2Control' | 'repro173Prod' | 'reproGgcProd' | 'reproCtbT1Absent'
  | 'reproObmT1Absent' | 'reproObmT1CheckAndShow' | 'reproDlcT1Choice';
...
const matchOf = (seed: number, arm: WalkArm): Match => {
  ...
  /** ⭐⭐ THE G-ANCHOR RE-WALK runs in DLC-T1's OWN world on ITS CHOICE ARM — percept-armed,
   *  `dlcDeliveryChoice` armed, the gene at the domain max, NO OBM matrix, and `ptpPassLead` /
   *  `dlcStrikePlane` NEVER passed (DLC-T1 predates the plane entirely). It is
   *  CONFIGURATION-IDENTICAL to this exam's CHOICE-ANCHOR arm, which is exactly what makes the
   *  CONTRAST anchor honest — and it is the arm the #240 OVERSHOOT prediction is read against. */
  if (arm === 'reproDlcT1Choice') {
    const mm = new Match({
      ...base, ...PERCEPT_FLAGS, dlcDeliveryChoice: true,
    } as ConstructorParameters<typeof Match>[0]);
...
const DLCT1 = readJson(DLCT1_PATH);
...
const SELF_SMOKE_PATH = 'docs/world-model/data/dlc-t1s-strike-exam-smoke.json';
...
const nRule = (() => {
  ...
  const smoke = SELF_SMOKE === null ? null : (() => {
    ...
    const d2 = rate2.planeXCas.pairedDelta;
    const seBoot2 = ((d2.upper - d2.lower) / 2) / Z975;
    const pK = rate2.planeXCas.point as number;
    ...
    return {
      ...
      deffProvenance: 'the MOST-PERTURBED arm (planeXCas — the relational pair with the '
...
if (MODE === 'full' && RUN_N <= 0) {
  console.error(`DLC-T1s FATAL — full mode needs the committed artifacts for the N rule (${O2T1_PATH}).`);
  process.exit(2);
}

banner('');
banner('=============================================================================');
banner(`DLC-T1s STRIKE-PLANE EXAM (#242, the self-drive) · mode ${MODE} · N ${RUN_N} seeds × ${ARMS.length} arms`);
banner(`seeds ${RUN_BASE}..${RUN_BASE + RUN_N - 1} · world = PERCEPT-ARMED (edsPerceivedChoice)`);
banner('arms differ by EXACTLY the delivery DOOR (sp | dlc | none) + the OBM matrix');
banner(`N rule ⇒ N* ${String(nRule.nStar)} (ledger room ${BATTERY_ROOM}, cap ${N_CAP})`);
if (OVERRIDDEN) {
  banner('⚠ OVERRIDE IN FORCE (DLCT1S_N / DLCT1S_SKIP_FP) — routed onto the EXIT-SEMANTICS GUARD');
...
/* ========================================================================== */
const CKPT_PATH = process.env.DLCT1S_CHECKPOINT ?? '/tmp/dlc-t1s-checkpoint.jsonl';
const RESUME = process.env.DLCT1S_RESUME === '1';
const CHECKPOINTING = MODE === 'full';
const PROBE_SELF_PATH = 'scripts/probes/dlc-t1s-strike-exam.ts';
...
const computeCore = (pass: number): Core => {
  ...
  banner(`  pass ${pass} · ⭐⭐ G-ANCHOR (G-REPRO-DLCT1): block ${REPRO_DLCT1_BASE} `
    + `(${REPRO_DLCT1_N} matches on DLC-T1's OWN CHOICE arm)`);
  const reproDlcT1Choice: PerMatch[] = [];
  for (let i = 0; i < REPRO_DLCT1_N; i++) {
    reproDlcT1Choice.push(walkSeed(REPRO_DLCT1_BASE + i, 'reproDlcT1Choice'));
  }

  return {
    byArm, reproO2, repro173, reproGgc, reproCtbT1, reproObmT1Absent, reproObmT1CheckAndShow,
    reproDlcT1Choice,
    dose, strike, restored, computed,
  };
};
...
const coreBody = (core: Core) => {
   ...
   *  re-walks the first `REPRO_DLCT1_N` rows of its CHOICE arm — `dlcDeliveryChoice` armed alone
  ...
  const dlcT1RowOf = (r: PerMatch): Record<string, unknown> => ({
    seed: r.seed, eligible: r.eligible, trueHoldable: r.trueHoldable,
    firstRecOpen: r.firstRecOpen, firstRecOpenPressed: r.firstRecOpenPressed,
    possTicks: r.possTicks, possTicksShort: r.possTicksShort,
    possTicksPressed: r.possTicksPressed, possTicksPressedShort: r.possTicksPressedShort,
  ...
  const dlcT1Committed = DLCT1 === null ? [] : ((DLCT1.j.perMatch.choice ?? []) as any[]);
  const dlcT1Rows = core.reproDlcT1Choice.map((r, i) => {
    const want = dlcT1Committed[i];
    const got = dlcT1RowOf(r);
    const differing = want === undefined ? [...DLCT1_FIELDS]
      : DLCT1_FIELDS.filter((k) => JSON.stringify(got[k]) !== JSON.stringify(want[k]));
    return { seed: r.seed, differingFields: differing, got };
  });
  const dlcT1Mismatches = dlcT1Rows.filter((x) => x.differingFields.length > 0);
  ...
  return {
    ...
    gAnchor: {
      block: `${REPRO_DLCT1_BASE}..${REPRO_DLCT1_BASE + REPRO_DLCT1_N - 1}`,
      source: DLCT1_PATH,
      sourceArm: 'choice',
      sourceResultSha: DLCT1 === null ? null : DLCT1.j.resultSha256,
      ...
      fieldsPerRow: DLCT1_FIELDS.length,
      rowsChecked: dlcT1Rows.length,
      committedRowsAvailable: dlcT1Committed.length,
      mismatches: dlcT1Mismatches.length,
      mismatchRows: dlcT1Mismatches,
      identical: dlcT1Rows.length > 0 && dlcT1Mismatches.length === 0
        && dlcT1Committed.length >= dlcT1Rows.length,
...
const fpObserved = SKIP_FP ? 'skipped (DLCT1S_SKIP_FP)' : leagueHash(FINGERPRINT_SEED);
...
const walkedBlocksRaw: { name: string; first: number; last: number; kind: BlockKind }[] = [
  ...
  {
    name: '⭐⭐ reproDlcT1Choice (re-walk — G-ANCHOR: DLC-T1\'s BATTERY block on its CHOICE arm)',
    first: REPRO_DLCT1_BASE, last: REPRO_DLCT1_BASE + REPRO_DLCT1_N - 1, kind: 're-walk',
  },
];
...
const twoDoors = {
  ...
  /** ⭐⭐ THE RETIRED DIAL IS NOWHERE IN THIS EXAM: no arm opens `ptpPassLead` at all (#235). */
  ...
  /** ⭐⭐ THE ARMING RULE'S TWO LIMBS, AND THE ONE DECLARED EXCEPTION THAT IS THIS STAGE'S
   *  IDENTITY ARM. A gene is written IF AND ONLY IF a door is open — and EXACTLY ONE arm
   *  (PLANE-INERT) opens a door with the gene ABSENT, which is precisely the arming identity
   *  being measured (flag armed ⇒ the rule is evaluated on every on-ball decision and returns
   *  `null`). Stated as a predicate rather than left as an inference. */
  doorMatchesGenePresence: ARMS.every((a) => (GENE[a] === null
    ? (DOOR[a] === 'none' || a === 'planeInert')
    : DOOR[a] !== 'none')),
  exactlyOneArmedInertArm: ARMS.filter((a) => DOOR[a] !== 'none' && GENE[a] === null).length === 1
    && DOOR.planeInert === 'sp' && GENE.planeInert === null,
...
const armConfigEcho = Object.fromEntries(ARMS.map((a) => [a, {
  ...
  geneSemanticsUnderThisDoor: DOOR[a] === 'ptp'
    ? 'DOSE — the gene FORCES the aim (the banked, retired dial; NO ARM HERE OPENS IT)'
    : DOOR[a] === 'dlc'
      ? 'TASTE — the gene scales the PROJECTION MAGNITUDE of the ONE led candidate, which is '
        + 'free to lose (the banked two-point contest, this stage\'s CONTRAST ANCHOR)'
      : DOOR[a] === 'sp'
        ? '⭐⭐ PRESENCE-GATE ONLY — the magnitude RETIRED at #240/#241 and was MEASURED retired '
          + 'at DLC-T0s (G-VALUE: 0 ≡ 0.37 ≡ 1 byte for byte). The grid\'s scale is the banked '
          + 'projection AT WEIGHT 1. ⚠ 0 is NOT "off" under this door; ABSENCE is.'
...
const identityRows = coreA.byArm[CONTROL_ARM].map((r, i) => {
  const z = coreA.byArm.planeInert[i];
...
const flagHygiene = {
  pass: identityRows.every((x) => x.signatureIdentical && x.rowIdentical)
    /** the CONTROL has NO seam at all; every other arm opens exactly one delivery door. */
    && DOSE[CONTROL_ARM] === null && GENE[CONTROL_ARM] === null && DOOR[CONTROL_ARM] === 'none'
    && ARMS.filter((a) => a !== CONTROL_ARM).every((a) => DOOR[a] !== 'none')
    /** ⭐ THE ARMING-IDENTITY ARM: the PLANE door open, the gene ABSENT, no OBM matrix. */
    && DOSE.planeInert === null && GENE.planeInert === null && DOOR.planeInert === 'sp'
    /** every OTHER non-control arm carries the gene at the domain MAX behind its own door. */
    && ARMS.filter((a) => a !== CONTROL_ARM && a !== 'planeInert')
    ...
    && DOSE.planeXCas === CHECK_AND_SHOW_MATRIX
    && ARMS.filter((a) => a !== 'planeXCas').every((a) => DOSE[a] === null)
    /** ⭐⭐ the CONTRAST ANCHOR is the ONLY arm on the banked two-point CONTEST door, and the
     *  three PLANE arms are the only ones on this stage's own. */
    && DOOR.choiceAnchor === 'dlc'
    && ARMS.filter((a) => a !== 'choiceAnchor').every((a) => DOOR[a] !== 'dlc')
    && ARMS.filter((a) => DOOR[a] === 'sp').length === 3
    && doseWellFormed
  ...
  identityExcludedWhy: '⭐ THE EXCLUDED FIELDS ARE THE ARM DEFINITION ITSELF or its code-path '
    + 'signature — whether the OBM matrix and the lead gene sit on the genome views, how many '
    + 'entries the policy cache holds, and the three PLANE-PRESENCE classes (present / absent / '
    + 'zero) which exist only in the OBM-armed arms. ⚠ AT THIS STAGE THE IDENTITY IS STRICTLY '
    + 'STRONGER THAN DLC-T1\'s WAS ON ITS OWN GENE COLUMN: PLANE-INERT carries NO gene at all, so '
    + '`leadGeneOnAllViews` is 0 in BOTH compared arms and its exclusion changes nothing here. '
    + 'EVERYTHING the world produced — every ruler, every guard, every geometric quantity, the '
    + 'metre-shift sums, the DELIVERED-STRIKE columns (`ptpLedHandled` / `ptpLedNonZero` / '
    + '`ptpLeadSum`) and the whole-match SIGNATURE including the rng stream state — is compared. '
    + 'Excluded and stated, never quietly dropped.',
  note: '⭐ THE ARMS DIFFER BY EXACTLY ONE DELIVERY DOOR (+ the OBM matrix on the one combined '
    + 'cell): `dlcStrikePlane` on the three PLANE arms, `dlcDeliveryChoice` on the CONTRAST '
    + 'ANCHOR, nothing on the control — every gene written on all three genome views of both '
    + 'teams. Everything else (percept-armed world, seeds, teams, duration, `ctbSupportPlane` '
    + 'false throughout, `ptpPassLead` false throughout) is identical by construction. '
    + '⭐⭐ PLANE-INERT ≡ ABSENT is proved PER SEED on the whole-match signature INCLUDING the '
    + 'rng stream state AND on every measured row field: the plane\'s door OPEN, its arming rule '
    + 'EVALUATED on every on-ball decision, and the world untouched — the T0s born-absent '
    + 'identity re-measured at exam grain instead of cited.',
};
 ...
 *
 *   contract §1 H-DLC read at THIS slice's treatment cell: at the PLANE arm the TRUE-holdable
 *   gain is retained (resolved helpful) AND goals stay inside the frozen band —
 *   PLUS the #240 OVERSHOOT clause against the CHOICE ANCHOR (see `overshootPrediction`).
 *
 ...
 * The claim at this slice is that choosing THE KICK buys the first WITHOUT spending the second.
 *
 * ⭐ AND THE CONTRAST ANCHOR IS PART OF THE READING, not decoration: the same two limbs are
 * computed at CHOICE-ANCHOR, which G-ANCHOR proves IS ruling #239's own arm. The rows are
  ...
  return {
    ...
    whichLimbFails: supply.resolvedHelpful && band.inBand ? null
      : !supply.resolvedHelpful && !band.inBand ? 'BOTH'
        : supply.resolvedHelpful ? 'the GOALS BAND (the F-T1s-a shape: the poison retained)'
          : 'the SUPPLY GAIN (the F-T1s-b shape: the gain killed)',
  };
};
/**
 * ⭐⭐ THE #240 OVERSHOOT PREDICTION, FROZEN MECHANICALLY EX ANTE (ruling #240.2, carried into
 * this slice by #241/#242.3 as the CONTRAST clause of the primary).
 *
 * The ruling's words: *"a led ball that wins the two-way contest is forced to the FULL projection
 * magnitude; intermediate aims may retain the gain at lower cost … supply retained + INTERCEPTIONS
 * FALL + goals RECOVER toward the band, vs the two-point contest as CONTRAST ANCHOR."* The finer
 * grid should pick GENTLER strikes.
 *
 * The exact predicate, frozen here before any receipt ran, and computed against the CHOICE ANCHOR
 * with the anchor-referenced paired bootstrap (same clusters, same draws — never two
 * control-referenced CIs subtracted):
 *
 *   LIMB I  (interceptions FALL)  point(plane) < point(anchor) on `interceptionsPerMatch`.
 *                                 STRICT form: the anchor-referenced paired CI is entirely BELOW
 *                                 zero (`resolvedFall`).
 *   LIMB G  (goals RECOVER)       `bandDistance` STRICTLY DECREASES, where bandDistance is 0
 *                                 inside the frozen band and otherwise the distance to the NEARER
 *                                 edge. STRICT form: the plane is INSIDE the band where the
 *                                 anchor is not (`intoBand`).
 *   SATISFIED = LIMB I OR LIMB G  — the ruling's own disjunction ("interceptions FALL or goals
 *                                 RECOVER toward the band"), with both limbs published separately
 *                                 and their strict forms beside them. It is a CONTRAST clause of
 *                                 the primary, not a replacement for it: the JOINT limbs at PLANE
 *                                 are read first and this is read beside them.
 *
 * ⚠ MECHANICAL FLAGS ONLY (#203). The probe fires nothing.
 */
...
const overshootPrediction = (() => {
  const inter = anchorDeltaOf(PLANE_ARM, 'interceptionsPerMatch');
  const supply = anchorDeltaOf(PLANE_ARM, 'trueHoldableShare');
  const dPlane = bandDistanceOf(PLANE_ARM);
  const dAnchor = bandDistanceOf(ANCHOR_ARM);
  const interPointFall = inter.pairedDelta !== null && inter.pairedDelta.point < 0;
  const interResolvedFall = inter.pairedDelta !== null && inter.pairedDelta.upper < 0;
  const goalsRecover = dPlane < dAnchor;
  const intoBand = goalsBandOf(PLANE_ARM).inBand && !goalsBandOf(ANCHOR_ARM).inBand;
  return {
    frozenText: 'ruling #240.2, VERBATIM: "the residual poison … may be OVERSHOOT: a led ball '
      + 'that wins the two-way contest is forced to the FULL projection magnitude; intermediate '
      + 'aims may retain the gain at lower cost. Frozen as the T1c pre-registered prediction '
      + '(supply retained + interceptions fall + goals recover toward the band, vs the two-point '
      + 'contest as CONTRAST ANCHOR)." Carried into slice one-s by #241 (the 1D segment is '
      + 'SUBSUMED by the strike plane) and named as this exam\'s contrast clause by #242.3.',
    reference: ANCHOR_ARM,
    estimator: 'the ANCHOR-REFERENCED paired bootstrap (`contrasts.ratesVsAnchor`): the same '
      + 'resampled seed-index sets, differenced PLANE − ANCHOR. Never two control-referenced CIs '
      + 'subtracted.',
    limbInterceptions: {
      quantity: 'interceptionsPerMatch',
      planeMinusAnchor: inter.pairedDelta,
      fall: interPointFall,
      resolvedFall: interResolvedFall,
    },
    limbGoalsBand: {
      quantity: 'the frozen equilibrium band on goals',
      bandDistancePlane: dPlane,
      bandDistanceAnchor: dAnchor,
      recover: goalsRecover,
      intoBand,
      bandDistanceRule: '0 inside the frozen band; otherwise the distance to the NEARER edge',
    },
    supplyRetainedVsAnchor: {
      quantity: 'trueHoldableShare',
      planeMinusAnchor: supply.pairedDelta,
      resolvedAgainstAnchor: supply.resolved,
      note: 'RETENTION is the JOINT primary\'s limb A against the CONTROL; this row is the '
        + 'plane-versus-anchor difference, published so "retained at lower cost" can be read as '
        + 'one sentence rather than assembled after sight.',
    },
    satisfied: interPointFall || goalsRecover,
    satisfiedStrict: interResolvedFall || intoBand,
    status: '⚠ MECHANICAL PREDICATE FLAGS (#203). At SMOKE GRAIN none of these rows is evidence: '
      + 'the band gates at battery N only and the CIs at 12 clusters are wider than anything they '
      + 'could detect. Published now so the predicate cannot be re-cut after sight.',
  };
})();

const preRegisteredPrimary = {
  frozenText: '⭐ THE JOINT FORM, contract §1 H-DLC read at THIS slice\'s treatment cell: at the '
    + 'PLANE arm the TRUE-holdable gain is retained (resolved helpful) AND goals stay inside the '
    + 'frozen band — BOTH, JOINTLY. ⭐⭐ PLUS THE #240 OVERSHOOT PREDICTION AS THE CONTRAST '
    + 'CLAUSE: against the CHOICE ANCHOR (the banked two-point contest), interceptions FALL or '
    + 'goals RECOVER toward the band — the finer grid should pick GENTLER strikes. Both are '
    + 'frozen in this probe\'s own predicates before any receipt ran.',
  successRule: '⭐ JOINT: at the PLANE arm, `trueHoldableShare` RESOLVED HELPFUL (paired-delta CI '
    + 'excludes zero in the UP direction) AND the `goals` equilibrium-band dimension INSIDE the '
    + 'frozen band (2.3944 ± 15 %), BOTH TOGETHER. Neither limb alone is the prediction: the '
    + 'retired dial already had the gain and died on the band, the banked two-point contest kept '
    + 'the gain unresolved-vs-control and stayed 0.18 below the band, and the band alone is what '
    + 'ABSENT has for free.',
  planeCell: PLANE_ARM,
  primaryAtPlane: jointAt(PLANE_ARM),
  ...
  contrastAnchor: {
    arm: ANCHOR_ARM,
    what: '⭐⭐ RULING #239\'s OWN ARM, proved by G-ANCHOR against DLC-T1\'s committed battery '
      + 'rows. At battery N its published reading was the two-point contest STRICTLY BETTER than '
      + 'the dial on every measured axis, with interceptions +4.1 RESOLVED against the control '
      + 'and goals 0.18 BELOW the band (in a world whose own control sits 0.005 above the '
      + 'floor). That is the residual the #240 overshoot decode is about, and it is RE-WALKED '
      + 'here on this stage\'s own seeds rather than quoted at.',
    reading: jointAt(ANCHOR_ARM),
  },
  failBranches: '⭐ PRE-NAMED FOR THIS SLICE: F-T1s-a — the PLANE RETAINS the poison (goals out '
    + 'of band at PLANE; the pricing itself misprices strikes and the evaluation surgery '
    + 'reopens). F-T1s-b — the PLANE KILLS the gain (supply null at PLANE; the contest\'s gain '
    + 'was the two-point geometry\'s artifact). F-T1s-c — a guard STOPs. All three fire PER DOSE '
    + '(the standing #225.3(c) granularity). ⚠ THIS PROBE FIRES NONE OF THEM (#203).',
  secondaryReported: '⭐ REPORTED HEADLINE, NOT A PRIMARY: (a) the CHOSEN-STRIKE DISTRIBUTION '
    + 'over the NINE grid members per arm (by member / direction / power — the emergent KICK); '
    + '(b) ⭐⭐ the DELIVERED RATE PER ARM in both forms — the zero-pull strike-time share on the '
    + 'exam walks themselves (`ledPassShare`, at battery grain) and the DECODED four-bucket rate '
    + 'on the declared observational seed (#242.2: in percept worlds ~2/3 of the plane\'s choices '
    + 'are DISCARDED by the target substitution, so the treatment as REALLY DELIVERED is ~30 %); '
    + '(c) the EMERGENT LED SHARE with its situational profile. Discoveries, never set points; no '
  ...
  status: '⚠ MECHANICAL PREDICATE FLAGS ON PUBLISHED CIs AND ON THE FROZEN BAND, exactly like '
    ...
    + 'evidence about anything, the band gates at battery N only, and F-T1s-a/b/c are the '
    + 'commander\'s.',
};
...
const gates = {
  ...
  gAnchor: {
    ...
    block: bodyA.gAnchor.block, source: DLCT1_PATH, sourceArm: bodyA.gAnchor.sourceArm,
    ...
    sourceSha256: DLCT1 === null ? null : sha(DLCT1.bytes.toString('utf8')),
    armConfigurationIdentical: DOOR.choiceAnchor === 'dlc' && GENE.choiceAnchor === PTP_GENE_MAX
      && DOSE.choiceAnchor === null,
  ...
  gCleanInvocation: {
    ...
    note: 'any DLCT1S_N / DLCT1S_SKIP_FP override is BY DEFINITION not the exam: the run is routed '
...
/* ========================================================================== */
const body = {
  stage: 'DLC T1s — THE STRIKE-PLANE EXAM (控制的是那一脚: does making THE KICK ITSELF a priced '
    + 'choice keep the contest\'s gain and shed its residual cost?)',
  ruling: '#240/#241/#242 (continuous aim ruled in · 控制的是那一脚: M-DLC.1″ the STRIKE SPACE · '
    + 'DLC-T0s banked, the plane IEEE-exact at its zero-point and the SUBSTITUTION confound '
    + 'caught), under the live self-drive · DELIVERY-CHOICE-CONTRACT §2 M-DLC.1″ slice ONE-S '
    + '(+ §1 H-DLC read at this cell, the four #236 amendments binding) · #239 (DLC-T1 '
    + 'adjudicated: the two-point contest beats the dial on every axis but leaves interceptions '
    + '+4.1 resolved and goals 0.18 below the band — the residual this slice is about, and whose '
    + 'CHOICE arm is re-walked here as the CONTRAST ANCHOR) · #242.2 (the DELIVERED-RATE fact of '
    + 'record: in percept worlds ~2/3 of the plane\'s choices are discarded upstream) · '
    + '#225.3(c) (per-dose STOP granularity) · #181.2 (the standing receipt rule). SUPERSEDED '
    + 'HEADER KEPT FOR THE CHAIN: '
    + '#235/#236/#237 (the dial retired · the contract drafted and re-audited · DLC-T0 '
  ...
  doc: 'docs/world-model/DLC-T1S-STRIKE-EXAM.md',
  mode: MODE,
  ...
  world: '⭐ ONE PERCEPT-ARMED BASE WORLD, IDENTICAL IN EVERY ARM: `new Match({seed, teamA, '
    ...
    + 'ANCHOR re-walks DLC-T1\'s CHOICE arm, which lives in exactly this world, so any other '
    + 'world would forfeit G-ANCHOR. The THIN channel that follows from it is therefore the HONEST '
  ...
  armDefinitions: Object.fromEntries(ARMS.map((a) => [a, [
    ...
    DOOR[a] === 'sp' ? `dlcStrikePlane:true (⭐ THE GROUND STRIKE PLANE, K = ${STRIKE_PLANE_K}) · `
      + `dlcDeliveryChoice:false · ptpPassLead:false · passLeadSupport ${GENE[a]} = PRESENCE-GATE `
      + 'ONLY (the magnitude retired at #240/#241)'
      : DOOR[a] === 'dlc' ? `dlcDeliveryChoice:true (THE BANKED TWO-POINT CONTEST — THE CONTRAST `
        + `ANCHOR) · dlcStrikePlane:false · ptpPassLead:false · passLeadSupport ${GENE[a]} = TASTE`
        : DOOR[a] === 'ptp' ? `ptpPassLead:true (THE RETIRED DIAL — unused here) · `
  ...
  doseProvenance: 'RECEIVER SIDE (the ONE OBM-dosed arm): every non-zero weight is a DOMAIN '
    ...
    + 'PASSER SIDE: ⭐⭐ THERE IS NO DOSE TO PROVENANCE — under the plane door the gene\'s '
    + 'MAGNITUDE RETIRED (#240/#241) and was MEASURED retired at DLC-T0s (G-VALUE: 0 ≡ 0.37 ≡ 1 '
    + 'byte for byte); it gates PRESENCE and the grid\'s scale is the banked projection AT WEIGHT '
    + '1, i.e. the receiver-reachable set the engine already believes in. The domain map is still '
    + 'PROBED through the shipped `passLeadSupportWeight` by G-TRACE-PTP at absence, both ends '
    + 'and beyond both ends, because the CONTRAST ANCHOR still reads the gene as TASTE. ⭐⭐ AND ITS MEANING IS DECLARED PER DOOR, because the SAME NUMBER MEANS TWO '
  ...
  twoDoorsDeclaration: '⭐⭐ FOUR DOORS, AND THE THREE DELIVERY DOORS ARE MUTUALLY EXCLUSIVE '
    + 'BY CONSTRUCTION — asserted per arm off the REAL constructed matches, never merely stated. '
    + '⭐ `ptpPassLead` is FALSE IN EVERY ARM (the dial is retired, #235). No arm opens more than '
    + 'ONE of {`dlcStrikePlane`, `dlcDeliveryChoice`, `ptpPassLead`}, and that is load-bearing '
    + 'rather than tidy: under DLC-T0s §LAW the NEWEST seam YIELDS to every banked one '
    + '(`o1PassWindup > ptpPassLead > dlcDeliveryChoice > dlcStrikePlane`, guarded on SEATS not '
    + 'flags), so an armed-both arm would silently BE the banked contest wearing the plane\'s '
    + 'name and the whole PLANE-versus-CONTEST contrast would be a comparison of an arm with '
    + 'itself. THE INHERITED SENTENCE, KEPT: `ctbSupportPlane` is FALSE in EVERY arm (asserted per arm in '
  ...
  thinChannelDeclaration: '⭐⭐ TWO LIMITS ON DELIVERY, BOTH DECLARED AT THE HEAD RATHER THAN '
    + 'DISCOVERED AT THE FOOT. (1) THE HONEST (THIN) CHANNEL, inherited (#232.3): in a percept '
    + 'world a carrier can only bend the plane around a mate whose motion his OWN EYES carry — '
    + 'DLC-T0s measured 648 of 836 support samples with NO remembered motion here — and for such '
    + 'a mate every one of the nine members collapses onto today\'s kick BY ARITHMETIC (#200, no '
    + 'branch). That is a footballing outcome, not an undelivered treatment. (2) ⭐⭐ THE '
    + 'SUBSTITUTION (#242.2, THE FACT OF RECORD): the plane chooses at DECISION time, the percept '
    + 'world then RE-CHOOSES the pass TARGET, and the banked led-strike guard DISCARDS the '
    + 'plane\'s winner whenever the man changed — DLC-T0s measured delivered rate 0.298 percept '
    + 'against 0.776 bare, substitution rate 0.560. ⇒ THIS EXAM PUBLISHES DELIVERED RATE PER ARM '
    + 'AS A FIRST-CLASS NUMBER, in both forms (the zero-pull strike-time share at battery grain, '
    + 'and the decoded four-bucket rate on its own declared seed), budgets its reading against '
    + 'the DELIVERED n, and NEVER reads a decision-time rate as a strike-time one. Whether the '
    + 'substitution guard\'s interaction with the plane is itself a contract question rides '
    + 'these numbers and is the commander\'s.',
  preRegisteredSuccess: '⭐ THE JOINT FORM, at the PLANE arm: `trueHoldableShare` RESOLVED '
    + 'HELPFUL **AND** the `goals` equilibrium-band dimension INSIDE the frozen band, BOTH '
    + 'TOGETHER (contract §1 H-DLC read at this slice\'s treatment cell). ⭐⭐ PLUS THE #240 '
    + 'OVERSHOOT PREDICTION AS THE CONTRAST CLAUSE, mechanically exact ex ante: against the '
    + 'CHOICE ANCHOR, interceptions FALL (point) or goals RECOVER toward the band (band distance '
    + 'strictly decreases), with the strict forms (`resolvedFall`, `intoBand`) and the '
    + 'plane-minus-anchor supply delta published beside them — see `preRegisteredPrimary. '
    + 'overshootPrediction`. FAIL branches pre-named for this slice: F-T1s-a (the PLANE retains '
    + 'the poison — goals out of band at PLANE) · F-T1s-b (the PLANE kills the gain — supply null '
    + 'at PLANE) · F-T1s-c (a guard STOPs). ⚠ THIS PROBE FIRES NONE OF THEM: it emits PER-ARM '
    + 'ROWS and paired deltas with mechanical `resolved` CI flags only (#203); adjudication is '
    + 'the commander\'s.',
  ...
  primaryRulers: '⭐ JOINT, FROZEN EX ANTE (see `preRegisteredPrimary`): ruler 1 '
    ...
    + 'BAND, both at the PLANE cell, both together — plus the #240 OVERSHOOT contrast clause '
    + 'against the CHOICE ANCHOR. ⭐ REPORTED HEADLINES beside it: the CHOSEN-STRIKE DISTRIBUTION '
    + 'over the nine grid members, the DELIVERED RATE per arm (both forms), and the emergent led '
    + 'share with its situational profile. '
...
o('  at PLANE: trueHoldableShare RESOLVED HELPFUL *AND* goals IN BAND — JOINTLY');
for (const a of ARMS.filter((x) => x !== CONTROL_ARM)) {
  const j = a === PLANE_ARM ? preRegisteredPrimary.primaryAtPlane
...
o('  ⭐⭐ THE #240 OVERSHOOT CONTRAST (PLANE vs the CHOICE ANCHOR, anchor-referenced paired CIs)');
{
  const ov = overshootPrediction;
  const li = ov.limbInterceptions;
  o(`    interceptions/match Δ(plane−anchor) ${li.planeMinusAnchor === null ? 'n/a'
    : `${li.planeMinusAnchor.point} [${li.planeMinusAnchor.lower}, ${li.planeMinusAnchor.upper}]`}`
    + ` · FALL=${li.fall} · resolvedFall=${li.resolvedFall}`);
  o(`    band distance plane ${ov.limbGoalsBand.bandDistancePlane} vs anchor `
    + `${ov.limbGoalsBand.bandDistanceAnchor} · RECOVER=${ov.limbGoalsBand.recover}`
    + ` · intoBand=${ov.limbGoalsBand.intoBand}`);
  const sv = ov.supplyRetainedVsAnchor.planeMinusAnchor;
  o(`    trueHoldableShare Δ(plane−anchor) ${sv === null ? 'n/a' : `${sv.point} [${sv.lower}, ${sv.upper}]`}`
    + ` · resolvedVsAnchor=${ov.supplyRetainedVsAnchor.resolvedAgainstAnchor}`);
  o(`    ⇒ SATISFIED=${ov.satisfied} (strict form ${ov.satisfiedStrict}) — a MECHANICAL flag (#203)`);
}
o('');
