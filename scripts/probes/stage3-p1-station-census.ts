...
// Authority: docs/world-model/STAGE3-P1-STATION-CENSUS.md (ruling #39 +
// the commander's 2026-07-27 review: 4.5-compliant, approved to run).
//
...
/** §2.4, derived from P0's anchors and the travel time to the mid ring. */
const W_S = 2.0;
...
const runBlock = (
  ...
  return { rows, moments, clonesTaken, x5Checked, x5Mismatched, x6 };
};
...
const runExperiment = () => {
  ...
  return {
    experiment: 'STAGE3-P1 (station census)',
    authority: 'STAGE3-P1-STATION-CENSUS',
    parameters: {
    ...
    coverage: {
      moments, clonesTaken, cloneCoverage: moments === 0 ? Number.NaN : round(clonesTaken / moments),
...
writeFileSync('docs/world-model/data/stage3-p1-station-table.json',
