...
// Authority: docs/world-model/GAZE-SUPPORTED-MOTION-EVIDENCE.md
...
const SEED_START = Number(process.argv[3] ?? 89_000);
const MAX_SEEDS = 192;
...
const runArm = (state: FrozenState, arm: Arm): ArmResult => {
  ...
  for (let index = 0; index < gazeLog.length; index++) {
    ...
    if (JSON.stringify(replayGaze) !== JSON.stringify(gazeLog[index])) {
      diagnostics.recomputeMismatches++;
    }
...
const runExperiment = () => {
  ...
  for (
    ...
    while (!match.finished && !accepted) {
      ...
      for (const arm of ['H', 'E', 'R'] as const) {
        ...
        gazeInvalid += result.diagnostics.gazeInvalid;
        nonNormalisedGaze += result.diagnostics.nonNormalisedGaze;
        recomputeMismatches += result.diagnostics.recomputeMismatches;
  ...
  return {
    experiment: 'D-PROC-1MG',
    authority: 'GAZE-SUPPORTED-MOTION-EVIDENCE',
    parameters: {
...
console.error(
  `D-PROC-1MG ${output.verdict} · accepted ${output.census.acceptedStates}/${REQUIRED_STATES}`
