/**
 * ⭐⭐ RA-T1 — THE ACCESS-PRICE EXAM (docs/world-model/RA-T1-ACCESS-EXAM.md).
 *
...
/* ========================================================================== */
const ENV_WHITELIST = ['RAT1_MODE', 'RAT1_N', 'RAT1_OUT'] as const;
...
const rogueOwn = Object.keys(process.env)
  .filter((k) => k.startsWith('RAT1_') && !(ENV_WHITELIST as readonly string[]).includes(k));
const rogueEngine = ENGINE_DOORS.filter((k) => process.env[k] !== undefined);
if (rogueOwn.length > 0 || rogueEngine.length > 0) {
  banner(`RA-T1 FATAL — unrecognised env: ${[...rogueOwn, ...rogueEngine].join(', ')}`);
  process.exit(3);
}
const MODE = process.env.RAT1_MODE as 'smoke' | 'full' | undefined;
if (MODE !== undefined && MODE !== 'smoke' && MODE !== 'full') {
  banner('RA-T1 FATAL — RAT1_MODE must be smoke|full'); process.exit(3);
}
const N_ENV = process.env.RAT1_N !== undefined ? Number(process.env.RAT1_N) : undefined;
if (N_ENV !== undefined && (!Number.isInteger(N_ENV) || N_ENV < 1)) {
  banner('RA-T1 FATAL — RAT1_N must be a positive integer'); process.exit(3);
}
const OUT_ENV = process.env.RAT1_OUT;
const OVERRIDE_REASONS = [
  ...(MODE !== undefined ? [`RAT1_MODE=${MODE}`] : []),
  ...(N_ENV !== undefined ? [`RAT1_N=${N_ENV}`] : []),
  ...(OUT_ENV !== undefined ? [`RAT1_OUT=${OUT_ENV}`] : []),
];
const IS_OVERRIDE = OVERRIDE_REASONS.length > 0;
const CANONICAL_OUT = 'docs/world-model/data/ra-t1-access-exam.json';
const CANONICAL_DIR_ABS = pathResolve('docs/world-model/data');
const OUT_BASE = OUT_ENV ?? (IS_OVERRIDE ? '/tmp/ra-t1-override.json' : CANONICAL_OUT);
const isCanonical = (p: string): boolean => pathResolve(p).startsWith(CANONICAL_DIR_ABS);
if (IS_OVERRIDE && isCanonical(OUT_BASE)) {
  banner('RA-T1 FATAL — an override run may never write the canonical artifact path');
  process.exit(3);
}
...
const dxc2Face = (k: string): { value: number; numerator: number; denominator: number } => {
  ...
  if (f === undefined) { banner(`RA-T1 FATAL — DX-C2 face missing: ${k}`); process.exit(3); }
  return f!;
};
...
banner('RA-T1 — the lockstep receipt (the observation wrappers, traced vs untraced, PER ARM)');
...
/** ⭐ G-ARMS-DIVERGE receipt: the two arms of the SAME seed are genuinely different worlds */
const ARMS_DIVERGE = LOCKSTEP_SEEDS.every((seed) => {
...
const pairs: Pair[] = [];
banner(`RA-T1 — the battery: ${N} PAIRS (${N * 2} walks), seeds `
...
defFace('groundPassesPerMatch', 'passes per match (240 s match clock)',
  '⭐ (b1) the H-DX.2(b) volume face — measured ground passes per match',
  (r) => r.gpMeasured, (r) => r.matches);
defFace('passCompletion', 'share',
  '⭐ (b2) the H-DX.2(b) completion face — the engine\'s own whole-match pass completion',
...
defFace('goalsPerMatch', 'goals per match (240 s match clock)', 'REPORTED — goals',
  (r) => r.goals, (r) => r.matches);
defFace('interceptionsPerMatch', 'per match (240 s match clock)', 'REPORTED — interceptions',
...
const face = (arm: 'shut' | 'armed', k: string): FaceRow => {
  ...
  if (f === undefined) { banner(`RA-T1 FATAL — unknown face ${arm}.${k}`); process.exit(3); }
  return f!;
};
...
const RULE_B2 = (d: { ciLo: number; ciHi: number }): boolean => !(d.ciHi < 0);
const deltas: DeltaRow[] = [
  ...
  pairedDelta('passCompletion', RULE_B2),
  ...
  pairedDelta('goalsPerMatch', null),
  pairedDelta('interceptionsPerMatch', null),
  pairedDelta('measuredCompletionAll', null),
];
const delta = (k: string): DeltaRow => {
  const d = deltas.find((x) => x.key === k);
  if (d === undefined) { banner(`RA-T1 FATAL — unknown delta ${k}`); process.exit(3); }
  return d!;
};

/* ========================================================================== */
/* §14 H-RA.1 — THE FROZEN RULES APPLIED                                       */
/* ========================================================================== */
const dA = delta('unmeetableCarriedPerMatch');
const A_VERDICT: 'FALLS' | 'DOES-NOT-FALL' = RULE_A(dA) ? 'FALLS' : 'DOES-NOT-FALL';
/** b1 — the DX-T2 §R3 band construction of record: armed POINT ≥ shut arm's own CI lower edge */
const b1ShutBand = face('shut', 'groundPassesPerMatch').ciLo;
const b1ArmedPoint = face('armed', 'groundPassesPerMatch').value;
const B1_VERDICT: 'HOLDS' | 'BELOW-BAND' = b1ArmedPoint >= b1ShutBand ? 'HOLDS' : 'BELOW-BAND';
const dB2 = delta('passCompletion');
const B2_VERDICT: 'DOES-NOT-FALL' | 'FALLS' = RULE_B2(dB2) ? 'DOES-NOT-FALL' : 'FALLS';
const H_RA1: 'PASS' | 'FAIL' =
  A_VERDICT === 'FALLS' && B1_VERDICT === 'HOLDS' && B2_VERDICT === 'DOES-NOT-FALL'
    ? 'PASS' : 'FAIL';
...
const SIZING_INPUTS: { face: string; group: string; hwSmoke: number; target: number }[] = [
  { face: 'unmeetableCarriedPerMatch', group: '(a)',
    hwSmoke: 1.3333333333333335, target: -DXC2_UNMEETABLE_PER_MATCH / 4 },
  { face: 'passCompletion', group: '(b2)', hwSmoke: 0.03354021139811786, target: 0.01 },
];
...
const gates: Record<string, { ok: boolean; note: string }> = {
  ...
  gArmsDiverge: {
    ok: ARMS_DIVERGE,
    note: '⭐ the RECEIPT that the two arms are genuinely different worlds (RA-T0\'s G-BITE '
      + 're-observed on this exam\'s scratch seeds) — an INSTRUMENT receipt, never a finding',
  },
  gQuotedSourceIntact: {
    ok: DXC2_QUOTED_OK,
...
const BODY_SCHEMA = [
  'stage', 'gates', 'faces', 'deltas', 'hRA1', 'bins', 'arms',
...
const artifact: Record<string, unknown> = {
  stage: {
    ...
    doc: 'docs/world-model/RA-T1-ACCESS-EXAM.md',
    ...
    authorizedBy: 'COMMANDER RULING #361 item 3',
    ...
    instrument: 'scripts/probes/ra-t1-access-exam.ts',
    instrumentSha256: sha(readFileSync('scripts/probes/ra-t1-access-exam.ts', 'utf8')),
  ...
  hRA1: {
    frozenRules: {
      a: '(a) Δ(armed − shut) on unmeetableCarriedPerMatch: 95 % interval ENTIRELY BELOW '
        + 'ZERO ⇒ FALLS. ⚠ DEVIATION FROM #361\'s WORDING, STATED: the ruling says "share"; '
        + 'the share is REPORTED beside but is undefined exactly when the door works hardest '
        + '(carried → 0), so the COUNT is primary. FLAGGED FOR RATIFICATION.',
      b1: '(b1) the armed groundPassesPerMatch POINT sits ABOVE the SHUT arm\'s own 95 % '
        + 'interval LOWER EDGE (DX-T2 §R3\'s band construction of record).',
      b2: '(b2) Δ(armed − shut) on passCompletion: the 95 % interval is NOT entirely below '
        + 'zero ⇒ DOES-NOT-FALL (a non-fall form at its declared MDE, never "restored").',
    },
    aVerdict: A_VERDICT,
    b1: { band: b1ShutBand, armedPoint: b1ArmedPoint, verdict: B1_VERDICT },
...
const disk = JSON.parse(readFileSync(OUT_PATH_PRE, 'utf8')) as typeof artifact & {
  ...
  hRA1: { verdict: string; aVerdict: string; b1: { band: number; armedPoint: number; verdict: string };
    b2Verdict: string; aDelta: DeltaRow; b2Delta: DeltaRow };
};
...
{
  const dsa = disk.hRA1.aDelta;
  const reA = dsa.ciHi < 0 ? 'FALLS' : 'DOES-NOT-FALL';
  binChecks.push({ bin: 'hRA1.aVerdict', ok: reA === disk.hRA1.aVerdict });
  const shutVol = disk.faces.find((x) => x.arm === 'shut' && x.face === 'groundPassesPerMatch')!;
  const armedVol = disk.faces.find((x) => x.arm === 'armed' && x.face === 'groundPassesPerMatch')!;
  const reB1 = armedVol.value >= shutVol.ciLo ? 'HOLDS' : 'BELOW-BAND';
  binChecks.push({
    bin: 'hRA1.b1',
    ok: reB1 === disk.hRA1.b1.verdict && disk.hRA1.b1.band === shutVol.ciLo
      && disk.hRA1.b1.armedPoint === armedVol.value,
  });
  const dsb = disk.hRA1.b2Delta;
  const reB2 = !(dsb.ciHi < 0) ? 'DOES-NOT-FALL' : 'FALLS';
  binChecks.push({ bin: 'hRA1.b2Verdict', ok: reB2 === disk.hRA1.b2Verdict });
  const reAll = (reA === 'FALLS' && reB1 === 'HOLDS' && reB2 === 'DOES-NOT-FALL') ? 'PASS' : 'FAIL';
  binChecks.push({ bin: 'hRA1.verdict', ok: reAll === disk.hRA1.verdict });
}
{
...
/* ========================================================================== */
banner('');
banner(`RA-T1 — ${ALL_GREEN ? 'ALL GATES GREEN' : '⛔ RED — artifact routed to the .RED path'}`);
...
banner(`  (b1) groundPassesPerMatch armed point ${b1ArmedPoint.toFixed(6)} vs shut band `
  + `${b1ShutBand.toFixed(6)}  ⇒ ${B1_VERDICT}`);
banner(`  (b2) passCompletion Δ ${dB2.delta.toFixed(6)} [${dB2.ciLo.toFixed(6)}, `
