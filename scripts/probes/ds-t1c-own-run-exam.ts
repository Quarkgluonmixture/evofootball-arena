 ...
 * (docs/world-model/DS-T1C-OWN-RUN-EXAM-RANK.md).
 *
 * Authorized by COMMANDER RULING #410 item 3. Lineage: DS-C0 (the WALKER) → DS-T0 (the seam) →
 * DS-T1 (the exam, read 3 — THE FLOOD) → DS-T0b (the velocity restraint)
 * → DS-T1b (THE INSTRUMENT THIS FILE INHERITS — #408 item 5, banked at #409; read 4 — THE DRAIN;
 * ⛔ NO number of theirs is typed in this file: R1's and G9's readings are quoted BY FIELD out
 * of both artifacts under `hNumbers.hDs5.dsT1Quoted` / `.dsT1bQuoted`)
 * → DS-T0c (THE SEAM UNDER EXAM: the coach's RANKED selection moved into the player —
 * `runRank(role, localX)` code-moved, `rankAbove` over the mates his own eyes hold by the coach's
 * own comparator, and `restraint = clamp01(count − rankAbove) ∈ {0, 1}`, the coach's
 * `slice(0, count)` expressed as a CAP; the velocity mass REMOVED) → OBM-T1 / LN-T1 (THE TWO
 * DOSES: RUN-CAUTION, a hand-set PROBE CORNER in OBM-T1's own `matrix(...)` idiom, and
 * KITCHEN-SINK byte-copied from OBM-T1's ceiling probe, both re-derived slot for slot off the
 * `OBM_*` exports in LN-T1's G-DOSE-COPY form).
 *
 * THE QUESTION (#410 item 3, not re-argued here): with the coach's RANKED selection moved into
 * the player, does the own run hold the coach's band WITHOUT the coach (H-DS-5), and does the
 * eyes' price move it INSIDE the band rather than draining it (H-DS-6)?
 *
 ...
 * unchanged and "dosed" = RUN-CAUTION; NO VERDICT WORD is printed on any yield, coupling, seam
 * or hypothesis face — H-DS-5 / H-DS-6's numbers are PRINTED BESIDE the read, never judged.
 ...
 *
 * ⭐⭐ THE SEAM'S FACES ARE BACKED OUT, NEVER RECOMPUTED, AND THE RANK LAW MAKES THEM A STEP.
 ...
 * is EXACTLY `restraint · obmRunMul`; on a seat-ABSENT arm that IS the restraint, and under the
 * rank law it lies in {0, 1}. ⚠ A ZERO SCORE IS AMBIGUOUS — restraint 0 OR a ZERO PRIOR (the DF
 * clamp: a defender deep in his own half prices his own run at exactly 0) — so the prior is
 * RECOMPUTED from the body's OWN pos and role, which reads NO snapshot, and the two are
 * SEPARATED: `restraint.exactlyZeroShare` (prior > 0, back-out 0), `priorZeroShare` and the
 * AMBIGUOUS OVERLAP are all stored. ⛔ `rankAbove`'s exact value is NOT recoverable from a score
 * — the observable is `rankAbove < count`, i.e. the restraint-1 share, published as
 * `rankBelowCount` and DECLARED as that equivalence, never as a rank distribution.
 * Recomputing the rank itself is IMPOSSIBLE byte-inertly — `perceivedSnapshot` MUTATES
 * perception memory.
 *
...
/* ========================================================================== */
const ENV_WHITELIST = ['DST1C_MODE', 'DST1C_N', 'DST1C_OUT'] as const;
...
const rogueOwn = Object.keys(process.env)
  .filter((k) => k.startsWith('DST1C_') && !(ENV_WHITELIST as readonly string[]).includes(k));
const rogueEngine = ENGINE_DOORS.filter((k) => process.env[k] !== undefined);
if (rogueOwn.length > 0 || rogueEngine.length > 0) {
  banner(`DS-T1c FATAL — unrecognised env: ${[...rogueOwn, ...rogueEngine].join(', ')}`);
  process.exit(3);
}
const MODE = process.env.DST1C_MODE as 'smoke' | 'full' | undefined;
if (MODE !== undefined && MODE !== 'smoke' && MODE !== 'full') {
  banner('DS-T1c FATAL — DST1C_MODE must be smoke|full'); process.exit(3);
}
const N_ENV = process.env.DST1C_N !== undefined ? Number(process.env.DST1C_N) : undefined;
if (N_ENV !== undefined && (!Number.isInteger(N_ENV) || N_ENV < 1)) {
  banner('DS-T1c FATAL — DST1C_N must be a positive integer'); process.exit(3);
}
const OUT_ENV = process.env.DST1C_OUT;
const OVERRIDE_REASONS = [
  ...(MODE !== undefined ? [`DST1C_MODE=${MODE}`] : []),
  ...(N_ENV !== undefined ? [`DST1C_N=${N_ENV}`] : []),
  ...(OUT_ENV !== undefined ? [`DST1C_OUT=${OUT_ENV}`] : []),
];
const IS_OVERRIDE = OVERRIDE_REASONS.length > 0;
const CANONICAL_OUT = 'docs/world-model/data/ds-t1c-own-run-exam.json';
const CANONICAL_DIR_ABS = pathResolve('docs/world-model/data');
const OUT_BASE = OUT_ENV ?? (IS_OVERRIDE ? '/tmp/ds-t1c-override.json' : CANONICAL_OUT);
const isCanonical = (p: string): boolean => pathResolve(p).startsWith(CANONICAL_DIR_ABS);
if (IS_OVERRIDE && isCanonical(OUT_BASE)) {
  banner('DS-T1c FATAL — an override run may never write the canonical artifact path');
  process.exit(3);
}
/** ⭐⭐ THE INSTRUMENT OF RECORD — this file's own path, for the stage block's hash. */
const INSTRUMENT_PATH = 'scripts/probes/ds-t1c-own-run-exam.ts';
...
/* ========================================================================== */
/* §5 SEEDS — block 12,556,000–999 (#410 items 3(v) and 7)                     */
/* ========================================================================== */
const BLOCK_BASE = 12_556_000;
const BLOCK_TOP = 12_556_999;
/** ⭐⭐ N_FROZEN = 999 — the block's OWN AFFORDANCE after the construction receipt at
 *  12,556,999 (battery seeds 12,555,000–12,555,998). §DEV-PREFLIGHT's sizing rows are computed
...
const SCRATCH_BASE = 900_007_000;
...
/** ⭐⭐⭐ G-REPRO-DST1b's RE-WALK band — DS-T1b's OWN CONSUMED block. NOT a consumption. */
const REPRO_SEEDS = Array.from({ length: 12 }, (_, i) => 12_555_000 + i);
/** ⭐⭐⭐ THE TWO PRIOR ARTIFACTS — DS-T1b's is what G-REPRO-DST1b re-walks against, and BOTH are
 *  quoted BY FIELD beside the reads. ⛔ NOT ONE of their numbers is typed in this file. */
...
const ARM_LABEL: Record<Arm, string> = {
  ...
  'OWN-E13-ABSENT': '⭐⭐⭐ world 13 EMPTY-BOOK + `dsOwnRun` + `dsHatsOff` — OWN RUN ALONE with '
    + 'the RANK RESTRAINT in the player, seat ABSENT. THE ARM OF RECORD (H-DS-5).',
  'OWN-E13-RUNCAUTION': '⭐⭐⭐ OWN RUN ALONE, seat DOSED at RUN-CAUTION — "dosed" in the reads; '
    + 'H-DS-6\'s arm (a dose whose `runScore` row is NOT zero)',
...
if (L3_DOSE_BYTES_SHA !== L3_DOSE_PIN || PC_DOSE_BYTES_SHA !== PC_DOSE_PIN) {
  banner('DS-T1c FATAL — a dose file\'s BYTES do not match the pinned value');
...
if (!D13_REACHABLE) {
  banner(`DS-T1c FATAL — the D13 arms are not reachable: ${DOSE_LOAD_ERROR ?? 'empty dose'}`);
  process.exit(3);
}
...
/** the THREE READ-FORK rows of §SEAM-C, with the LINE the commander MEASURED at #410 */
const DOC_READ_FORKS = DOC_SEAM_C_SITES.filter((r) => r.cls === 'READ FORK')
...
/** ⚠ the SAME per-file flag counts are written THREE times in the seam doc (§SEAM, §SEAM-B and
 *  §SEAM-C's refreshed paragraph). The count of ROWS is a receipt; what the gate compares is the
 *  DEDUPED set against the measured one, so a doc copy that DRIFTED would break the dedupe. */
fx('readForks.thePerFileCountsAppearMoreThanOnceInTheDoc_soTheGateDEDUPES',
  DOC_FLAG_COUNTS.length, 10);
fx('readForks.andTheyDEDUPEToFOUR_whichIsWhatADRIFTEDCopyWouldBreak',
  DOC_FLAG_COUNTS_DEDUPED.length, 4);
...
/* ========================================================================== */
banner('DS-T1c — the lockstep receipt (observed vs unobserved, PER ARM)');
...
const cells: Cell[] = [];
banner(`DS-T1c — the battery: ${N} seeds × ${ARMS.length} arms, seeds `
...
/* ========================================================================== */
/* §12b G-REPRO-DST1b — RE-WALK DS-T1b's OWN BAND ON HATS-E13-ABSENT           */
/*      + THE TWO PRIOR STAGES' NUMBERS, QUOTED BY FIELD                       */
/* ========================================================================== */
/** ⭐⭐⭐ DS-T1's and DS-T1b's own numbers, QUOTED **BY FIELD** out of their artifacts and NEVER
 *  typed in this instrument (canon: *doc-prose fidelity*, applied to an instrument). Each raw
 *  artifact is parsed inside its own scope so only the PICKED objects are retained. */
const pickFrom = (path: string) => {
  ...
  return {
    ...
    r1DeltaOwnDosed: pickDelta('r1.runsPerInPossessionTick', 'OWN-E13-RUNCAUTION')
      ?? pickDelta('r1.runsPerInPossessionTick', 'OWN-E13-DOSED'),
...
/** ⭐⭐⭐ G-REPRO-DST1b — the seam's OFF path RE-WALKED on DS-T1b's OWN CONSUMED BAND and compared
 *  FIELD FOR FIELD against its stored cells. A MISMATCH IS RED. */
const reproDetail = (() => {
  if (!existsSync(DST1B_ARTIFACT)) {
    return { ran: false, ok: false, seeds: REPRO_SEEDS, comparedFields: [] as string[],
      ...
      note: `the DS-T1b artifact is absent at ${DST1B_ARTIFACT}` };
  }
  const raw = JSON.parse(readFileSync(DST1B_ARTIFACT, 'utf8')) as {
  ...
  const rows = REPRO_SEEDS.map((seed) => {
    ...
    if (theirs === undefined) {
      return { seed, mismatches: ['ABSENT FROM DS-T1b'],
  ...
  return {
    ...
    seeds: REPRO_SEEDS, comparedFields: fields, rows,
    note: '⭐⭐⭐ G-REPRO-DST1b: `HATS-E13-ABSENT` re-walked on DS-T1b\'s OWN CONSUMED BAND '
      + '(12,555,000–011 — NOT a consumption) and compared FIELD FOR FIELD against DS-T1b\'s '
      + 'stored `perSeedCells[].[HATS-E13-ABSENT]`. This is THE SEAM\'S OFF PATH: with both DS '
      + 'flags absent DS-T0c is byte-identical to DS-T0b\'s head by its own G-OFF pins (five '
      + 'worlds, world 13 included, the rng draw inside the hash), so EVERY field DS-T1b stored '
      + 'must reproduce — the whole-match signature included. A MISMATCH IS RED. Only `wallMs` '
      + '(a machine timing) is excluded; DS-T1c\'s OWN NEW FIELDS (the prior-zero split and the '
      + 'restraint\'s third cell) have no counterpart in DS-T1b\'s row and are not compared, '
      + 'and DS-T1b\'s RETIRED `runningMates` fields have none in this one.',
  };
})();
const REPRO_OK = reproDetail.ok;
banner(`  G-REPRO-DST1b ${REPRO_OK ? 'GREEN' : 'RED'} `
...
const face = (k: string, armK: Arm): FaceRow => {
  ...
  if (f === undefined) { banner(`DS-T1c FATAL — unknown face ${k}/${armK}`); process.exit(3); }
  return f as FaceRow;
};
...
const delta = (faceKey: string, armK: Arm): DeltaRow => {
  ...
  if (dd === undefined) { banner(`DS-T1c FATAL — unknown Δ ${faceKey}@${armK}`); process.exit(3); }
...
/* ========================================================================== */
/** ⭐⭐ THE PAIRED RATIO — the arm's level ÷ its control's, on shared seeds, with the SAME cluster
 *  bootstrap. #410 item 3(ii)'s new beside-face for H-DS-5: how much of the coach's rate the
 *  player's own run is. ⛔ PRINTED, NO VERDICT WORD; a ratio of two rates carries no unit. */
const pairedRatio = (faceKey: string, armK: Arm) => {
  const f = FACES[faceKey];
  const ctrl = CONTROL_OF[armK];
  const nA = cells.map((c) => f.num(c.rows[armK]));
  const dA = cells.map((c) => f.dn(c.rows[armK]));
  ...
  draws.sort((a, b) => a - b);
  return {
    key: `ratio.${faceKey}@${armK}`, face: faceKey, arm: armK, controlArm: ctrl,
    ...
    note: 'the arm\'s level DIVIDED BY its control\'s, cluster-bootstrapped on the same seeds. '
...
const floodRowFor = (armK: Arm) => {
  ...
  return {
    ...
    /* ⭐ #409 item 5 (the FORM RULE OF RECORD): the FLOOD selector's column is named
...
const selectorFor = (armK: Arm) => ({
  ...
  r1BeyondToleranceUp: FLOOD_ROWS[armK].beyondToleranceUp,
  r1AbsDeltaBeyondToleranceEitherWay:
    FLOOD_ROWS[armK].absDeltaBeyondToleranceEitherWay,
});
...
const seatBiteRows = (['HATS-E13-RUNCAUTION', 'HATS-E13-KITCHENSINK'] as Arm[]).map((armK) => {
  ...
  const identical = cells.filter((c) =>
    c.rows[armK].signature === c.rows['HATS-E13-ABSENT'].signature).map((c) => c.seed);
  return {
    ...
    /** ⭐ #409 §COMMANDER CORRECTIONS 4: the seat-bite seeds are STORED AS A FIELD, not
     *  "recoverable" — DS-T1b promised them in prose and stored none. */
    identicalSeeds: identical,
    identicalSeedCount: identical.length,
...
/** ⭐⭐⭐ H-DS-5 / H-DS-6's NUMBERS, printed BESIDE the read — DS-T1's and DS-T1b's own values
 *  are QUOTED BY FIELD out of their artifacts, never typed. ⛔ NO VERDICT WORD: this block
 *  stores numbers and stored booleans only, and the hypotheses are NOT judged. */
...
const H_NUMBERS = {
  note: '⛔ PRINTED, NEVER JUDGED (#410 item 3(iii)): no verdict word is stored on any hypothesis '
    + 'face. DS-T1\'s and DS-T1b\'s numbers are read out of their own artifacts BY FIELD.',
  hDs5: {
    hypothesis: 'H-DS-5 — the rank restraint reproduces the coach\'s band without the coach: R1 '
      + 'within tolerance of HATS and G9 inside the band (ruling #409 item 3(iv); the seam doc '
      + '§HONESTY-C 1)',
    r1: hRow(R1_KEY, ARM_OF_RECORD),
    r1Level: face(R1_KEY, ARM_OF_RECORD).value,
    r1ControlLevel: face(R1_KEY, CONTROL_OF[ARM_OF_RECORD]).value,
    r1ShareAtLeastThree: face('r1.floodShareAtLeastThree', ARM_OF_RECORD).value,
    r1ShareAtLeastThreeControl:
      face('r1.floodShareAtLeastThree', CONTROL_OF[ARM_OF_RECORD]).value,
    /** ⭐⭐ #410 item 3(ii)'s NEW BESIDE-FACE: the own-run rate ÷ the hats rate as a RATIO with
     *  its cluster-bootstrapped interval. PRINTED, no verdict word. */
    ownOverHatsRatio: pairedRatio(R1_KEY, ARM_OF_RECORD),
    g9: hRow('guard.throughBallsPerMatch', ARM_OF_RECORD),
    g9OwnOverHatsRatio: pairedRatio('guard.throughBallsPerMatch', ARM_OF_RECORD),
    dsT1Quoted: DST1_QUOTED === null ? null : {
    ...
    dsT1bQuoted: DST1B_QUOTED === null ? null : {
      ...
      seamFaces: DST1B_QUOTED.seamFacesOwnAbsent,
      readOfRecord: DST1B_QUOTED.readOfRecord, source: DST1B_QUOTED.source,
    },
    thisStageBreachedGuards: breachingGuards(ARM_OF_RECORD),
    thisStageHoldsBand: holdsBand(ARM_OF_RECORD),
    thisStageFloods: floods(ARM_OF_RECORD),
  },
  hDs6: {
    hypothesis: 'H-DS-6 — with the rank restraint the eyes\' price (RUN-CAUTION) moves R1 down '
      + 'INSIDE the band rather than draining it (ruling #409 item 3(iv); the seam doc '
      + '§HONESTY-C 2)',
    seatAbsent: hRow(R1_KEY, ARM_OF_RECORD),
    runCaution: hRow(R1_KEY, DOSED_ARM_OF_RECORD),
    kitchenSink: hRow(R1_KEY, CEILING_ARM),
    runCautionHoldsBand: holdsBand(DOSED_ARM_OF_RECORD),
    runCautionBreachedGuards: breachingGuards(DOSED_ARM_OF_RECORD),
    runCautionFloods: floods(DOSED_ARM_OF_RECORD),
    kitchenSinkHoldsBand: holdsBand(CEILING_ARM),
    kitchenSinkBreachedGuards: breachingGuards(CEILING_ARM),
    theDoseReachesTheRun: {
      what: 'RUN-CAUTION and KITCHEN-SINK both carry a NON-ZERO `runScore` row; MARKER-ESCAPE '
        + '(DS-T1\'s dose) does not, which is why #407 item 3 struck it. THE SEAT\'S FACE OF '
        + 'RECORD IS THE CLEAN `runMulLic` LIMB (the LICENSED run\'s score carries no restraint '
        + 'factor), with the seat-ABSENT arms as its EXACTLY-ZERO floor.',
      runMulLicensedLimb: {
    ...
    dsT1bQuotedRunCaution: DST1B_QUOTED === null ? null : DST1B_QUOTED.r1DeltaOwnDosed,
    dsT1QuotedMarkerEscape: DST1_QUOTED === null ? null : DST1_QUOTED.r1DeltaOwnDosed,
  },
  theSupersession: 'H-DS-3 (the perceived restraint holds the coach\'s count) is SUPERSEDED by '
    + 'H-DS-5 (#410 item 2). H-DS-4 (withdrawing the in-flight run costs the through-ball gain) '
    + 'was answered at #409 item 3(ii) and its face — the in-flight own-run share — is still '
    + 'stored on every arm.',
};
...
const SIZING_INPUTS = [
  { face: 'r1.runsPerInPossessionTick@OWN-E13-ABSENT',
    hwSmoke: 0.07295841836933506, target: 0.05 },
  { face: 'guard.passCompletion@OWN-E13-ABSENT',
    hwSmoke: 0.041914891678440425, target: 0.05 },
];
...
const gates: Record<string, { ok: boolean; note: string }> = {
  ...
  gSeedsBookedEqualWalked: {
    ...
    note: 'BOOKED = WALKED, derived from the CELLS\' OWN distinct seeds: every battery seed and '
      + 'the construction receipt lie inside block 12,556,000–999, each seed is walked ONCE PER '
      ...
      + '12,555,000–011 are DS-T1b\'s OWN CONSUMED BAND and are NOT a consumption — canon, '
      + 'VERBATIM: "verifier '
  ...
  gSeedDisjoint: {
    ok: walkedSeeds.every((s) => s >= BLOCK_BASE) && ALL_SCRATCH.every((s) => s >= 900_000_000)
      ...
      && REPRO_SEEDS.every((s) => s >= 12_555_000 && s <= 12_555_999),
    note: 'SEED-DISJOINT at the frontier of #410 item 7 (next sim ≥ 12,556,000): every battery '
      + 'seed is ≥ 12,556,000 and inside THIS block, disjoint from every consumed block (LN-C0 '
      ...
      + '12,553,000–999 · DS-T1 12,554,000–999 · DS-T1b 12,555,000–999), each of which is '
      + 'checked to end BELOW this block\'s base; the G-REPRO-DST1b re-walks lie INSIDE '
      + 'DS-T1b\'s own consumed block; '
      + 'ZERO stats consumed',
  },
  gN: {
...
const artifact: Record<string, unknown> = {
  stage: {
    id: 'DS-T1c',
    title: '「自己的前插 · 三考」 THE OWN-RUN EXAM, THIRD RUN — with the COACH\'S RANKED SELECTION '
      + 'in the player: '
      ...
      + 'DS-T1\'s, re-walked with its three debts still paid, PLUS the seam\'s faces ADAPTED TO '
      + 'THE RANK LAW (the restraint at EXACTLY 0 and EXACTLY 1, the `priorZeroShare` that is '
      + 'the ambiguous overlap, the OBSERVABLE `rankBelowCount` share, the count distribution, '
      + 'the perceived-owner guard\'s pass floor, the in-flight and restart shares, and the '
      + 'seat\'s `runMulLic` clean limb with its zero floor beside)',
    doc: 'docs/world-model/DS-T1C-OWN-RUN-EXAM-RANK.md',
    instrument: INSTRUMENT_PATH,
    instrumentSha256: sha(readFileSync(INSTRUMENT_PATH, 'utf8')),
    inheritedInstrument: 'scripts/probes/ds-t1b-own-run-exam.ts',
    inheritedStageDoc: 'docs/world-model/DS-T1B-OWN-RUN-EXAM-RERUN.md',
    inheritedStageDocGrandparent: 'docs/world-model/DS-T1-OWN-RUN-EXAM.md',
    ...
    authorizedBy: 'COMMANDER RULING #410 item 3',
    kind: 'EXAM — it arms NOTHING in the game and SHIPS NOTHING. The READ SENTENCES are #406 '
      + 'item 5(v)\'s FOUR literals RE-FROZEN VERBATIM by #410 item 3(iii), selected by STORED '
      + 'booleans with the precedence unchanged and "dosed" = RUN-CAUTION; NO VERDICT WORD is '
      + 'printed on any yield, coupling, SEAM or HYPOTHESIS face — H-DS-5 / H-DS-6\'s numbers '
      + 'are PRINTED BESIDE the read and never judged. The commander rules.',
    ...
    theQuestion: 'With the coach\'s RANKED selection moved into the player (his rank among the '
      + 'mates he perceives, by the shared convention), does the own run hold the coach\'s band '
      + 'WITHOUT the coach (H-DS-5), and does the eyes\' price move it INSIDE the band rather '
      + 'than draining it (H-DS-6)? The DF path (M-DF.2): the cap retires by measurement, never '
      + 'by deletion.',
    theArcSoFarQuotedByField: '⭐⭐⭐ DS-T1 FLOODED and DS-T1b DRAINED — both readings are QUOTED '
      + 'BY FIELD out of their own artifacts under `hNumbers.hDs5.dsT1Quoted` and '
      + '`.dsT1bQuoted`, never typed here. DS-T0c replaced the velocity mass with the coach\'s '
      + 'own ranked selection; this exam measures that form.',
    ...
    theSeamsFacesAreBackedOut: '⭐⭐⭐ THE RESTRAINT is INVERTED out of the engine\'s own recorded '
      + 'candidate score, never recomputed: recomputing it would need `match.perceivedSnapshot`, '
      + 'which MUTATES the body\'s perception memory, so the observation would stop being '
      + 'byte-inert and `gLockstep` would be a lie. On a seat-ABSENT arm `obmRunMul` is EXACTLY '
      + '1 by construction and the back-out IS the restraint — which under the RANK LAW lies in '
      + '{0, 1}; on a DOSED arm the same number is the PRODUCT `restraint · obmRunMul` and is '
      + 'stored under that name, with the seat\'s own multiplier taken instead from the LICENSED '
      + 'run\'s score, which carries no restraint factor. ⚠ A ZERO SCORE IS AMBIGUOUS — '
      + 'restraint 0 OR a ZERO PRIOR (the DF clamp) — so the prior is RECOMPUTED from the '
      + 'body\'s OWN pos and role (no snapshot read) and the two populations are SEPARATED and '
      + 'both stored. ⛔ `rankAbove`\'s exact value is NOT recoverable from a score; the '
      + 'OBSERVABLE `rankAbove < count` is published as `rankBelowCount` and NO rank '
      + 'distribution is written.',
    theVelocityMassIsGone: '⭐⭐ DS-T1b\'s `runningMates` family is RETIRED with the term it '
      + 'inverted (§LAW-C removed the velocity mass). A STORED BOOLEAN derived from the block\'s '
      + 'WHOLE TEXT records it: `codeFacts.ownRunBlock.blockReadsNoVelocityNoTopSpeed`.',
    theThreeDebtsPaid: {
    ...
    honestLimitsNote: '⛔ canon, VERBATIM: "a stage doc\'s HONEST LIMITS list is the ONE home; '
      ...
      + 'The ONE home is docs/world-model/DS-T1C-OWN-RUN-EXAM-RANK.md §HONEST LIMITS.',
  ...
  sizing: {
    ...
    varianceSource: 'THIS exam\'s own 12-cluster SCRATCH SMOKE (seeds 900,007,000–011, TWELVE '
  ...
  stats: { consumed: 0, nextBase: 117_600, registryOfRecord: 85 },
  perf: {
  ...
  reads: {
    note: '⭐⭐⭐ #406 item 5(v)\'s SENTENCES are FROZEN LITERALS. The SELECTORS are STORED '
      ...
      + 'E13; the DOSED OWN arm answers H-DS-6; D13\'s word is computed by the SAME rule '
    ...
    annotations: [
      ...
      `H-DS-5 (printed, not judged) — R1's paired Δ on OWN vs HATS, seat absent: `
      + `${H_NUMBERS.hDs5.r1.delta} [${H_NUMBERS.hDs5.r1.ci[0]}, ${H_NUMBERS.hDs5.r1.ci[1]}]; `
      + `DS-T1's own field ${JSON.stringify(H_NUMBERS.hDs5.dsT1Quoted?.r1Delta?.delta ?? null)} `
      + `· DS-T1b's own field `
      + `${JSON.stringify(H_NUMBERS.hDs5.dsT1bQuoted?.r1Delta?.delta ?? null)}`,
      `H-DS-5 (printed, not judged) — G9 through balls' paired Δ: ${H_NUMBERS.hDs5.g9.delta} `
      + `[${H_NUMBERS.hDs5.g9.ci[0]}, ${H_NUMBERS.hDs5.g9.ci[1]}]; DS-T1's own field `
      + `${JSON.stringify(H_NUMBERS.hDs5.dsT1Quoted?.g9Delta?.delta ?? null)} · DS-T1b's own `
      + `field ${JSON.stringify(H_NUMBERS.hDs5.dsT1bQuoted?.g9Delta?.delta ?? null)}`,
      `H-DS-5 (printed, not judged) — the OWN ÷ HATS ratio of R1: `
      + `${H_NUMBERS.hDs5.ownOverHatsRatio.ratio} `
      + `[${H_NUMBERS.hDs5.ownOverHatsRatio.ci[0]}, ${H_NUMBERS.hDs5.ownOverHatsRatio.ci[1]}]`,
      `H-DS-6 (printed, not judged) — R1's paired Δ seat absent ${H_NUMBERS.hDs6.seatAbsent
        .delta} · at RUN-CAUTION ${H_NUMBERS.hDs6.runCaution.delta} · at KITCHEN-SINK `
      + `${H_NUMBERS.hDs6.kitchenSink.delta}; holdsBand at RUN-CAUTION `
      + `${H_NUMBERS.hDs6.runCautionHoldsBand}`,
      ...
      + `${SEAM_FACES[ARM_OF_RECORD].restraint.exactlyOneShare} · rankBelowCount `
      + `${SEAM_FACES[ARM_OF_RECORD].rankBelowCount.share} · a ZERO PRIOR (the DF clamp, the `
      + `AMBIGUOUS overlap) ${SEAM_FACES[ARM_OF_RECORD].priorZero.priorZeroShare}`,
    ],
    ...
    besideTheRead: {
      ...
      hypothesisNumbersPointer: 'hNumbers (H-DS-5 · H-DS-6, printed never judged)',
...
banner('');
banner(`DS-T1c — ${ALL_GREEN_FINAL ? 'ALL GATES GREEN' : '⛔ RED — artifact routed to .RED'}`);
...
for (const armK of ARMS) {
  ...
  banner(`    rankBelowCount share ${f6(sf.rankBelowCount.share)} · n `
    + `${sf.rankBelowCount.observations} · priorZero share `
    + `${f6(sf.priorZero.priorZeroShare)} · neither-0-nor-1 `
    + `${f6(sf.priorZero.restraintNeitherZeroNorOneShare)}`);
...
banner(`  H-DS-5 R1 Δ ${f6(H_NUMBERS.hDs5.r1.delta)} (DS-T1 `
  + `${JSON.stringify(H_NUMBERS.hDs5.dsT1Quoted?.r1Delta?.delta ?? null)} · DS-T1b `
  + `${JSON.stringify(H_NUMBERS.hDs5.dsT1bQuoted?.r1Delta?.delta ?? null)}) · G9 Δ `
  + `${f6(H_NUMBERS.hDs5.g9.delta)} (DS-T1 `
  + `${JSON.stringify(H_NUMBERS.hDs5.dsT1Quoted?.g9Delta?.delta ?? null)} · DS-T1b `
  + `${JSON.stringify(H_NUMBERS.hDs5.dsT1bQuoted?.g9Delta?.delta ?? null)}) · OWN÷HATS `
  + `${f6(H_NUMBERS.hDs5.ownOverHatsRatio.ratio)} `
  + `[${f6(H_NUMBERS.hDs5.ownOverHatsRatio.ci[0])}, `
  + `${f6(H_NUMBERS.hDs5.ownOverHatsRatio.ci[1])}]`);
banner(`  H-DS-6 absent ${f6(H_NUMBERS.hDs6.seatAbsent.delta)} / RUN-CAUTION `
  + `${f6(H_NUMBERS.hDs6.runCaution.delta)} (holdsBand `
  + `${H_NUMBERS.hDs6.runCautionHoldsBand}) / KITCHEN-SINK `
  + `${f6(H_NUMBERS.hDs6.kitchenSink.delta)}`);
