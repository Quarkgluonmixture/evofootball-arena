...
//
// D-PROC-1 verbatim with exactly two causal substitutions: the observation
// channel is the S3-G1 memory-guided gaze (one-tick latency, previous-gaze
// threading, purity audit), and the single-observation 0.25m support trigger is
// replaced by the qualified three-sample motion-phase predicate in
// motionGatedIntentResponse.ts. Every response, edge, progress, separation,
// anti-oscillation and exact gate value is inherited from D-PROC-1.
...
const SEED_START = Number(process.argv[3] ?? 90_000);
const MAX_SEEDS = 192;
...
const runArm = (state: FrozenState, arm: Arm): ArmResult => {
  ...
  // Gaze-threaded observation: aim B's gaze at A from B's own memory (S3-G1),
  // one-tick latency, previous-gaze fallback, then perceive and refresh the
  // three-sample motion history and the motion-gated belief.
  const observe = (): 'ok' | 'unsupported' | 'schema' => {
    const truth = capturePerceptionTruth(match);
    const gaze = chooseAttentionGaze(previousSnapshot, actor.gid, previousGaze);
    gazeLog.push(gaze);
    if (gaze !== null) {
      if (
        gaze.observerGid !== observer.gid
        || !Number.isInteger(gaze.establishedTick)
        || gaze.establishedTick < 0
        || gaze.establishedTick > truth.tick
      ) gazeInvalid++;
      if (Math.abs(Math.hypot(gaze.gazeDir.x, gaze.gazeDir.y) - 1) > 1e-9) nonNormalisedGaze++;
    }
    const rngBefore = (match.rng as unknown as { s: number }).s;
    const snapshot = perceiveSnapshot(
      truth, state.observerGid, AWARENESS, state.seed, memory, gaze,
    );
    ...
    if (!observedActor || !snapshot.players.some((entry) => entry.gid === carrier.gid)) {
      return 'unsupported';
    }
    const next = appendObservedMotionSample(
      snapshot, actor.gid, state.carrierGid, state.referenceEpoch, history,
    );
    ...
    evidenceSignatures.push(JSON.stringify({ history: next, belief }));
    return 'ok';
  };

  const firstObserve = observe();
  if (firstObserve === 'unsupported') return emptyResult('observerUnsupported');
  if (firstObserve === 'schema') return emptyResult('schemaFailure');
  ...
  // S3-G1 purity audit: every gaze must be recomputable from the logged
  // snapshot sequence alone (acceptance snapshot, then each perceived snapshot).
  let replayPrevious: ObserverGaze | null = null;
  let replayInput: PerceptionSnapshot = state.acceptanceSnapshot;
  for (let index = 0; index < gazeLog.length; index++) {
    const replayGaze = chooseAttentionGaze(replayInput, actor.gid, replayPrevious);
    if (JSON.stringify(replayGaze) !== JSON.stringify(gazeLog[index])) recomputeMismatches++;
    replayPrevious = replayGaze;
    replayInput = snapshotLog[index];
  }
...
const runExperiment = () => {
  ...
  for (
    ...
    while (!match.finished && !accepted) {
        ...
        for (const observer of possibleObservers) {
          ...
          for (const actorOffer of actorOffers) {
            ...
            state = {
              ...
              acceptanceSnapshot: observerSnapshot,
      ...
      for (const arm of ['H', 'I', 'C'] as const) {
        ...
        gazeInvalid += result.gazeInvalid;
        nonNormalisedGaze += result.nonNormalisedGaze;
        recomputeMismatches += result.recomputeMismatches;
      }
      const h = arms.get('H')!;
      const i = arms.get('I')!;
      const c = arms.get('C')!;
  ...
  return {
    experiment: 'D-PROC-1G',
    authority: 'MOTION-GATED-INTENT-REOPENING',
    parameters: {
...
console.error(
  `D-PROC-1G ${output.verdict} · accepted ${output.support.acceptedStates}/${REQUIRED_STATES}`
  + ` · completed ${output.support.completed} · ordered ${output.counts.orderedFingerprints}`
  + ` · held ${output.counts.hFingerprints} · SHA ${sha256}`,
);
