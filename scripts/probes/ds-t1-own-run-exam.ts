 ...
 * (docs/world-model/DS-T1-OWN-RUN-EXAM.md).
 *
 * Authorized by COMMANDER RULING #406 item 5. Lineage: DS-C0 (the WALKER this exam inherits —
 * populations A–D, the hat-class classifier off `p.action.scores[0].why`, the hat episodes, the
 * yield joins, the wall/overlap faces, the code-fact extractor, the estimator, the hash order)
 * → DS-T0 (the SEAM this exam examines, X-SRC-ZERO: `dsOwnRun` + `dsHatsOff`) → GK-T1 (the EXAM
 * FORM: paired arms on shared seeds, a primary ruler with a paired Δ and a resolved word, the
 * guards in the NI_FRACTION tolerance form, the #157 FLAG-form offsides guard, G-BITE in the
 * #402 item 2(iii) form, gLockstep, LOO scoped, two-fractions, frozen literals on stored
 * booleans) → LN-T1 (the OBM DOSE: the byte-copied MARKER-ESCAPE matrix, G-DOSE-COPY's second
 * derivation straight off the `OBM_*` exports, the crowding family).
 *
 * THE QUESTION (#406 item 5, not re-argued here): can the coach's open-play hat come off — does
 * the player's OWN priced run hold the match's band without flooding, with or without the
 * off-ball eyes? The DF path (M-DF.2): the cap retires by measurement, never by deletion.
 *
 * ⛔ THIS IS AN EXAM. It arms NOTHING in the game; NOTHING ships. The READ SENTENCES are FROZEN
 * LITERALS selected by STORED booleans, and NO VERDICT WORD is printed on any yield or coupling
 * face — the reads are the only verdict sentences.
 ...
 *
 * ⭐⭐ THE THREE DS-C0 DEBTS (#405 item 1), PAID HERE:
 *   (a) the decision-tick predicate reads `pcLatency`'s own holds map AFTER `m.step(DT)` for the
 *       tick just walked (the engine's `pcLatencyObserve` arms holds INSIDE the step, before the
 *       decide loop); DS-C0's PRE-STEP form is recomputed BESIDE it and the delta is stored
 *       against `pcLatency.ledger.decisionsHeld`'s own per-tick delta as `decisionTickCalibration`
 *       — canon: engine ledgers before heuristics.
 *   (b) the shooter gid is recorded AT THE SHOT'S PUSH (`pendingShot.shooterGid` while the shot is
 *       live) into a per-`logIndex` map, so the goal join — and `goalsPerEpisode` — is no longer
 *       VOID at the outcome flip.
 *   (c) the episode-tick bins run PAST one full `wallRun` licence (the licence is 2.3 s, DERIVED
 *       into ticks as `round(2.3 / DT)`, never typed), the TOP BIN'S SHARE is stored beside every
 *       bin-derived median, and episodes still ACTIVE at full time are counted in a stored field.
 */
...
import { RUN_ROLE_W, RUN_DEPTH_DIV, RUN_PRIOR_MAX } from '../../src/ai/TeamBrain';
...
/* ========================================================================== */
const ENV_WHITELIST = ['DST1_MODE', 'DST1_N', 'DST1_OUT'] as const;
...
const rogueOwn = Object.keys(process.env)
  .filter((k) => k.startsWith('DST1_') && !(ENV_WHITELIST as readonly string[]).includes(k));
const rogueEngine = ENGINE_DOORS.filter((k) => process.env[k] !== undefined);
if (rogueOwn.length > 0 || rogueEngine.length > 0) {
  banner(`DS-T1 FATAL — unrecognised env: ${[...rogueOwn, ...rogueEngine].join(', ')}`);
  process.exit(3);
}
const MODE = process.env.DST1_MODE as 'smoke' | 'full' | undefined;
if (MODE !== undefined && MODE !== 'smoke' && MODE !== 'full') {
  banner('DS-T1 FATAL — DST1_MODE must be smoke|full'); process.exit(3);
}
const N_ENV = process.env.DST1_N !== undefined ? Number(process.env.DST1_N) : undefined;
if (N_ENV !== undefined && (!Number.isInteger(N_ENV) || N_ENV < 1)) {
  banner('DS-T1 FATAL — DST1_N must be a positive integer'); process.exit(3);
}
const OUT_ENV = process.env.DST1_OUT;
const OVERRIDE_REASONS = [
  ...(MODE !== undefined ? [`DST1_MODE=${MODE}`] : []),
  ...(N_ENV !== undefined ? [`DST1_N=${N_ENV}`] : []),
  ...(OUT_ENV !== undefined ? [`DST1_OUT=${OUT_ENV}`] : []),
];
const IS_OVERRIDE = OVERRIDE_REASONS.length > 0;
const CANONICAL_OUT = 'docs/world-model/data/ds-t1-own-run-exam.json';
const CANONICAL_DIR_ABS = pathResolve('docs/world-model/data');
const OUT_BASE = OUT_ENV ?? (IS_OVERRIDE ? '/tmp/ds-t1-override.json' : CANONICAL_OUT);
const isCanonical = (p: string): boolean => pathResolve(p).startsWith(CANONICAL_DIR_ABS);
if (IS_OVERRIDE && isCanonical(OUT_BASE)) {
  banner('DS-T1 FATAL — an override run may never write the canonical artifact path');
  process.exit(3);
}
/** ⭐⭐ THE INSTRUMENT OF RECORD — this file's own path, for the stage block's hash. */
const INSTRUMENT_PATH = 'scripts/probes/ds-t1-own-run-exam.ts';
...
anchor('⭐⭐⭐ THE SCORE — `W.runScore · prior`, the body\'s OWN evolved run weight', BRAIN_PATH,
  '        let s = W.runScore * prior;', 1);
...
anchor('⭐⭐⭐ DOSE COPY — MARKER-ESCAPE, BYTE FOR BYTE (the dose of record)', OBMT1_PATH,
...
/* ========================================================================== */
const BLOCK_BASE = 12_554_000;
const BLOCK_TOP = 12_554_999;
/** ⭐⭐ N_FROZEN = 999 — the block's OWN AFFORDANCE after the construction receipt at
 *  12,554,999 (battery seeds 12,554,000–12,554,998). §DEV-PREFLIGHT's sizing rows are computed
...
const SCRATCH_BASE = 900_006_200;
...
/** ⭐⭐ G-REPRO-DSC0's RE-WALK band — DS-C0's OWN CONSUMED block. NOT a consumption. */
const REPRO_SEEDS = Array.from({ length: 12 }, (_, i) => 12_553_000 + i);

/* ========================================================================== */
/* §6 THE ARMS — NINE, PAIRED on shared seeds; the composer CALLED, never copied              */
/* ========================================================================== */
const ARMS = [
  'HATS-E13-ABSENT', 'HATS-E13-DOSED',
  'HATSOWN-E13-ABSENT', 'HATSOWN-E13-DOSED',
  'OWN-E13-ABSENT', 'OWN-E13-DOSED',
  'HATS-D13', 'HATSOWN-D13', 'OWN-D13',
] as const;
type Arm = (typeof ARMS)[number];
/** ⭐⭐⭐ THE ARM OF RECORD for the reads (#406 item 5(i)). */
...
const ARM_LABEL: Record<Arm, string> = {
  'HATS-E13-ABSENT': 'world 13 EMPTY-BOOK, neither DS flag, OBM seat ABSENT — THE SHIPPED PATH '
    + 'and ③\'s control (DS-C0\'s E13, byte for byte)',
  'HATS-E13-DOSED': 'the same, OBM seat DOSED at MARKER-ESCAPE',
  'HATSOWN-E13-ABSENT': 'world 13 EMPTY-BOOK + `dsOwnRun` — THE ADDITIVE FORM, seat ABSENT',
  'HATSOWN-E13-DOSED': 'the additive form, seat DOSED at MARKER-ESCAPE',
  'OWN-E13-ABSENT': '⭐⭐⭐ world 13 EMPTY-BOOK + `dsOwnRun` + `dsHatsOff` — OWN RUN ALONE, seat '
    + 'ABSENT. THE ARM OF RECORD.',
  'OWN-E13-DOSED': 'OWN RUN ALONE, seat DOSED at MARKER-ESCAPE — H-DS-1 / H-DS-2\'s arm',
...
const ARM_KIND: Record<Arm, ArmFlagKind> = {
  'HATS-E13-ABSENT': 'HATS', 'HATS-E13-DOSED': 'HATS',
  'HATSOWN-E13-ABSENT': 'HATSOWN', 'HATSOWN-E13-DOSED': 'HATSOWN',
  'OWN-E13-ABSENT': 'OWN', 'OWN-E13-DOSED': 'OWN',
  'HATS-D13': 'HATS', 'HATSOWN-D13': 'HATSOWN', 'OWN-D13': 'OWN',
};
const ARM_DOSED: Record<Arm, boolean> = {
  'HATS-E13-ABSENT': false, 'HATS-E13-DOSED': true,
  'HATSOWN-E13-ABSENT': false, 'HATSOWN-E13-DOSED': true,
  'OWN-E13-ABSENT': false, 'OWN-E13-DOSED': true,
  'HATS-D13': false, 'HATSOWN-D13': false, 'OWN-D13': false,
};
const ARM_BOOK: Record<Arm, 'E13' | 'D13'> = {
  'HATS-E13-ABSENT': 'E13', 'HATS-E13-DOSED': 'E13',
  'HATSOWN-E13-ABSENT': 'E13', 'HATSOWN-E13-DOSED': 'E13',
  'OWN-E13-ABSENT': 'E13', 'OWN-E13-DOSED': 'E13',
  'HATS-D13': 'D13', 'HATSOWN-D13': 'D13', 'OWN-D13': 'D13',
};
const E13_ARMS = ARMS.filter((a) => ARM_BOOK[a] === 'E13');
const DOSED_ARMS = ARMS.filter((a) => ARM_DOSED[a]);
/** the R1 / band PAIRINGS: every arm is compared to the HATS arm at the SAME seat state and
 *  the SAME book (#406 item 5(ii): "paired Δ of the mean vs HATS (same seat state)"). */
const CONTROL_OF: Record<Arm, Arm> = {
  'HATS-E13-ABSENT': 'HATS-E13-ABSENT', 'HATS-E13-DOSED': 'HATS-E13-DOSED',
  'HATSOWN-E13-ABSENT': 'HATS-E13-ABSENT', 'HATSOWN-E13-DOSED': 'HATS-E13-DOSED',
  'OWN-E13-ABSENT': 'HATS-E13-ABSENT', 'OWN-E13-DOSED': 'HATS-E13-DOSED',
  'HATS-D13': 'HATS-D13', 'HATSOWN-D13': 'HATS-D13', 'OWN-D13': 'HATS-D13',
};
...
if (L3_DOSE_BYTES_SHA !== L3_DOSE_PIN || PC_DOSE_BYTES_SHA !== PC_DOSE_PIN) {
  banner('DS-T1 FATAL — a dose file\'s BYTES do not match the pinned value');
...
if (!D13_REACHABLE) {
  banner(`DS-T1 FATAL — the D13 arms are not reachable: ${DOSE_LOAD_ERROR ?? 'empty dose'}`);
  process.exit(3);
}
...
/** ⭐⭐⭐ MARKER-ESCAPE — OBM-T1's f2-driven corner, byte for byte. */
const MARKER_ESCAPE = matrix([O_DEPTH, F2, MAX], [O_WIDTH, F2, MAX]);
/** ⭐⭐ G-DOSE-COPY's SECOND, INDEPENDENTLY SHAPED DERIVATION, straight off the `OBM_*` exports:
 *  a full (output × feature) sweep filling each slot from a rule, compared slot for slot against
 *  the byte-copied matrix above. A DIFFERENT SHAPE over the same convention. */
const markerEscapeFromExports = (): number[] => {
  const nF = OBM_FEATURE_KEYS.length;
  const nO = OBM_OUTPUT_KEYS.length;
  const w = new Array<number>(nO * nF).fill(0);
  for (let o = 0; o < nO; o++) {
    for (let f = 0; f < nF; f++) {
      w[o * nF + f] = (f === 1 && (o === 0 || o === 1)) ? OBM_WEIGHT_MAX : 0;
    }
  }
  return w;
};
const DOSE_REDERIVED = markerEscapeFromExports();
const DOSE_COPY_SLOTS = MARKER_ESCAPE.map((v, i) => ({
  slot: i,
  output: OBM_OUTPUT_KEYS[Math.floor(i / OBM_FEATURE_KEYS.length)],
  feature: OBM_FEATURE_KEYS[i % OBM_FEATURE_KEYS.length],
  copied: v, rederived: DOSE_REDERIVED[i], same: v === DOSE_REDERIVED[i],
}));
const DOSE_COPY_OK = MARKER_ESCAPE.length === OBM_WEIGHT_SLOTS
  && DOSE_REDERIVED.length === OBM_WEIGHT_SLOTS
  && DOSE_COPY_SLOTS.every((s) => s.same)
  && MARKER_ESCAPE.every((v) => v === 0 || v === OBM_WEIGHT_MIN || v === OBM_WEIGHT_MAX)
  && MARKER_ESCAPE.filter((v) => v !== 0).length === 2;
 ...
 *  MARKER-ESCAPE matrix on the two MATCH-LOCAL genome views, and (on D13) the SHIPPED loaders'
...
const buildMatch = (seed: number, arm: Arm): Match => {
  ...
  if (ARM_DOSED[arm]) armMatrixLocal(m, MARKER_ESCAPE);
  return m;
};
...
fx('literals.theDoseIsTWOSlotsAtTheDomainMAX',
  MARKER_ESCAPE.map((v, i) => (v !== 0 ? i : -1)).filter((i) => i >= 0),
  [IDX(O_DEPTH, F2), IDX(O_WIDTH, F2)]);
fx('literals.theUnusedDoseIndicesAreNAMED', [F1, F3, F4, O_SUPPORT, O_RUN], [0, 2, 3, 2, 3]);
...
const norm = (a: { file: string; line: number }[]): string => JSON.stringify(
  a.map((x) => `${x.file}:${x.line}`).sort(),
);
...
const byFileName = (rows: { file: string; own: number; hats: number }[]) => JSON.stringify(
  rows.map((r) => [r.file.split('/').pop(), r.own, r.hats])
    .sort((a, b) => (String(a[0]) < String(b[0]) ? -1 : 1)),
);
const FLAG_COUNTS_AGREE = DOC_FLAG_COUNTS.length === 4
  && byFileName(FLAG_COUNTS) === byFileName(DOC_FLAG_COUNTS);
...
fx('readForks.theSeamDocPerFileCountsPARSED', DOC_FLAG_COUNTS.length, 4);
fx('readForks.theyAGREE', FORK_INVENTORY_AGREES && FLAG_COUNTS_AGREE, true);
...
const CODE_FACT_GRAPH_OK = ROOTS_COMPLETE && EVERY_FIELD_SITE_RESOLVED
...
/* ========================================================================== */
banner('DS-T1 — the lockstep receipt (observed vs unobserved, PER ARM)');
...
const cells: Cell[] = [];
banner(`DS-T1 — the battery: ${N} seeds × ${ARMS.length} arms, seeds `
...
/* ========================================================================== */
const reproDetail = (() => {
  if (!existsSync(DSC0_ARTIFACT)) {
    return { ran: false, acOk: false, bdComparable: false,
      seeds: REPRO_SEEDS, comparedAcFields: [] as string[], comparedBdFields: [] as string[],
      rows: [] as { seed: number; acMismatches: string[]; bdMismatches: string[];
        bdDelta: Record<string, [unknown, unknown]> }[],
      note: `the DS-C0 artifact is absent at ${DSC0_ARTIFACT}` };
  }
  const dsc0 = JSON.parse(readFileSync(DSC0_ARTIFACT, 'utf8')) as {
    perSeedCells: { seed: number; E13: Record<string, unknown> }[];
  };
  const bySeed = new Map(dsc0.perSeedCells.map((c) => [c.seed, c.E13]));
  const firstTheirs = bySeed.get(REPRO_SEEDS[0]);
  const has = (k: string): boolean => firstTheirs !== undefined
    && Object.prototype.hasOwnProperty.call(firstTheirs, k);
  const acFields = (DSC0_POP_AC_FIELDS as readonly string[]).filter(has);
  const bdFields = (DSC0_POP_BD_FIELDS as readonly string[]).filter(has);
  const rows = REPRO_SEEDS.map((seed) => {
    const theirs = bySeed.get(seed);
    if (theirs === undefined) {
      return { seed, acMismatches: ['ABSENT FROM DS-C0'], bdMismatches: ['ABSENT FROM DS-C0'],
        bdDelta: {} as Record<string, [unknown, unknown]> };
    }
    const mine = walkMatch(buildMatch(seed, 'HATS-E13-ABSENT'), 'HATS-E13-ABSENT', true) as
      unknown as Record<string, unknown>;
    const acBad = acFields.filter(
      (k) => JSON.stringify(mine[k]) !== JSON.stringify(theirs[k]),
    );
    const bdBad = bdFields.filter(
      (k) => JSON.stringify(mine[k]) !== JSON.stringify(theirs[k]),
    );
    const bdDelta: Record<string, [unknown, unknown]> = {};
    for (const k of bdBad) bdDelta[k] = [mine[k], theirs[k]];
    return { seed, acMismatches: acBad, bdMismatches: bdBad, bdDelta };
  });
  return {
    ran: true,
    acOk: acFields.length > 0 && rows.every((r) => r.acMismatches.length === 0),
    bdComparable: bdFields.length > 0,
    seeds: REPRO_SEEDS, comparedAcFields: acFields, comparedBdFields: bdFields, rows,
    note: '⭐⭐⭐ HATS-E13-ABSENT re-walked on DS-C0\'s OWN CONSUMED BAND (12,553,000–011 — NOT a '
      + 'consumption) and compared FIELD FOR FIELD against DS-C0\'s stored `perSeedCells[].E13`. '
      + 'THE A/C HALF (population A — every coach tick in possession — and population C — the '
      + 'hat episodes and their yield, the wall, the overlap, the arriver, the downstream pair) '
      + 'is compared with the PRE-STEP-independent walker and a mismatch is RED. THE B/D HALF '
      + '(the decision-tick populations) is compared with DS-C0\'s OWN PRE-STEP form recomputed '
      + 'beside the new one; the delta between the PRE-STEP and POST-STEP forms is the '
      + 'CALIBRATION RECEIPT (`decisionTickCalibration`), not a failure.',
  };
})();
const REPRO_AC_OK = reproDetail.acOk;
const REPRO_BD_ROWS_CLEAN = reproDetail.rows.every((r) => r.bdMismatches.length === 0);
banner(`  G-REPRO-DSC0 A/C ${REPRO_AC_OK ? 'GREEN' : 'RED'} `
  + `(${reproDetail.comparedAcFields.length} fields × ${reproDetail.rows.length} seeds) · `
  + `B/D pre-step form ${REPRO_BD_ROWS_CLEAN ? 'IDENTICAL' : 'DIFFERS'}`);
...
const face = (k: string, armK: Arm): FaceRow => {
  ...
  if (f === undefined) { banner(`DS-T1 FATAL — unknown face ${k}/${armK}`); process.exit(3); }
  return f as FaceRow;
};
...
const delta = (faceKey: string, armK: Arm): DeltaRow => {
  ...
  if (dd === undefined) { banner(`DS-T1 FATAL — unknown Δ ${faceKey}@${armK}`); process.exit(3); }
...
/** ⭐⭐⭐ THE FROZEN READ LITERALS — copied VERBATIM from COMMANDER RULING #406 item 5(v), and
...
const READ_WORD = readWordFrom(floods('OWN-E13-ABSENT'), holdsBand('OWN-E13-ABSENT'),
  floods('OWN-E13-DOSED'), holdsBand('OWN-E13-DOSED'));
const READ_SENTENCE = READ_LITERALS[READ_WORD];
const BREACH_NAMED = [...breachingGuards('OWN-E13-ABSENT'),
  ...breachingGuards('OWN-E13-DOSED')].join(' · ');
...
const READ_WORD_IF_DOSED_WERE_OF_RECORD = readWordFrom(
  floods('OWN-E13-DOSED'), holdsBand('OWN-E13-DOSED'),
  floods('OWN-E13-DOSED'), holdsBand('OWN-E13-DOSED'),
);
const READ_WORD_D13 = readWordFrom(floods('OWN-D13'), holdsBand('OWN-D13'),
  floods('OWN-D13'), holdsBand('OWN-D13'));
...
const HATSOWN_WORDS = {
  absent: { arm: 'HATSOWN-E13-ABSENT', floods: floods('HATSOWN-E13-ABSENT'),
    holdsBand: holdsBand('HATSOWN-E13-ABSENT'),
    breachingGuards: breachingGuards('HATSOWN-E13-ABSENT') },
  dosed: { arm: 'HATSOWN-E13-DOSED', floods: floods('HATSOWN-E13-DOSED'),
    holdsBand: holdsBand('HATSOWN-E13-DOSED'),
    breachingGuards: breachingGuards('HATSOWN-E13-DOSED') },
};
...
/** ⭐⭐ THE SEAT'S OWN BITE, beside: the DOSED control against the ABSENT control. */
const seatBiteRow = (() => {
  const differing = cells.filter((c) =>
    c.rows['HATS-E13-DOSED'].signature !== c.rows['HATS-E13-ABSENT'].signature);
  return {
    arm: 'HATS-E13-DOSED', controlArm: 'HATS-E13-ABSENT', eligibleSeeds: cells.length,
    seedsDiffering: differing.length, allDiffer: differing.length === cells.length,
    note: 'the OBM seat\'s own liveness: MARKER-ESCAPE against the seat-absent world',
  };
})();
const BITE_OK = biteRows.every((r) => r.allDiffer) && seatBiteRow.seedsDiffering > 0;
...
const POOL_KEYS = ['r1Bins', 'r1RunnersByRole', 'branchTicks', 'runCountBins', 'runnersByRole',
 ...
 *  900,006,200–211 (NINE walks per seed), transcribed here and re-derived off the artifact by
...
const SIZING_INPUTS = [
  { face: 'r1.runsPerInPossessionTick@OWN-E13-ABSENT',
    hwSmoke: 0.07897749214533145, target: 0.05 },
  { face: 'guard.passCompletion@OWN-E13-ABSENT',
    hwSmoke: 0.04279762412355759, target: 0.05 },
];
...
const CONSUMED_BLOCKS = [12_544_000, 12_545_000, 12_546_000, 12_547_000, 12_548_000,
  12_549_000, 12_550_000, 12_551_000, 12_552_000, 12_553_000];
...
const gates: Record<string, { ok: boolean; note: string }> = {
  gWorld: {
    ...
    note: '⭐⭐ PER ARM, on EVERY walked match and the construction receipt: `bqArmedVersion(m) '
      ...
      + '`obmMovement` EXACTLY AS DUE, with the 16-slot MARKER-ESCAPE matrix present on '
  ...
  gDoseCopy: {
    ok: DOSE_COPY_OK,
    note: '⭐⭐⭐ G-DOSE-COPY in LN-T1\'s form: the MARKER-ESCAPE matrix is BYTE-COPIED from '
      + `\`${OBMT1_PATH}\`'s own line (anchored) and RE-DERIVED SLOT FOR SLOT by a SECOND, `
      + 'INDEPENDENTLY SHAPED derivation straight off the `OBM_*` exports (a full output × '
      + `feature sweep). ${OBM_WEIGHT_SLOTS} slots compared, `
      + `${DOSE_COPY_SLOTS.filter((s) => s.same).length} equal; exactly `
      + `${MARKER_ESCAPE.filter((v) => v !== 0).length} non-zero slots, both at the domain `
      + `MAX (${OBM_WEIGHT_MAX}), and every slot is 0 or a domain corner`,
  },
  gDoseSource: {
  ...
  gAnchoredConstants: {
    ...
    note: `⭐⭐ anchored extraction with line receipts, ${ANCHORS.length} sites, EVERY one at `
      ...
      + '/ MIN-MAX aliases / MARKER-ESCAPE line) · THE TOLERANCE (`NI_FRACTION` as an '
  ...
  gClassesNonVacuous: {
    ...
    note: '⛔ NO FALSE UNIVERSAL. The EMPTY classes are ENUMERATED as stored lists, per arm: '
      ...
      + `with a zero count = [${EMPTY_STATE_CLASSES.join(', ') || 'none'}]. The gate asserts `
  ...
  gBite: {
    ...
    note: '⭐⭐⭐ G-BITE in the #402 item 2(iii) form: on every battery seed WHERE THE FLAG CAN '
      ...
      + `own bite: MARKER-ESCAPE vs seat-absent differs on ${seatBiteRow.seedsDiffering}/`
      + `${seatBiteRow.eligibleSeeds} seeds. ⚠ LIVENESS only: a differing signature says the `
      + 'flag fired, never that it helped',
  },
  gRepro: {
    ok: REPRO_AC_OK,
    note: `⭐⭐⭐ G-REPRO-DSC0: the A/C half compared ${reproDetail.comparedAcFields.length} `
      + `fields × ${reproDetail.rows.length} seeds — `
      + `${reproDetail.rows.every((r) => r.acMismatches.length === 0) ? 'ZERO mismatches'
        : `MISMATCHES on ${reproDetail.rows.filter((r) => r.acMismatches.length > 0)
          .map((r) => r.seed).join(', ')}`}. The B/D half compared `
      + `${reproDetail.comparedBdFields.length} fields with DS-C0's OWN PRE-STEP form and is `
      + `${REPRO_BD_ROWS_CLEAN ? 'IDENTICAL' : 'DIFFERENT (the deltas are stored per seed)'}; `
      + 'the PRE-STEP vs POST-STEP delta is published as `decisionTickCalibration` and as the '
      + '`calib.*` faces. ' + reproDetail.note,
  },
  gLockstep: {
    ok: LOCKSTEP_OK,
  ...
  gSeedsBookedEqualWalked: {
    ...
    note: 'BOOKED = WALKED, derived from the CELLS\' OWN distinct seeds: every battery seed and '
      + 'the construction receipt lie inside block 12,554,000–999, each seed is walked ONCE PER '
      ...
      + 'band, the world pin, the lockstep pair which the X-DET pairs re-use, and the fixture '
      + 'attribute draw) is out-of-band and STORED there. ⭐ THE RE-WALKS on 12,553,000–011 are '
      + 'DS-C0\'s OWN CONSUMED BAND and are NOT a consumption — canon, VERBATIM: "verifier '
  ...
  gSeedDisjoint: {
    ok: walkedSeeds.every((s) => s >= BLOCK_BASE) && ALL_SCRATCH.every((s) => s >= 900_000_000)
      ...
      && REPRO_SEEDS.every((s) => s >= 12_553_000 && s <= 12_553_999),
    note: 'SEED-DISJOINT at the frontier of #406 item 8 (next sim ≥ 12,554,000): every battery '
      + 'seed is ≥ 12,554,000 and inside THIS block, disjoint from every consumed block (LN-C0 '
      ...
      + '12,553,000–999), each of which is checked to end BELOW this block\'s base; the '
      + 'G-REPRO re-walks lie INSIDE DS-C0\'s own consumed block; ZERO stats consumed',
  },
  gN: {
...
const artifact: Record<string, unknown> = {
  stage: {
    id: 'DS-T1',
    title: '「自己的前插 · 考试」 THE OWN-RUN EXAM — can the coach\'s open-play hat come off: '
      + 'HATS · HATS + OWN · OWN ALONE × the OBM seat ABSENT / DOSED at MARKER-ESCAPE on world '
      + '13 EMPTY-BOOK (six arms of record), with the played form D13 beside; R1 = EXECUTED '
      + 'runs per in-possession open-play tick (the flood face); the band = ten guards; the '
      + 'faces = DS-C0\'s populations re-walked with its three instrument debts paid, runs and '
      + 'yield PER STATE, runs BY ROLE, the coupling, the crowding family and the seat\'s own '
      + '`runMul` distribution',
    doc: 'docs/world-model/DS-T1-OWN-RUN-EXAM.md',
    ...
    doseOfRecord: 'docs/world-model/LN-T1-LANE-EXAM.md',
    authorizedBy: 'COMMANDER RULING #406 item 5',
    kind: 'EXAM — it arms NOTHING in the game and SHIPS NOTHING. The READ SENTENCES of #406 '
      + 'item 5(v) are FROZEN LITERALS selected by STORED booleans, and NO VERDICT WORD is '
      + 'printed on any yield or coupling face. The commander rules.',
    ...
    theQuestion: 'Can the coach\'s open-play hat come off — does the player\'s OWN priced run '
      + 'hold the match\'s band without flooding, with or without the off-ball eyes? The DF '
      + 'path (M-DF.2): the cap retires by measurement, never by deletion.',
    theThreeDebtsPaid: {
    ...
    honestLimitsNote: '⛔ canon, VERBATIM: "a stage doc\'s HONEST LIMITS list is the ONE home; '
      ...
      + 'The ONE home is docs/world-model/DS-T1-OWN-RUN-EXAM.md §HONEST LIMITS.',
  ...
  arms: Object.fromEntries(ARMS.map((a) => [a, {
    ...
    obmSeatDosed: ARM_DOSED[a], controlArm: CONTROL_OF[a],
  ...
  doseCopy: {
    what: '⭐⭐⭐ G-DOSE-COPY (LN-T1\'s form): MARKER-ESCAPE byte-copied from OBM-T1\'s own line '
      + 'and re-derived slot for slot by a SECOND, independently shaped derivation off the '
      + '`OBM_*` exports.',
    source: OBMT1_PATH,
    copiedMatrix: MARKER_ESCAPE, rederivedFromExports: DOSE_REDERIVED,
    slots: DOSE_COPY_SLOTS, ok: DOSE_COPY_OK,
    nonZeroSlots: MARKER_ESCAPE.filter((v) => v !== 0).length,
  ...
  loo: LOO_ROWS,
  bite: { rows: biteRows, seatBite: seatBiteRow, ok: BITE_OK },
  repro: reproDetail,
  sizing: {
    form: 'se(n) = hw(n)/z.975 · se(needed) = |target|/(z.975+z.80) · N = ceil(n·(se(n)/'
      + 'se(needed))²) · MDE(N) = hw(n)·sqrt(n/N)·(z.975+z.80)/z.975',
    varianceSource: 'THIS exam\'s own 12-cluster SCRATCH SMOKE (seeds 900,006,200–211, NINE '
  ...
  stats: { consumed: 0, nextBase: 117_600, registryOfRecord: 83 },
  perf: {
  ...
  reads: {
    ...
    selectors: SELECTORS,
    annotations: [
      `floods(OWN, seat absent) = ${floods('OWN-E13-ABSENT')} · holdsBand(OWN, seat absent) = `
      + `${holdsBand('OWN-E13-ABSENT')} · floods(OWN, dosed) = ${floods('OWN-E13-DOSED')} · `
      + `holdsBand(OWN, dosed) = ${holdsBand('OWN-E13-DOSED')}`,
    ...
    counterfactuals: {
      ...
      dosedOwnAsIfArmOfRecord: {
        ...
        floods: floods('OWN-E13-DOSED'), holdsBand: holdsBand('OWN-E13-DOSED'),
      },
...
}
{
  ...
  const fa = rd.selectors['OWN-E13-ABSENT'];
  const fd = rd.selectors['OWN-E13-DOSED'];
  ...
  binChecks.push({ bin: 'reads.breachAnnotationIsAStoredField',
    ok: rd.breachNamed === [...disk.guards.table['OWN-E13-ABSENT'].filter((g) => g.breach)
      .map((g) => `${g.key}`), ...disk.guards.table['OWN-E13-DOSED'].filter((g) => g.breach)
      .map((g) => `${g.key}`)].map((k, i) => {
      const all = [...GUARD_TABLE['OWN-E13-ABSENT'].filter((g) => g.breach),
        ...GUARD_TABLE['OWN-E13-DOSED'].filter((g) => g.breach)];
      return `${all[i].id} ${k}`;
    }).join(' · ') });
}
...
banner('');
banner(`DS-T1 — ${ALL_GREEN_FINAL ? 'ALL GATES GREEN' : '⛔ RED — artifact routed to .RED'}`);
...
banner(`  HATS+OWN (absent): floods ${HATSOWN_WORDS.absent.floods} · holdsBand `
  + `${HATSOWN_WORDS.absent.holdsBand}; (dosed): floods ${HATSOWN_WORDS.dosed.floods} · `
  + `holdsBand ${HATSOWN_WORDS.dosed.holdsBand}`);
banner(`  the counterfactual word had the DOSED OWN arm been of record: `
  + `${READ_WORD_IF_DOSED_WERE_OF_RECORD}`);
