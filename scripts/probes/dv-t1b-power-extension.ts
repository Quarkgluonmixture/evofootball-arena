// DV T1b — THE VIRGIN-SEED POWER EXTENSION: is DV-T1's supply limb (limb A) UNRESOLVED because
// the effect is absent, or because 283 clusters could not resolve it? Ruling #251.2's disposition,
// run as a THREE-ARM REDUCTION of DV-T1 on VIRGIN SEEDS at an EX-ANTE-SIZED N.
//
// Doc:      docs/world-model/DV-T1B-POWER-EXTENSION.md (§FORM/§N/§SEEDS/§GATES frozen before sight)
// Charter:  ruling #251.2 — "arms CONTROL · PLANE-ANCHOR · dvTruthP ONLY, configuration
//           byte-identical to DV-T1's (identity gated, and the predicate DERIVED for this exam,
//           not inherited — the #251.3 lesson); N sized EX ANTE from DV-T1's committed artifact
//           moments to resolve the anchor's replicated supply estimate; the frozen letter = the
//           same two limbs on virgin seeds (which simultaneously re-tests the goals recovery).
//           ROUTES, pre-named: supply resolves HELPFUL at parity => the #244 inversion is COMPLETE
//           at exam grain · supply resolves KILLED => F-DV-b true on mechanism after all · still
//           UNRESOLVED at the extension's N => the supply limb is declared below this world's
//           measurable grain at these doses."
// Contract: docs/world-model/DELIVERY-VALUE-CONTRACT.md §3 — the JOINT's two limbs, verbatim.
// Rulings:  #251 (DV-T1 adjudicated: the deflation MOVES on the exposure limb, +0.2438 resolved
//           toward the band at parity; F-DV-b fired on the LETTER and is REFUTED on the
//           MECHANISM — the ANCHOR ITSELF fails limb A on 283 virgin seeds (+0.001344
//           [-0.000076, 0.002758]) while REPLICATING #244's +0.1365 pp point estimate => limb A is
//           POWER-CONFOUNDED) · ⭐ #251.3 (THE PROBE-HYGIENE LESSONS THIS PROBE IS BUILT AGAINST:
//           NEVER inherit another exam's configuration predicate without re-deriving it — a dead
//           conjunct inside a PASS gate is a defect one severity above a stale note; MODE-CONDITION
//           every caveat literal, because a battery must not print smoke-grain disclaimers) ·
//           #250.3 (machine timings ride the UNHASHED envelope so `resultSha256` re-derives;
//           percentage grep-forms FORMATTED before String()) · #250.4 (H-250a, the delivered-rate
//           publication, the no-cost-% rule) · #249 (DV-C0 — THE TRUE TABLE, the belief dose) ·
//           #247 (TRUTH vs BELIEF: the census table is the INSTRUMENT's and is DOSED, never wired
//           into src) · #244 (THE FAILURE THE ARC INVERTS) · #225.3(c) · #207 (checkpoint) ·
//           #203 (ROWS, never verdicts — this probe PRINTS the route and RESOLVES nothing) ·
//           #197-M1/#198 · #181.2 · #163 · #20 · #128.
//
// ⭐ INSTRUMENT-ONLY ROUND. src/** is byte-untouched (X-SRC-UNTOUCHED is a HARD gate). The three
// arms are built by exactly the constructors DV-T1 used, from THIS file's own DOOR / GENE / DOSE /
// DV tables — and the CONFIGURATION IDENTITY with DV-T1 is not asserted from those tables, it is
// EARNED: ⭐⭐ G-REPRO-T1 re-walks the first rows of DV-T1's OWN COMMITTED BATTERY BLOCK on ALL
// THREE of this exam's arms, through this exam's own arm constructors, and must reproduce every
// published field EXACTLY — whole-match signature (rng stream state inside) and the
// delivered-strike columns included. A configuration that differed anywhere could not do that.
// Beside it a DERIVED (never inherited) configuration-identity predicate is published WITH ITS
// OWN LIVENESS PROOF: the same pure predicate is re-evaluated against MUTATED configuration
// tables and must come back FALSE, so it cannot be constant-true by construction (#251.3).
//
// ⭐⭐ N IS SIZED EX ANTE, FROM DV-T1'S COMMITTED ARTIFACT MOMENTS, BEFORE ANY BATTERY RAN:
// the anchor's replicated supply estimate (+0.001344, CI [-0.000076, 0.002758] at 283 clusters) is
// read from the committed artifact; its bootstrap SE is recovered from the published half-width;
// N is the cluster count at which that SE gives 80 % power to resolve THAT SAME point estimate at
// 95 % two-sided. See `powerRule` below and §N in the doc. G-N machine-checks the frozen literal
// against the derivation, and publishes the MDE actually bought.
//
...
// numbers — the same two derivations DV-T1 ran, re-run rather than copied (G-DOSE).
//
//   DVT1B_MODE=smoke|full    (default smoke: 8 seeds @ 12,431,000)
//   DVT1B_RESUME=1           full mode only — restore finished (pass, seed) units (#207)
//   DVT1B_CHECKPOINT=<path>  /tmp scratch; never committed, never read by a gate
//   DVT1B_N=<n> / DVT1B_SKIP_FP=1 — OVERRIDES: routed onto the EXIT-SEMANTICS GUARD BLOCK, turn
...
const MODE = (process.env.DVT1B_MODE ?? 'smoke') === 'full' ? 'full' : 'smoke';
...
const DOSE_READ_SEED = 12_431_010;
...
const STRIKE_READ_SEED = 12_431_015;
const GUARD_BLOCK: readonly [number, number] = [12_431_050, 12_431_099];
const BATTERY_BASE = 12_431_100;
/** ⭐ THE DISPATCH CAP (ruling #251.2: "battery cap 900") — a CEILING, not a target. If the
 *  ex-ante power rule asks for more than the cap, the cap is run and the MDE actually bought is
 *  DECLARED (`mdeBoughtAtNStar`), never quietly re-cut. `capBinds` is published either way. */
const N_CAP = 900;
...
/** ⭐⭐ NEW, AND THIS STAGE'S IDENTITY OF RECORD — G-REPRO-T1: the first rows of DV-T1's OWN
 *  COMMITTED BATTERY BLOCK, re-walked on ALL THREE of this exam's arms THROUGH THIS EXAM'S OWN
 *  ARM CONSTRUCTORS (`matchOf` reading this file's DOOR / GENE / DOSE / DV tables). Every
 *  published field must reproduce EXACTLY — the whole-match signature (rng stream state inside)
 *  and the delivered-strike columns included. This is what makes "configuration byte-identical to
 *  DV-T1's" (#251.2) an EARNED fact rather than an inherited assertion: a door, a gene, a dose or
...
const CONSUMED: readonly { name: string; range: readonly [number, number] }[] = [
   ...
   *  battery block can never run past the ledger room ruling #251.2 granted (12,431,000–899). */
...
/** §4.2 the stats stream — a SEPARATE namespace. DLC-T1s's base was 105,800 and DV-T1's was
 *  106,200 (its paired cluster bootstrap really was drawn); the floor for THIS stage is 106,600,
 *  which clears the #163 200-gap against every published base by 400. The list is DLC-T1s's
 *  COMPLETE published ledger + 105,800 (DLC-T1s's own) + 106,200 (DV-T1's own). */
const BOOTSTRAP_SEED = 106_600;
...
const PUBLISHED_STATS_BASES = [
  ...
  /** ⭐ DV-T1's own base (#251) — the stage this one extends. */
  106_200,
];
 ...
 *  per-match rows on the three arms this stage keeps). */
...
const N_ENV = process.env.DVT1B_N ? Math.max(1, Number.parseInt(process.env.DVT1B_N, 10)) : null;
const SKIP_FP = process.env.DVT1B_SKIP_FP === '1';
const OVERRIDDEN = N_ENV !== null || SKIP_FP;
const OUT_PATH = OVERRIDDEN
  ? '/tmp/dv-t1b-guard-run.json'
  : (MODE === 'smoke'
    ? 'docs/world-model/data/dv-t1b-power-extension-smoke.json'
    : 'docs/world-model/data/dv-t1b-power-extension.json');
...
/* ========================================================================== */
/** ⭐⭐ EXACTLY THREE ARMS (ruling #251.2, frozen): CONTROL · PLANE-ANCHOR · dvTruthP. DV-T1's
 *  other five (`dvInert`, `dvTruth0`, `dvTruthG`, `dvLoud`, `dvTruthPxCas`) are DROPPED — this is
 *  a POWER extension, not a re-run: every seed spent on a rung that is not the frozen letter's is
 *  a seed not spent on resolving the anchor's supply estimate. The three that remain are the two
 *  references the letter is read against plus the one dose the letter is read AT. */
type ArmName = 'absent' | 'planeAnchor' | 'dvTruthP';
const ARMS: readonly ArmName[] = ['absent', 'planeAnchor', 'dvTruthP'];
...
const ARM_SENTENCE: Record<ArmName, string> = {
  absent: 'the CONTROL — the percept-armed world with EVERY seat unreached. DLC-T1s\'s own '
    + 'ABSENT arm, world for world, and DV-T1\'s own ABSENT arm on virgin seeds.',
  planeAnchor: '⭐⭐ THE CONTRAST ANCHOR AND THE OBJECT OF THIS EXTENSION: ruling #244\'s own '
...
/**
 * ⭐⭐ THE N RULE OF THIS STAGE IS A POWER RULE, AND IT IS SIZED EX ANTE — BEFORE ANY BATTERY,
 * FROM DV-T1's COMMITTED ARTIFACT MOMENTS AND NOTHING ELSE.
 *
 * WHY IT IS NOT DV-T1's MDE RULE. DV-T1 sized itself off an INHERITED out-of-world MDE (the O2-T1
 * paired delta on `trueContextShare`) because no same-world paired delta existed. One does now:
 * DV-T1's OWN battery published the quantity this extension exists to resolve —
 *
 *     the PLANE-ANCHOR arm's supply delta vs the control:
 *     point +0.001344, 95 % paired-cluster CI [-0.000076, 0.002758], at 283 clusters
 *
 * — which REPLICATED ruling #244's +0.1365 pp point estimate almost exactly while failing to
 * exclude zero. Ruling #251.2 therefore orders N sized "to resolve the anchor's replicated supply
 * estimate", and that is a POWER question with a published variance, not an MDE guess.
 *
 * THE ARITHMETIC, in four steps, every input READ from the committed artifact:
 *   1. SE at the source N: the published paired-delta CI is a 2.5/97.5 percentile interval, so
 *      `se283 = ((upper - lower) / 2) / z.975`.
 *   2. The SE required to resolve the SAME point estimate with 80 % power at 95 % two-sided:
 *      `seRequired = |point| / (z.975 + z.80)` — the standard power identity, the same
 *      `(z.975 + z.80)` this family's N rule has always used.
 *   3. The paired cluster bootstrap's SE scales as `1/sqrt(N)` in the number of CLUSTERS (#20:
 *      cluster = match seed), so `N = ceil( nSource · (se283 / seRequired)^2 )`.
 *   4. `N* = min(N, cap, ledger room)`; when the cap binds, the run declares the MDE it ACTUALLY
 ...
 * `trueHoldableShare` is DV-T1's. It is the same world, the same rulers, the same three arm
 * configurations and the same estimator — but they are DIFFERENT SEEDS, so the assumption is an
 * assumption. It is the honest one available ex ante, and the realised half-width is published
 * beside the ex-ante one in the result half so the reader can see whether it held.
 *
 * ⚠ AND WHAT IT DOES NOT CLAIM: resolving an estimate is not the same as the estimate being real.
 * If the extension's CI still straddles zero at N*, that is route (c) of #251.2 — the supply limb
 * declared below this world's measurable grain at these doses — and it is the COMMANDER's to rule.
 */
const N_FROZEN = 643;
const powerRule = (() => {
  if (DVT1 === null) {
    return { available: false, note: `absent: ${DVT1_PATH}`, nStar: null as number | null };
  }
  const j = DVT1.j;
  const src = j.preRegisteredPrimary.allArms.planeAnchor.supplyLimb as {
    delta: number; ci: [number, number];
  };
  const nSource = j.seeds as number;
  const point = Math.abs(src.delta);
  const [lo, hi] = src.ci;
  const halfWidth = (hi - lo) / 2;
  const seSource = halfWidth / Z975;
  const seRequired = point / (Z975 + Z80);
  const nRaw = Math.ceil(nSource * ((seSource / seRequired) ** 2));
  const ceiling = Math.min(N_CAP, BATTERY_ROOM);
  const nStar = Math.min(nRaw, ceiling);
  const seAt = (n: number): number => seSource * Math.sqrt(nSource / Math.max(1, n));
  const mdeAt = (n: number): number => (Z975 + Z80) * seAt(n);
  return {
    available: true,
    rule: 'se_source = (CI half-width)/z.975 at the SOURCE cluster count ; se_required = '
      + '|point| / (z.975 + z.80) [80 % power, 95 % two-sided] ; N = ceil(nSource · '
      + '(se_source/se_required)^2) ; N* = min(N, cap, ledger room) — and when a ceiling binds, '
      + 'the MDE ACTUALLY BOUGHT is declared rather than the MDE asked for',
    source: {
      path: DVT1_PATH,
      sha256: sha(DVT1.bytes.toString('utf8')),
      resultSha: j.resultSha256,
      field: 'preRegisteredPrimary.allArms.planeAnchor.supplyLimb (trueHoldableShare, ARM − control)',
      clusters: nSource,
      point: src.delta,
      ci: src.ci,
    },
    targetEstimate: point,
    targetProvenance: '⭐⭐ THE ANCHOR\'s OWN REPLICATED SUPPLY ESTIMATE — ruling #251.2\'s named '
      + 'target. It is DV-T1\'s committed reading (+0.001344 = +0.1344 pp), which replicated '
      + 'ruling #244\'s +0.1365 pp on 283 virgin seeds while its CI touched zero. Sizing to the '
      + 'REPLICATED point estimate (rather than to a taste MDE) is what makes this extension a '
      + 'POWER test of the #244 effect and not a new hypothesis.',
    halfWidthSource: round(halfWidth, 9),
    seSource: round(seSource, 9),
    ...
    mdeAskedFor: round(point, 9),
    expectedHalfWidthAtNStar: round(Z975 * seAt(nStar), 9),
    capForkNote: nRaw > Math.min(N_CAP, BATTERY_ROOM)
      ? '⚠ A CEILING BINDS: the power rule asks for more clusters than the dispatch cap / ledger '
        + 'room allows. The cap is RUN and `mdeBoughtAtNStar` is the effect size this battery can '
        + 'actually resolve at 80 % power — DECLARED, not re-cut (#251.2).'
      : 'no ceiling binds at this reading: the power rule\'s own N is run',
    batteryBlock: `${BATTERY_BASE}..${BATTERY_BASE + nStar - 1}`,
    primaryRulers: 'ruler 1 (TRUE-holdable supply) is the column the N is cut on, because it is '
      + 'the column the frozen letter\'s limb A reads. Ruler 2 and the rest ride published with '
      + 'their CIs and size nothing.',
    costNote: '⭐ BUDGET IT (#228.4): an ARMED, percept-armed battery costs ≈1.4× the wall of a '
      + 'CTB-T1-shaped one — but this stage walks THREE arms per seed instead of eight, so the '
      + 'wall per seed is roughly three eighths of DV-T1\'s.',
  };
})();
/** the emitted name the rest of the file uses, so the artifact key and the sizing object cannot
 *  drift apart. */
const nRule = powerRule;
...
if (MODE === 'full' && RUN_N <= 0) {
  console.error(`DV-T1b FATAL — full mode needs DV-T1's committed artifact for the EX-ANTE power `
    + `rule (${DVT1_PATH}).`);
  process.exit(2);
}
...
banner('');
banner('=============================================================================');
banner(`DV-T1b THE VIRGIN-SEED POWER EXTENSION (#251.2) · mode ${MODE} · N ${RUN_N} seeds × ${ARMS.length} arms`);
banner(`seeds ${RUN_BASE}..${RUN_BASE + RUN_N - 1} · world = PERCEPT-ARMED (edsPerceivedChoice)`);
banner('arms: CONTROL · PLANE-ANCHOR · dvTruthP — differing by EXACTLY the RISK PRICE');
banner(`EX-ANTE power rule ⇒ N* ${String(nRule.nStar)} (frozen literal ${N_FROZEN}, ledger room `
  + `${BATTERY_ROOM}, cap ${N_CAP})`);
if (OVERRIDDEN) {
  banner('⚠ OVERRIDE IN FORCE (DVT1B_N / DVT1B_SKIP_FP) — routed onto the EXIT-SEMANTICS GUARD');
...
/* ========================================================================== */
const CKPT_PATH = process.env.DVT1B_CHECKPOINT ?? '/tmp/dv-t1b-checkpoint.jsonl';
const RESUME = process.env.DVT1B_RESUME === '1';
const CHECKPOINTING = MODE === 'full';
const PROBE_SELF_PATH = 'scripts/probes/dv-t1b-power-extension.ts';
...
const refuse = (why: string): never => {
  console.error(`DV-T1b FATAL — REFUSING TO RESUME: ${why}`);
  ...
  return {
    ...
    gReproT1: {
      ...
      note: '⭐⭐ THE IDENTITY OF RECORD (#251.2 / the #251.3 lesson). Ruling #251.2 requires this '
        + 'extension\'s three arms to be CONFIGURATION-BYTE-IDENTICAL to DV-T1\'s, and #251.3 '
        + 'forbids inheriting another exam\'s configuration predicate. So the identity is EARNED '
        + 'rather than asserted: each of THIS exam\'s three arms re-walks the first rows of '
...
const fpObserved = SKIP_FP ? 'skipped (DVT1B_SKIP_FP)' : leagueHash(FINGERPRINT_SEED);
...
const walkedBlocksRaw: { name: string; first: number; last: number; kind: BlockKind }[] = [
  ...
  {
    name: '⭐⭐ reproDvt1 (re-walk — G-REPRO-T1: DV-T1\'s BATTERY block on ALL THREE of this '
      + 'exam\'s own arms; the IDENTITY OF RECORD)',
 ...
 *       DV state per arm. It is written against THIS exam's three arms and THIS exam's declared
 ...
 * three of these arms — is what proves the configuration is DV-T1's. The predicate is the cheap
...
type ConfigTable = Record<ArmName, ArmConfig>;
 ...
 *  THIS exam's three arms; none of them mentions a door, an arm or a dose this exam does not
 *  have. */
const configIdentityOf = (t: ConfigTable) => {
  const c = t.absent; const an = t.planeAnchor; const dv = t.dvTruthP;
  const limbs = {
    /** the CONTROL: no delivery door, no gene, no matrix, no price. */
    controlIsBare: c.door === 'none' && c.gene === null && c.obm === null && c.dv === null,
    /** the ANCHOR: the banked STRIKE PLANE alone, gene at the domain max, and NO risk price —
     *  which is exactly what makes it ruling #244's arm. */
    anchorIsPlaneAlone: an.door === 'sp' && an.gene === PTP_GENE_MAX && an.obm === null
      && an.dv === null,
    /** the DOSED arm: the SAME plane at the SAME gene — so the only thing that varies between it
     *  and the anchor is the price — plus the DV state at the declared dose. */
    dosedRidesTheSamePlane: dv.door === an.door && dv.gene === an.gene && dv.obm === null,
    dosedCarriesThePrice: dv.dv !== null,
    /** the BELIEF is the census vector this probe READ at exam time (never a literal). */
    beliefIsTheCensusVector: dv.dv !== null && sameVector(dv.dv.belief, TRUTH_BELIEF),
    /** the EXPOSURE is the PARITY rung, which G-DOSE separately proves equals its derivation. */
    exposureIsTheParityRung: dv.dv !== null && dv.dv.exposure === EXPOSURE_LADDER.parity,
    /** NO OBM matrix anywhere: the relational pair is DV-T1's and is not re-asked here. */
    noArmDosesTheMatrix: (Object.keys(t) as ArmName[]).every((a) => t[a].obm === null),
  };
  return { limbs, pass: Object.values(limbs).every((v) => v === true) };
};
const configIdentity = (() => {
  const real = configIdentityOf(REAL_CONFIG);
  const mutate = (name: string, f: (t: ConfigTable) => void) => {
    const t = JSON.parse(JSON.stringify(REAL_CONFIG)) as ConfigTable;
    f(t);
    const r = configIdentityOf(t);
    return { mutant: name, pass: r.pass, brokeThePredicate: r.pass === false, limbs: r.limbs };
  };
  const mutants = [
    mutate('anchor door flipped to the retired dial', (t) => { t.planeAnchor.door = 'ptp'; }),
    mutate('dosed arm\'s plane gene halved', (t) => { t.dvTruthP.gene = PTP_GENE_HALF; }),
    mutate('exposure moved off the PARITY rung', (t) => {
      if (t.dvTruthP.dv !== null) {
        (t.dvTruthP.dv as { exposure: number }).exposure = EXPOSURE_LADDER.gradient;
      }
    }),
    mutate('belief vector perturbed off the census table', (t) => {
      if (t.dvTruthP.dv !== null) {
  ...
  return {
    pass: real.pass && mutants.every((m) => m.brokeThePredicate),
    realTablePasses: real.pass,
    limbs: real.limbs,
    livenessProof: mutants,
    livenessNote: '⭐⭐ THE PREDICATE IS PROVED LIVE, not assumed live (#251.3): the SAME pure '
      + 'function is re-evaluated on four MUTATED configuration tables — a flipped door, a halved '
      + 'gene, an exposure off the parity rung, a belief perturbed by 1e-6 off the census vector — '
      + 'and every one of them must FLIP IT FALSE. A conjunct that cannot be broken by breaking '
      + 'the configuration is not reading the configuration, and a dead conjunct inside a PASS '
      + 'gate is exactly the defect this stage inherited a lesson about.',
    ofRecordNote: '⚠ THIS IS THE CHEAP HALF. The identity OF RECORD — that this configuration is '
      + 'DV-T1\'s — is ⭐⭐ G-REPRO-T1: the re-walk of DV-T1\'s own committed battery rows on all '
      + 'three of these arms, field-exact with the whole-match signature. This predicate proves '
      + 'the file is internally what it declares; the re-walk proves the declaration is DV-T1\'s.',
  };
})();
...
const flagHygiene = {
  pass: configIdentity.pass
    && divergentSeeds > 0
    /** the CONTROL has NO seam at all; the other two open the plane door and only it. */
  ...
  note: '⭐⭐ THE THREE ARMS DIFFER BY EXACTLY THE RISK PRICE. The control has no seam; the anchor '
    + 'and the dosed arm open the SAME banked strike-plane door at the SAME gene value, and '
    + '`dvDeliveryValue` plus the two DV genes are the only things that move between them. The '
    + 'configuration-identity predicate is DERIVED HERE (never inherited) and PROVED LIVE against '
    + 'four mutants; the identity with DV-T1 itself is G-REPRO-T1\'s re-walk.',
};
...
const goalsBandOf = (a: ArmName) => {
  ...
  return {
    ...
    /** ⚠ MODE-CONDITIONED (#251.3(ii)): DV-T1 shipped this literal unconditionally, so its
     *  BATTERY artifact carried a smoke-grain disclaimer over rows that were the adjudicating
     *  evidence. The string now states the grain the run actually has. */
 ...
 * EVERY arm; the truth-dosed ladder of THIS stage is ONE rung long (`dvTruthP`, the parity rung),
 * because a power extension spends its seeds on the cell its letter is written at. DV-T1's other
 * rungs and its above-truth arm are BANKED at #251.1 and are not re-asked here.
 *
...
/**
 * ⭐⭐ THE FROZEN LETTER (ruling #251.2), PRE-REGISTERED AND MECHANICALLY PRINTED.
 *
 * It is the SAME two limbs DV-T1 froze, on virgin seeds, at the parity rung:
 *
 *   LIMB A — SUPPLY vs the CONTROL, resolved HELPFUL, at the ANCHOR **and** at `dvTruthP`;
 *            plus the `dvTruthP`-vs-ANCHOR supply delta with `resolvedKilled` printed beside it.
 *   LIMB B — `dvTruthP` GOALS vs the ANCHOR, resolved TOWARD the frozen band (the band re-derived
 *            here exactly as DV-T1 derived it: baseline 2.3944, tolerance ±15 %), with `inBand`
 *            printed too.
 *
 * ⭐⭐ AND THE THREE ROUTES, PRE-NAMED BY #251.2 AND PRINTED MECHANICALLY:
 *   (a) HELPFUL    — supply resolves helpful at the parity rung ⇒ the #244 inversion is COMPLETE
 *                    at exam grain.
 *   (b) KILLED     — supply resolves KILLED (a resolved supply LOSS: down vs the control, or
 *                    resolvedly below the anchor) ⇒ F-DV-b is true ON THE MECHANISM after all.
 *   (c) UNRESOLVED — neither, at this extension's N ⇒ the supply limb is declared below this
 *                    world's measurable grain at these doses.
 *
 * ⚠ THE PROBE PRINTS THE ROUTE AND RESOLVES NOTHING (#203). Route (c) in particular is a
 * COMMANDER'S HONESTY RULING, not a probe output: only the commander can declare a limb below the
 * world's measurable grain, and this file has no opinion about it.
 */
const supplyRouteOf = (a: ArmName) => {
  const supply = supplyLimbOf(a);
  const vsAnchor = a === ANCHOR_ARM ? null : anchorDeltaOf(a, 'trueHoldableShare');
  const resolvedKilledVsControl = supply.delta !== null && supply.ci !== null
    && (supply.ci as number[])[1] < 0;
  const resolvedKilledVsAnchor = vsAnchor !== null && vsAnchor.pairedDelta !== null
    && vsAnchor.pairedDelta.upper < 0;
  const helpful = supply.resolvedHelpful;
  const killed = resolvedKilledVsControl || resolvedKilledVsAnchor;
  return {
    arm: a,
    supplyVsControl: {
      point: supply.point, delta: supply.delta, ci: supply.ci,
      resolved: supply.resolved, resolvedHelpful: helpful,
      pointDirectionHelpful: supply.pointDirectionHelpful,
    },
    supplyVsAnchor: vsAnchor === null ? null : vsAnchor.pairedDelta,
    resolvedKilledVsAnchor,
    resolvedKilledVsControl,
    route: helpful && !killed ? 'HELPFUL' : killed ? 'KILLED' : 'UNRESOLVED',
  };
};
const preRegisteredPrimary = {
  frozenText: '⭐⭐ THE FROZEN LETTER (ruling #251.2, VERBATIM): "the frozen letter = the same two '
    + 'limbs on virgin seeds (which simultaneously re-tests the goals recovery)". LIMB A = supply '
    + 'vs control resolved helpful, at the anchor AND at dvTruthP, plus dvTruthP-vs-anchor supply '
    + 'with resolvedKilled printed. LIMB B = dvTruthP goals vs anchor resolved toward the frozen '
    + 'band (band re-derived as T1 did), with inBand printed too. Frozen in this probe\'s own '
    + 'predicates before any receipt ran; computed at every arm; adjudicated at none.',
  successRule: '⭐ THE JOINT IS UNCHANGED FROM DV-T1 (contract §3): LIMB A = `trueHoldableShare` '
    + 'paired-delta CI excludes zero UP vs the CONTROL, AND the anchor-referenced paired CI is '
    + 'not entirely below zero. LIMB B = the `goals` band dimension INSIDE the frozen band '
    + '(2.3944 ± 15 %), OR bandDistance STRICTLY BELOW the anchor\'s AND the anchor-referenced '
    + 'paired delta on `goalsPerMatch` RESOLVED in the band\'s direction. JOINT = both, at the '
    + 'same arm.',
  primaryCell: PRIMARY_ARM,
  primaryCellWhy: '⭐ THE PRIMARY CELL IS `dvTruthP` — the PARITY rung, the cell ruling #251.2\'s '
    + 'letter names for BOTH limbs. DV-T1\'s zero and gradient rungs are banked and are not '
    + 're-asked; every seed this stage owns goes into resolving THIS cell and its anchor.',
  ...
  contrastAnchor: {
    arm: ANCHOR_ARM,
    what: '⭐⭐ RULING #244\'s OWN ARM, and THE OBJECT OF THIS EXTENSION. Its supply gain read '
      + '+0.1365 pp RESOLVED helpful at #244 and +0.1344 pp UNRESOLVED on DV-T1\'s 283 virgin '
      + 'seeds — a replicated point estimate with a CI that touched zero, which is why #251.2 '
      + 'calls limb A POWER-CONFOUNDED. Re-walked here on seeds neither exam has touched, at an N '
      + 'sized to resolve exactly that estimate.',
    reading: jointAt(ANCHOR_ARM),
  },
  /** ⭐⭐ THE LETTER ITSELF — the two limbs, printed as the ruling words them. */
  frozenLetter: {
    limbA: {
      atAnchor: supplyRouteOf(ANCHOR_ARM),
      atDosedCell: supplyRouteOf(PRIMARY_ARM),
      dosedVsAnchor: {
        pairedDelta: anchorDeltaOf(PRIMARY_ARM, 'trueHoldableShare').pairedDelta,
        resolved: anchorDeltaOf(PRIMARY_ARM, 'trueHoldableShare').resolved,
        resolvedKilled: supplyRouteOf(PRIMARY_ARM).resolvedKilledVsAnchor,
      },
      note: '⭐ THE ANCHOR ROW IS THE POINT OF THE EXTENSION, not context: #251.2 established '
        + 'that limb A fails AT THE ANCHOR ITSELF on virgin seeds, so the anchor\'s own supply '
        + 'reading at this N is what decides whether that failure was power or absence.',
    },
    limbB: {
      arm: PRIMARY_ARM,
      goalsPerMatch: goalsBandOf(PRIMARY_ARM).goalsPerMatch,
      band: [goalsBandOf(PRIMARY_ARM).bandLo, goalsBandOf(PRIMARY_ARM).bandHi],
      bandProvenance: 'RE-DERIVED HERE EXACTLY AS DV-T1 DERIVED IT: baseline '
        + `${BAND_BASELINE.goals} × (1 ± ${BAND_TOLERANCE.goals}) — the A4-S2P3 §4.2 equilibrium `
        + 'band, inherited verbatim, with the #198-form exclusion (a dimension the ABSENT arm '
        + 'itself fails is excluded AND disclosed).',
      inBand: goalsBandOf(PRIMARY_ARM).inBand,
      controlAlsoInBand: goalsBandOf(PRIMARY_ARM).controlAlsoInBand,
      anchorGoalsPerMatch: goalsBandOf(ANCHOR_ARM).goalsPerMatch,
      bandDistance: bandDistanceOf(PRIMARY_ARM),
      bandDistanceAnchor: bandDistanceOf(ANCHOR_ARM),
      goalsVsAnchor: anchorDeltaOf(PRIMARY_ARM, 'goalsPerMatch').pairedDelta,
      towardBandResolved: jointAt(PRIMARY_ARM).goalsBandLimb.towardBandResolved,
      limbB: jointAt(PRIMARY_ARM).goalsBandLimb.limbB,
      note: '⭐ DV-T1 read +0.2438 [0.0459, 0.4205] RESOLVED toward the band here. The extension '
        + 're-tests it on virgin seeds as a by-product of resolving limb A — a replication '
        + 'nobody had to pay extra for.',
    },
  },
  /** ⭐⭐ THE THREE ROUTES OF #251.2, PRINTED MECHANICALLY AND RESOLVED NOWHERE. */
  routes: (() => {
    const r = supplyRouteOf(PRIMARY_ARM);
    const anchor = supplyRouteOf(ANCHOR_ARM);
    return {
      definitions: {
        HELPFUL: 'supply resolves HELPFUL at parity ⇒ the #244 inversion is COMPLETE at exam grain',
        KILLED: 'supply resolves KILLED ⇒ F-DV-b true on mechanism after all',
        UNRESOLVED: 'still UNRESOLVED at the extension\'s N ⇒ the supply limb is declared below '
          + 'this world\'s measurable grain at these doses (a COMMANDER\'s honesty ruling)',
      },
      readAt: PRIMARY_ARM,
      selected: r.route,
      atAnchor: anchor.route,
      helpful: r.route === 'HELPFUL',
      killed: r.route === 'KILLED',
      unresolved: r.route === 'UNRESOLVED',
      mdeBoughtAtThisN: (powerRule as any).mdeBoughtAtNStar ?? null,
      grain: MODE === 'full'
        ? `BATTERY grain: ${RUN_N} clusters, the N sized ex ante for exactly this reading`
        : 'SMOKE grain: this route selection is PLUMBING and selects nothing — the extension\'s '
          + 'whole purpose is a battery-N reading, and at smoke N no CI here could resolve the '
          + 'target estimate even in principle',
      status: '⚠ PRINTED, NEVER RESOLVED (#203). The route is a mechanical function of published '
        + 'CI flags; which route the programme TAKES — and in particular whether route (c) is '
        + 'declared as an honest limit of this world\'s grain — is the commander\'s alone.',
    };
  })(),
  failBranches: '⭐ PRE-NAMED (contract §3, carried unchanged): F-DV-a — the map fix does NOT move '
    + 'the deflation (limb B false with limb A holding). F-DV-b — it KILLS the supply gain (limb A '
    + 'false ⇒ overcorrection STOP) — ⚠ and #251.2 records that F-DV-b fired on DV-T1\'s LETTER '
    + 'while being REFUTED on its MECHANISM, which is the confusion this extension exists to '
    + 'settle. F-DV-c — a guard STOPs. ⚠ THIS PROBE FIRES NONE OF THEM (#203), and ⭐ a fired fork '
    + 'is still a commit.',
  secondaryReported: '⭐ REPUBLISHED AT BATTERY GRAIN (ruling #251.2\'s order): the DELIVERED RATE '
    + 'per arm; the H-250a flip-vs-suppress counterfactual AT THE PARITY DOSE with its null-arm '
    + 'validity check; the frozen guards with the same tolerances re-derived; the offside/restart '
    + 'flags in the #157 form; and the equilibrium band. No gate and no success condition reads '
    + 'any of them.',
  status: MODE === 'full'
    ? '⚠ MECHANICAL PREDICATE FLAGS ON PUBLISHED CIs AND ON THE FROZEN BAND, exactly like '
      ...
      + 'band gates here. THE PROBE STILL ADJUDICATES NOTHING — F-DV-a/b/c and the three #251.2 '
      + 'routes are the commander\'s.'
    : '⚠ MECHANICAL PREDICATE FLAGS (#203) AT SMOKE GRAIN: none of these rows is evidence, the '
      + 'band gates at battery N only, and the printed route selects nothing. F-DV-a/b/c are the '
      + 'commander\'s.',
};
...
const gates = {
  xDet: {
    ...
    note: '⭐ THE SCOPE, STATED ACCURATELY (the #251.3(iii) correction made at the source rather '
      ...
      + 'EIGHT receipt walks (O2-T1 · #173 · GGC · CTB-T1 · OBM-T1 ×2 arms · G-ANCHOR · ⭐⭐ '
      + `G-REPRO-T1 ×${ARMS.length} arms), the ${ARMS.length} delivered-dose reads, the `
  ...
  /** ⭐⭐ G-N — THE EX-ANTE POWER ARITHMETIC, MACHINE-CHECKED AGAINST THE FROZEN LITERAL.
  ...
  gN: {
    pass: powerRule.available === true
      && (powerRule as any).frozenMatchesDerivation === true
      && (powerRule as any).nStar > 0
      && (MODE === 'smoke' || RUN_N === (powerRule as any).nStar),
    available: powerRule.available,
    nFrozen: N_FROZEN,
    nStar: (powerRule as any).nStar ?? null,
    nRaw: (powerRule as any).nRaw ?? null,
    frozenMatchesDerivation: (powerRule as any).frozenMatchesDerivation ?? false,
    runN: RUN_N,
    mode: MODE,
    source: (powerRule as any).source ?? null,
    seSource: (powerRule as any).seSource ?? null,
    seRequired: (powerRule as any).seRequired ?? null,
    mdeAskedFor: (powerRule as any).mdeAskedFor ?? null,
    mdeBoughtAtNStar: (powerRule as any).mdeBoughtAtNStar ?? null,
    capBinds: (powerRule as any).capBinds ?? null,
    roomBinds: (powerRule as any).roomBinds ?? null,
    note: '⭐⭐ N SIZED EX ANTE (#251.2). The target is DV-T1\'s OWN committed anchor supply '
      + 'estimate, read out of the artifact; the SE is recovered from its published CI '
      + 'half-width; N is the cluster count at which that SE gives 80 % power to resolve that '
      + 'same estimate at 95 % two-sided. The literal in this file is checked against the '
      + 'derivation, and in FULL mode the battery is required to have RUN at it — a battery that '
      + 'quietly ran a different N would red this gate rather than publish a smaller MDE as if it '
      + 'were the frozen one.',
  },
  gTracePtp,
  gTraceSp,
  ...
  seedDisjoint: {
    ...
    coverageNote: '⭐ THIRTEEN BLOCKS, EVERY ONE MACHINE-CHECKED: 4 FRESH (exam · delivered-dose '
      + 'read · ⭐⭐ strike read · ⭐ H-250a counterfactual read — the block DV-T1\'s own list '
      + 'omitted) + 2 RESERVED (guard · battery) + 7 RE-WALKS (O2-T1 · #173 · GGC · CTB-T1 · '
      + 'OBM-T1 · ⭐⭐ DLC-T1s PLANE, the G-ANCHOR block · ⭐⭐ DV-T1 BATTERY on all three of this '
      + 'exam\'s arms, the G-REPRO-T1 block). ⭐⭐ AND THE WHOLE STAGE LIVES ABOVE 12,431,000: '
      + 'every fresh and reserved block is VIRGIN ground, strictly above everything DV-T1 '
      + 'consumed (through 12,430,382) and clear of DV-T0\'s test band — which is the entire '
      + 'point of a virgin-seed extension and is checked here rather than promised.',
    reproBlocksNote: 'the SEVEN repro blocks (O2-T1 · #173 · GGC · CTB-T1 · ⭐ OBM-T1 · ⭐⭐ '
      + 'DLC-T1s PLANE · ⭐⭐ DV-T1 BATTERY) are DELIBERATE '
  ...
  gCleanInvocation: {
    ...
    note: 'any DVT1B_N / DVT1B_SKIP_FP override is BY DEFINITION not the exam: the run is routed '
...
/* ========================================================================== */
const body = {
  stage: 'DV T1b — THE VIRGIN-SEED POWER EXTENSION (是功率不足还是真的没有?: was DV-T1\'s supply '
    + 'limb unresolved because the effect is absent, or because 283 clusters could not see it?)',
  ruling: '#251.2 (THE CHARTER: the VIRGIN-SEED POWER EXTENSION — arms CONTROL · PLANE-ANCHOR · '
    + 'dvTruthP only, configuration byte-identical to DV-T1\'s with the identity predicate DERIVED '
    + 'for this exam; N sized EX ANTE from DV-T1\'s committed artifact moments to resolve the '
    + 'anchor\'s replicated supply estimate; the same two limbs on virgin seeds; the three routes '
    + 'pre-named) · ⭐ #251.3 (the probe-hygiene lessons this probe is built against: never inherit '
    + 'a configuration predicate; mode-condition every caveat literal) · #250.3 (timings ride the '
    + 'UNHASHED envelope; percentage grep-forms FORMATTED before String()) · #250.4 (H-250a, the '
    + 'delivered-rate publication, the no-cost-% rule) · #249 (DV-C0 — THE TRUE TABLE) · #247 (the '
    + 'TRUTH/BELIEF split) · #244 (THE FAILURE THE ARC INVERTS) · DELIVERY-VALUE-CONTRACT §3 (the '
    + 'JOINT\'s two limbs, verbatim) · #225.3(c) · #203 (rows and a printed route, never verdicts)',
  doc: 'docs/world-model/DV-T1B-POWER-EXTENSION.md',
  mode: MODE,
  ...
  world: '⭐ ONE PERCEPT-ARMED BASE WORLD, IDENTICAL IN EVERY ARM: `new Match({seed, teamA, '
    ...
    + 'PAIRED contrast is clean because ALL THREE ARMS SHARE THIS WORLD EXACTLY and differ by '
    + 'nothing but the RISK PRICE. Each G-REPRO walk runs in ITS SOURCE\'s own world — and '
    + '⭐⭐ G-REPRO-T1\'s walks run in THIS world on THIS exam\'s own three arms, which is what '
    + 'makes the configuration identity with DV-T1 an earned receipt rather than a claim.',
  ...
  costRule: '⚠ NO COST PERCENTAGE IS QUOTED ANYWHERE IN THIS STAGE (#250.4), and no cost '
    ...
    + 'byte-level facts are the two receipts it does run: G-REPRO-T1 (this exam\'s three arms '
  ...
  preRegisteredSuccess: '⭐⭐ THE FROZEN LETTER (ruling #251.2): LIMB A = supply vs control '
    + 'resolved helpful, at the ANCHOR **and** at `dvTruthP`, plus the dvTruthP-vs-anchor supply '
    + 'delta with `resolvedKilled` printed. LIMB B = `dvTruthP` goals vs the anchor resolved '
    + 'TOWARD the frozen band (band re-derived here as DV-T1 derived it: 2.3944 ± 15 %), with '
    + '`inBand` printed too. ⭐⭐ AND THE THREE ROUTES, PRE-NAMED AND PRINTED MECHANICALLY: '
    + 'HELPFUL ⇒ the #244 inversion is COMPLETE at exam grain · KILLED ⇒ F-DV-b true on mechanism '
    + 'after all · UNRESOLVED at this N ⇒ the supply limb is below this world\'s measurable grain '
    + 'at these doses. The probe PRINTS the route; the commander RESOLVES it (#203).',
  ...
  primaryRulers: '⭐ THE JOINT, FROZEN EX ANTE (see `preRegisteredPrimary`): ruler 1 '
    + '(TRUE-holdable supply) RETAINED **AND** the `goals` band dimension RECOVERED, at the '
    + 'PARITY cell, read against the CONTROL and against the PLANE-ALONE ANCHOR. ⭐⭐ Ruler 1 is '
    + 'also the column the EX-ANTE N is cut on, because it is the column limb A reads. ⭐ REPORTED HEADLINES beside it: the DELIVERED RATE per arm (both '
...
o('');
o(`=== DV-T1b THE VIRGIN-SEED POWER EXTENSION · mode ${MODE} · ${body.block} `
  + `(${RUN_N} seeds/arm, shared) ===`);
o('world: PERCEPT-ARMED (edsPerceivedChoice) · ctbSupportPlane FALSE in every arm');
o(`arms: CONTROL · PLANE-ANCHOR · dvTruthP — differing by EXACTLY the RISK PRICE · Δ = ARM − ${CONTROL_ARM}`);
...
o(`  ⇒ JOINT at the truth-dosed rung = ${preRegisteredPrimary.jointAtAnyLadderRung}`);
o(MODE === 'full'
...
o('');
o('⭐⭐ THE FROZEN LETTER (#251.2) — printed, never resolved (#203)');
{
  const fl = preRegisteredPrimary.frozenLetter;
  const rt = preRegisteredPrimary.routes;
  const sr = (x: any): string => `${x.supplyVsControl.delta} `
    + `[${x.supplyVsControl.ci === null ? '' : x.supplyVsControl.ci[0]}, `
    + `${x.supplyVsControl.ci === null ? '' : x.supplyVsControl.ci[1]}]`;
  o(`  LIMB A · supply vs CONTROL at the ANCHOR      ${sr(fl.limbA.atAnchor)}`
    + ` helpful=${fl.limbA.atAnchor.supplyVsControl.resolvedHelpful}`);
  o(`  LIMB A · supply vs CONTROL at ${PRIMARY_ARM}          ${sr(fl.limbA.atDosedCell)}`
    + ` helpful=${fl.limbA.atDosedCell.supplyVsControl.resolvedHelpful}`);
  o(`  LIMB A · supply ${PRIMARY_ARM} vs ANCHOR             `
    + `${fl.limbA.dosedVsAnchor.pairedDelta === null ? 'n/a'
      : `${fl.limbA.dosedVsAnchor.pairedDelta.point} [${fl.limbA.dosedVsAnchor.pairedDelta.lower}, ${fl.limbA.dosedVsAnchor.pairedDelta.upper}]`}`
    + ` resolvedKilled=${fl.limbA.dosedVsAnchor.resolvedKilled}`);
  o(`  LIMB B · goals ${PRIMARY_ARM} ${fl.limbB.goalsPerMatch} vs anchor ${fl.limbB.anchorGoalsPerMatch}`
    + ` · band [${fl.limbB.band[0]}, ${fl.limbB.band[1]}] inBand=${fl.limbB.inBand}`
    + ` · Δ vs anchor ${fl.limbB.goalsVsAnchor === null ? 'n/a'
      : `${fl.limbB.goalsVsAnchor.point} [${fl.limbB.goalsVsAnchor.lower}, ${fl.limbB.goalsVsAnchor.upper}]`}`
    + ` towardBandResolved=${fl.limbB.towardBandResolved} ⇒ LIMB B=${fl.limbB.limbB}`);
  o('  ⭐⭐ THE THREE ROUTES (#251.2), read at ' + String(rt.readAt) + ':');
  o(`     HELPFUL    ${rt.definitions.HELPFUL}`);
  o(`     KILLED     ${rt.definitions.KILLED}`);
  o(`     UNRESOLVED ${rt.definitions.UNRESOLVED}`);
  o(`  ⇒ THE FROZEN LETTER SELECTS: ${rt.selected}  (at the anchor: ${rt.atAnchor})`);
...
o('');
o('⭐⭐ THE EX-ANTE POWER RULE (in-probe, from DV-T1\'s COMMITTED artifact)');
if (nRule.available) {
  const nr = nRule as any;
  o(`  target = DV-T1's own ANCHOR supply estimate ${nr.source.point} `
    + `[${nr.source.ci[0]}, ${nr.source.ci[1]}] at ${nr.source.clusters} clusters`);
  o(`  se(source) ${nr.seSource} = (half-width ${nr.halfWidthSource})/z.975 · `
    + `se(required) ${nr.seRequired} = |point|/(z.975+z.80)`);
  o(`  N_raw ${nr.nRaw} = ceil(${nr.source.clusters} · (se_source/se_required)^2) ⇒ N* ${nr.nStar}`
    + ` (cap ${nr.nCap} binds=${nr.capBinds} · ledger room ${nr.batteryRoom} binds=${nr.roomBinds})`);
  ...
  o(`  MDE asked for ${nr.mdeAskedFor} · MDE BOUGHT at N* ${nr.mdeBoughtAtNStar}`
    + ` · expected CI half-width at N* ${nr.expectedHalfWidthAtNStar}`);
  o(`  battery block ${nr.batteryBlock} · ${nr.capForkNote}`);
  o('  ⭐ REALISED vs EX-ANTE (the assumption this rule made, checked after the fact):');
  {
    const cell = C.trueHoldableShare[ANCHOR_ARM].pairedDelta;
    const realised = cell === null ? null : (cell.upper - cell.lower) / 2;
    o(`     anchor supply CI half-width — ex-ante expectation ${nr.expectedHalfWidthAtNStar}`
