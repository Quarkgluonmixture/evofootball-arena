...
// Authority: docs/world-model/EDS-E3R-REVISED-BUNDLE-AUDIT.md
// (pre-registered 2026-07-25 under commander ruling #12.4, constraints (a)-(f)).
//
// E3 refuted the v1 thesis by measuring it: the band break was MECHANICAL, the
// chooser never chose it, and the ablation showed the same bundle MINUS the
// touch cost inside every band. Ruling #12 took touch cost out of the live set.
// This probe re-audits what remains — perceived choice + perceived defence +
// the evaluator — in full, because a diagnostic arm is not a pass.
//
//   C1  CHAIN FIRST (constraint (a)): the reference chooser, pricing declared
//       flag-off, must reproduce E2b-1R's banked aggregates bit-identically,
//       with G1 (not-looking must not win) and G2 (route mix) re-holding there.
//   X4R the live consumer must then chain to THAT reference, per moment per arm.
//   §2  the equilibrium band C1-B broke, paired 8-season calibrate.
//   ND  no-strict-dominance — E0's canary, live, purpose documented.
//   CE  co-evolution restoration + style, with H1/H2 pre-registered, plus the
//       D1/D2/D3 diagnostics that tell the two hypotheses apart.
...
/** Reported robustness only (contract §3): CE1/CE2 are judged on EVO_SEED. */
const EVO_SEED_ROBUSTNESS = 515151;
...
const runExperiment = () => {
  ...
  const evoRobustness = runs('evo') ? {
    baseline: runLeagueArm({
      seed: EVO_SEED_ROBUSTNESS, seasons: EVO_SEASONS, bundle: false, trace: false,
    }),
    bundle: runLeagueArm({
      seed: EVO_SEED_ROBUSTNESS, seasons: EVO_SEASONS, bundle: true, trace: false,
    }),
  } : null;
  ...
  if (x4) {
    gates.c1dHarnessReproduces = harness.every((entry) => entry.reproduces);
    // C1a: the re-banked reference reproduces E2b-1R's aggregates, digit for digit.
    for (const [key, value] of Object.entries(x4.banked)) gates[`c1aReBank_${key}`] = value;
    gates.c1bG1NonInferiority = x4.g1NonInferiority;
    gates.c1cG2LongShare = x4.g2LongShare;
    gates.c1cG2MeanDistance = x4.g2MeanDistance;
    // X4R: the live consumer chains to that reference, per moment per arm.
    gates.x4rPerMomentIdentical = x4.perMomentIdentical;
  }
  if (bandGate) {
  ...
  return {
    experiment: 'EDS-E3R',
    authority: 'EDS-E3R-REVISED-BUNDLE-AUDIT',
    section: SECTION,
    parameters: {
    ...
    dominance,
    coEvo,
    styleHypotheses: coEvo === null ? null : {
      // H1 touch-flattening: without the E1b curve the entropy recovers => CE2
      // passes, the question is closed and benign. H2 genome-blind chooser: it
      // does not, and the fork returns to the commander for a designed
      // PREFERENCE SEAT (ruling #12.4 (c)). The gate decides; D1/D2/D3 say why.
      entropyRatio: coEvo.styleSpread.entropyRatio,
      nameplateRatio: coEvo.styleSpread.nameplateRatio,
      h1Confirmed: coEvo.styleHeld,
      h2Stands: !coEvo.styleHeld,
      e3EntropyRatio: 0.49666123012796176,
      diagnostics: {
        bundle: evo?.bundle.diagnostics ?? null,
        flagsOff: evo?.baseline.diagnostics ?? null,
        bandBundle: band?.bundle.diagnostics ?? null,
        bandFlagsOff: band?.baseline.diagnostics ?? null,
      },
    },
    evoRobustness: evoRobustness === null ? null : {
      seed: EVO_SEED_ROBUSTNESS,
      entropy: {
        bundle: evoRobustness.bundle.styleSpread[evoRobustness.bundle.styleSpread.length - 1],
        flagsOff: evoRobustness.baseline.styleSpread[
          evoRobustness.baseline.styleSpread.length - 1],
      },
      advantage: evoRobustness.bundle.seasons.map((row, index) =>
        row.goalsPerMatch - evoRobustness.baseline.seasons[index].goalsPerMatch),
      goals: {
        bundle: evoRobustness.bundle.totals.goals, flagsOff: evoRobustness.baseline.totals.goals,
      },
    },
    reported: {
...
console.error(
  `EDS-E3R ${output.verdict} · section ${SECTION}`
  + (output.c1AndX4r ? ` · X4R ${output.c1AndX4r.perMomentIdentical}`
    + ` (${output.c1AndX4r.disagreements}/${output.c1AndX4r.comparisons})`
    + ` reBank ${Object.values(output.c1AndX4r.banked).filter(Boolean).length}/7`
    + ` G1 ${output.c1AndX4r.g1NonInferiority} G2 ${output.c1AndX4r.g2LongShare}/${output.c1AndX4r.g2MeanDistance}` : '')
  ...
  + (output.coEvo ? ` · coEvo ${output.coEvo.early.toFixed(3)}→${output.coEvo.late.toFixed(3)}`
    + ` · entropy ${output.coEvo.styleSpread.entropyRatio.toFixed(3)}`
    + ` (${output.styleHypotheses?.h1Confirmed ? 'H1' : 'H2'})` : '')
