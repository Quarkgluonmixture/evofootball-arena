...
const runExperiment = () => {
  ...
  const coverage = {
    c1Quintiles: threatA.every((bin) => bin.n >= QUINTILE_FLOOR)
      && threatB.every((bin) => bin.n >= QUINTILE_FLOOR),
  };
  const discriminationGate = {
    c2Discriminates: discrimination >= DISCRIMINATION_FLOOR,
  };
  const calibrationGates = {
    c3Quintiles: calibration.every((entry) => entry.error <= CALIBRATION_TOLERANCE),
    c3Marginal: Math.abs(marginalSuccessA - marginalSuccessB) <= CALIBRATION_MARGINAL_TOLERANCE,
  };
  ...
  return {
    experiment: 'EDS-E2b-0',
    authority: 'EDS-E2B0-THREAT-CALIBRATION',
    parameters: {
...
console.error(
  `EDS-E2b-0 ${output.verdict} · X5 harness ${output.exact.x5HarnessReproduces}`
  + ` · X5 reproduces-E2a2 ${output.exact.x5ReproducesE2a2}`
  + ` · X6 ${output.exact.x6CommittedCalibrationMatches}`
  + ` · threat quintiles ${output.threatQuintilesA.map((q) => (q.realizedSuccess * 100).toFixed(1)).join('/')}`
  + ` · discrimination ${(output.discrimination.topVsBottom * 100).toFixed(2)}pp`
  + ` · spreads threat ${(output.reported.r2WhichReadCarriesMore.threatSpread * 100).toFixed(2)}pp`
  + ` vs band ${(output.reported.r2WhichReadCarriesMore.bandPriorSpread * 100).toFixed(2)}pp`
  + ` · SHA ${sha256}`,
);
