...
console.log(`shots/match:        ${per(shots)}  (on target: ${per(onTarget)}, blocked: ${per(blocks)}, xG: ${per(xg)})`);
console.log(`passes/match:       ${per(passes)}  (completion: ${(completed / Math.max(passes, 1) * 100).toFixed(0)}%, through balls: ${per(throughBalls)}, forward share: ${(passesForward / Math.max(passes, 1) * 100).toFixed(0)}%, one-touch: ${per(oneTouch)})`);
console.log(`aerial/match:       crosses ${per(crosses)}, headers won ${per(headers)}, long balls ${per(longBalls)}`);
console.log(`cutbacks/match:     ${per(cutbacks)}  (goals off them: ${per(cutbackGoals)})`);
