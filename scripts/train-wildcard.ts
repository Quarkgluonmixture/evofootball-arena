...
// A champion trained before new policy keys existed (Phase 28 added five)
// backfills them from the defaults — otherwise the ES mutates NaN.
const prevChampion: WildcardCandidate = {
  genome: PREV_CHAMPION.genome,
  policies: PREV_CHAMPION.policies.map((p) => ({ ...DEFAULT_POLICY, ...p })),
};
let pop: WildcardCandidate[] = [base, prevChampion];
while (pop.length < POP) pop.push(mutateCandidate(pop.length % 2 ? prevChampion : base, rng, 0.12));
...
const prevPts = report('previous champion', prevChampion, 1);
 ...
 */
export const WILDCARD: StoredWildcardCandidate = ${JSON.stringify(best!.candidate, null, 2)};
`,
);
