...
describe('first touch and forward pressure in match play (Phase 27)', () => {
  ...
  it('a keeper who held the ball throws it out — never a panic hoof (28.3)', { timeout: 30000 }, () => {
    ...
    let sawThrow = false;
    // Seeds 58/182/207 are the throw-producers under the phase-92
    // containment repricing (re-probed 1..250 — the perennial dance:
    // 51.2, 58, 59, 41.2, 67, 70, 87 all reshuffled hold-release
    // choices); they go LAST so the no-hoof contract first scans
    // ordinary matches.
    for (const seed of [8, 38, 6, 19, 29, 58, 182, 207]) {
